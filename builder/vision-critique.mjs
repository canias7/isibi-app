...
//
// This is a BUILD-CONTAINER module (Node + Playwright), not a Worker module — worker.js never imports it, and
// Playwright is lazy-loaded so importing this module is cheap. The vision call uses ANTHROPIC_API_KEY exactly like
// worker.js's existing /v1/messages calls, so it activates wherever that key is present.
