...
// brandFrom(files, fallback) — the business name for the nav, derived in code ($0).
//
// A live build shipped a barbershop site whose header still read "Pipeline": the starter's template
// placeholder. The adapt step is asked for index.html and Home.tsx, and the brand lives in the shared
// Nav — a file the model is deliberately never asked to touch, so nothing could ever fix it.
//
// The model DOES write the real business name, into <title>, so take it from there. A title is
// conventionally "<Business> — <tagline>", so keep the part before the first separator. The planner's
// spec.name is the fallback, and it is deliberately second: it is derived from the brief's shape
// ("Booking Site"), not from the business.
export function brandFrom(files, fallback = "") {
  const html = (files && files["index.html"]) || "";
  const title = (html.match(/<title>([^<]*)<\/title>/i) || [, ""])[1].trim();
  const head = title.split(/\s+[—–|·-]\s+/)[0].trim();
  const pick = head || String(fallback || "").trim();
  // The placeholder must never win. If the model somehow echoed it back, fall through.
  return (/^pipeline$/i.test(pick) ? String(fallback || "").trim() : pick) || "App";
}

// routesModule(pages, brand) — src/routes.ts: the manifest the templated Nav reads. SignIn is excluded
// from navLinks (it lives in the nav's sign-in button, not the link row). `brand` rides along here
// because the nav already reads this file, so the name arrives without touching a component.
export function routesModule(pages, brand = "App") {
  ...
  return "// GENERATED by the scaffold from the pages that exist — do not edit by hand.\n" +
    ...
    "// The business name, shown in the nav. Derived from this app's own <title>.\n" +
...
export function scaffoldRouting(files, opts = {}) {
  ...
  out["src/routes.ts"] = routesModule(pages, brandFrom(out, opts.brand));
