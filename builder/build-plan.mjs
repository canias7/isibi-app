...
// With a whole-app STARTER matched, step 1 stops being "write Home + SignIn" and becomes "make this working app
// be about THIS business" — it re-emits the landing copy and, at most, a schema tweak. Everything else is already
// correct, so the step is budgeted for an edit rather than a build.
export const ADAPT_TOKENS = 3000;
...
export function buildPlan(spec, cap, opts = {}) {
  ...
  // opts.starter = { id, pages: ['Home','SignIn','Book',…], covers: ['bookings','slots',…] } when a whole-app
  // starter matched the brief. Two things change: step 1 is an ADAPT (cheap edit) instead of a SHELL
  // (from-scratch build), and every capability the starter already delivers is not generated at all. The second
  // is where the real saving is.
  //
  // Coverage is DECLARED (`covers`), not inferred from page names. A booking starter delivers the `bookings`
  // capability through pages called Book and MyBookings; matching on names alone silently regenerated both.
  const starter = opts.starter && Array.isArray(opts.starter.pages) ? opts.starter : null;
  const starterPages = new Set(starter ? starter.pages : []);
  const starterCovers = new Set(starter ? (starter.covers || []) : []);
  const shellEst = opts.shellTokens || (starter ? ADAPT_TOKENS : SHELL_TOKENS);
  ...
  const steps = starter
    ? [{ kind: "adapt", budget: shellBudget, starter: starter.id, includes: ["Home copy", "schema tweaks"] }]
    : [{ kind: "shell", budget: shellBudget, includes: ["scaffolding", "components", "Home", "SignIn", "nav", "router"] }];
  const admin = (spec.roles || []).includes("admin") && !starterPages.has("Admin");
  ...
  return { ok: true, cap, budget: cap, genBudget, reserves, reserveTotal, genTotal, total, steps, stepCount: steps.length,
    included: included.map((x) => x.id), dropped, covered, starter: starter ? starter.id : null, warnings };
}

// Capability id → the page name a starter would use for it ("slot-check" → "SlotCheck").
function pascalPage(id) { return String(id || "").replace(/(^|[-_ ])([a-z])/g, (_, __, c) => c.toUpperCase()).replace(/[-_ ]/g, ""); }
...
export function planSummary(plan) {
  ...
  for (const s of plan.steps) {
    ...
    const what = s.kind === "shell" ? "shell (index.html + Home + SignIn)"
      : s.kind === "adapt" ? `adapt the "${s.starter}" starter to this brief`
  ...
  return lines.join("\n") +
    ...
    (plan.covered && plan.covered.length ? `\n  covered by the "${plan.starter}" starter (not generated): ${plan.covered.join(", ")}` : "") +
