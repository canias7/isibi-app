    ...
    <form className={cx('card-surface p-6', className)}
      ...
      <div className="mt-5 space-y-4">
        <div className="grid gap-4 sm:grid-cols-2">
          <Input label="Full name" value={v.name || ''} onChange={(e) => set('name', e.target.value)} autoComplete="name" required />
          <Input label="Email" type="email" value={v.email || ''} onChange={(e) => set('email', e.target.value)} autoComplete="email" required
        ...
        <Textarea label="Cover note" rows={5} value={v.cover || ''} onChange={(e) => set('cover', e.target.value)}
          placeholder="Why this role, and what you would bring to it." />
        {questions.map((q) => {
          if (q.type === 'textarea') return <Textarea key={q.id} label={q.label} hint={q.hint} rows={3} value={v[q.id] || ''} onChange={(e) => set(q.id, e.target.value)} required={q.required} />
          if (q.type === 'select') return (
            <label key={q.id} className="block">
              <span className="mb-1.5 block text-sm font-medium text-ink-700">{q.label}</span>
              <select value={v[q.id] || ''} onChange={(e) => set(q.id, e.target.value)} required={q.required}
          ...
          if (q.type === 'checkbox') return (
            <label key={q.id} className="flex cursor-pointer items-start gap-3 text-sm">
              <input type="checkbox" checked={!!v[q.id]} onChange={(e) => set(q.id, e.target.checked)} className="mt-0.5 h-4 w-4 shrink-0 rounded accent-brand-600" />
              <span className="text-ink-700">{q.label}</span>
            </label>
          )
          return <Input key={q.id} label={q.label} hint={q.hint} value={v[q.id] || ''} onChange={(e) => set(q.id, e.target.value)} required={q.required} />
        })}
        {consent && (
          <label className="flex cursor-pointer items-start gap-3 text-sm">
            <input type="checkbox" checked={!!v.consent} onChange={(e) => set('consent', e.target.checked)} className="mt-0.5 h-4 w-4 shrink-0 rounded accent-brand-600" />
