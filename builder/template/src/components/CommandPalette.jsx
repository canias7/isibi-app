...
export default function CommandPalette({ items = [], placeholder = 'Search or jump to…', open: openProp, onOpenChange, hotkey = 'k' }) {
    ...
    <div className="fixed inset-0 z-[60] flex items-start justify-center bg-ink-900/40 p-4 pt-[12vh] backdrop-blur-[2px]"
      ...
      <div className="w-full max-w-xl overflow-hidden rounded-2xl border border-ink-100 bg-surface shadow-xl">
        <div className="flex items-center gap-3 border-b border-ink-100 px-4">
          ...
          <input autoFocus value={q} onChange={(e) => setQ(e.target.value)} onKeyDown={onKeyDown} placeholder={placeholder}
            aria-label={placeholder} className="w-full bg-transparent py-4 text-sm text-ink-900 placeholder:text-ink-400 focus:outline-none" />
