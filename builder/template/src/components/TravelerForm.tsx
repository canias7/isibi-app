  ...
  return (
    <fieldset className={cx('card-surface p-5', className)}>
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        <Select label="Title" value={value.title || ''} onChange={(e) => set('title', (e.target as any).value)}>
          <option value="">Choose…</option>
          {['Mr', 'Ms', 'Mrs', 'Mx', 'Dr'].map((t) => <option key={t} value={t}>{t}</option>)}
        </Select>
        <div className="hidden sm:block" />
        <Input label="First name(s)" value={value.firstName || ''} onChange={(e) => set('firstName', (e.target as any).value)} autoComplete="given-name" required />
        <Input label="Last name" value={value.lastName || ''} onChange={(e) => set('lastName', (e.target as any).value)} autoComplete="family-name" required />
        {requireDob && (
          <Input label="Date of birth" type="date" value={value.dob || ''} onChange={(e) => set('dob', (e.target as any).value)} autoComplete="bday" required />
        )}
        <Select label="Nationality" value={value.nationality || ''} onChange={(e) => set('nationality', (e.target as any).value)}>
          <option value="">Choose…</option>
          {['United Kingdom', 'United States', 'Ireland', 'Australia', 'Canada', 'France', 'Germany', 'Spain', 'Other'].map((c) => <option key={c} value={c}>{c}</option>)}
        </Select>
        {requirePassport && (
          <>
            <Input label="Passport number" value={value.passport || ''} onChange={(e) => set('passport', (e.target as any).value)} />
            <Input label="Passport expiry" type="date" value={value.passportExpiry || ''} onChange={(e) => set('passportExpiry', (e.target as any).value)}
