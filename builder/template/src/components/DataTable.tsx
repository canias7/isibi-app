...
export default function DataTable({ columns = [], rows = [], loading = false, empty, rowKey = (r, i) => r.id ?? i, onRowClick, className }: DataTableProps) {
  const [sort, setSort] = useState<Record<string, any>>({ key: null, dir: 'asc' })
  ...
  if (!rows.length) return <EmptyState title="Nothing here yet" description="Records will show up once they're created." className={className} />
