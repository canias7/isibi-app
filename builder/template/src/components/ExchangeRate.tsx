...
interface ExchangeRateProps {
  ...
  updated?: any
  currencies?: any[]
  onChangeFrom?: (...args: any[]) => any
  onChangeTo?: (...args: any[]) => any
  className?: string
}
    ...
    <div className={cx('card-surface p-5', className)}>
      <div className="space-y-2">
        <div className="flex items-center gap-3 rounded-xl border border-ink-200 px-3.5 py-3">
          <input type="number" inputMode="decimal" value={amount} onChange={(e) => setAmount((e.target as any).value)} aria-label={`Amount in ${a}`}
          ...
          {currencies && onChangeFrom
            ? <select value={a} onChange={(e) => onChangeFrom((e.target as any).value)} className={sel}>{currencies.map((c) => <option key={c} value={c}>{c}</option>)}</select>
        ...
        <div className="flex items-center gap-3 rounded-xl bg-ink-50 px-3.5 py-3">
          ...
          {currencies && onChangeTo
            ? <select value={b} onChange={(e) => onChangeTo((e.target as any).value)} className={sel}>{currencies.map((c) => <option key={c} value={c}>{c}</option>)}</select>
