...
export default function Drawer({ open, onClose, title, side = 'right', width = 'max-w-md', children, footer }: DrawerProps) {
  useEffect(() => {
    if (!open) return
  ...
  if (!open) return null
  return createPortal(
    <div className="fixed inset-0 z-50 flex" role="dialog" aria-modal="true" aria-label={String(title ?? "")}>
