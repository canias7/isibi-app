  ...
  return (
    <div className={cx('card-surface flex items-center gap-4 p-4', className)}>
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-2 flex items-center gap-2">
          <input type="range" min={0} max={dur || 0} value={time} aria-label="Seek"
            onChange={(e) => { const v = Number((e.target as any).value); setTime(v); if (el.current) el.current.currentTime = v }}
      ...
      <audio ref={el} src={src} preload="metadata" onEnded={() => setPlaying(false)}
        onTimeUpdate={(e) => setTime((e.target as any).currentTime)} onLoadedMetadata={(e) => setDur((e.target as any).duration)} />
    </div>
  )
}
