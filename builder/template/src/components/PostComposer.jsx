...
export default function PostComposer({
  ...
  const addMedia = (files) => setMedia((m) => [...m, ...Array.from(files || []).slice(0, 4 - m.length)
    ...
    <div className={cx('card-surface p-4', className)}>
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          <textarea value={text} onChange={(e) => setText(e.target.value)} rows={3} placeholder={placeholder} aria-label={placeholder}
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-ink-100 pt-3">
        <label className="cursor-pointer rounded-lg p-2 text-ink-400 transition hover:bg-ink-100 hover:text-brand-600 focus-within:ring-2 focus-within:ring-brand-500">
          ...
          <input type="file" accept="image/*" multiple className="sr-only" onChange={(e) => { addMedia(e.target.files); e.target.value = '' }} />
        </label>
        {onEmoji && (
        ...
        {audiences.length > 1 && (
          <select value={audience} onChange={(e) => setAudience(e.target.value)} aria-label="Who can see this"
