...
export default function ImageCompare({ before, after, beforeAlt = 'Before', afterAlt = 'After', labels = true, aspect = '16/10', className }: ImageCompareProps) {
  ...
  return (
    <div className={cx('relative select-none overflow-hidden rounded-xl2 bg-ink-100', className)} style={{ aspectRatio: aspect }}>
      ...
      <input type="range" min={0} max={100} value={pos} onChange={(e) => setPos(Number((e.target as any).value))}
