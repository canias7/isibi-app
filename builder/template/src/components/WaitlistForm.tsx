    ...
    <form onSubmit={submit} className={cx('card-surface p-6', className)}>
      ...
      <div className="mt-4 space-y-3">
        {fields.includes('name') && (
          <input value={values.name || ''} onChange={(e) => set('name', (e.target as any).value)} placeholder="Your name" aria-label="Your name" autoComplete="name"
        ...
        <input type="email" value={values.email || ''} onChange={(e) => set('email', (e.target as any).value)} placeholder="you@example.com" aria-label="Email address" autoComplete="email" required
          className="w-full rounded-xl border border-ink-200 bg-surface px-3.5 py-2.5 text-sm text-ink-900 placeholder:text-ink-400 transition-colors hover:border-ink-300 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/40" />
        {fields.includes('quantity') && (
          <input type="number" min="1" max="10" value={values.quantity || 1} onChange={(e) => set('quantity', (e.target as any).value)} aria-label="How many places"
