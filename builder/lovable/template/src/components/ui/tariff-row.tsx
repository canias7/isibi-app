    ...
    <li className={cn("space-y-0.5 px-3 py-2 text-sm", className)}>
      <p className="flex flex-wrap items-baseline gap-x-2">
        ...
        {estimateAnnual !== undefined && (
          <span className="shrink-0 tabular-nums">{money.format(estimateAnnual)} a year</span>
        )}
      </p>
      ...
      {(endsOn || exitFee !== undefined) && (
        <p className="text-xs">
          ...
          {endsOn && exitFee !== undefined ? " · " : ""}
          {exitFee !== undefined && (exitFee > 0 ? `${money.format(exitFee)} to leave early` : "No exit fee")}
        </p>
      )}
    </li>
  );
