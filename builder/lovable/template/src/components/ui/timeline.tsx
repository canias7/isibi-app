...
export function Timeline({ items, className }: { items: Moment[]; className?: string }) {
  return (
    <ol className={cn("relative flex flex-col gap-6 border-l border-border pl-6", className)}>
      {items.map((m, i) => (
        <li key={m.title + i} className="relative">
          <span className="absolute -left-[1.6875rem] top-1.5 size-2.5 rounded-full bg-foreground" aria-hidden="true" />
