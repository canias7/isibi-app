...
const API =
  "/api/db/" + (typeof location !== "undefined" ? location.pathname.split("/")[2] || "" : "");
...
const token = {
  ...
  set: (v: string) => {
    try {
      ...
      // Storage is unavailable in private mode and inside some embedded webviews. An app that
      // cannot remember a session is still an app that works; one that throws on boot is not.
    }
  },
};
...
// A table the generated types have not caught up with must stay usable, and `unknown` would make
// every field access a compile error. `any` is the deliberate escape hatch for that one case.
...
class Query<T extends string> implements PromiseLike<Result<Row<T>[]>> {
  ...
  constructor(
    private table: T,
    private scope: "rows" | "public" = "rows",
  ) {}
  ...
  ne(column: string, value: unknown) {
  ...
  then<R1 = Result<Row<T>[]>, R2 = never>(
    ...
    return request<{ rows: Row<T>[] }>(this.path())
      .then(
        (r) =>
          ({
            data: r.data ? (r.data.rows ?? (r.data as unknown as Row<T>[])) : null,
            error: r.error,
          }) as Result<Row<T>[]>,
      )
...
class TableRef<T extends string> {
  ...
  insert(values: Partial<Row<T>>) {
    return request<Row<T>>(`/rows/${this.table}`, { method: "POST", body: JSON.stringify(values) });
  }
  update(id: string | number, values: Partial<Row<T>>) {
    return request<Row<T>>(`/rows/${this.table}/${id}`, {
      method: "PATCH",
      body: JSON.stringify(values),
    });
  }
  delete(id: string | number) {
    return request<{ ok: true }>(`/rows/${this.table}/${id}`, { method: "DELETE" });
  }
}
...
export const db = {
   ...
   */
  publicFrom<T extends string>(table: T) {
    return new Query(table, "public");
  },

  auth: {
    async signUp(email: string, password: string): Promise<Result<Session>> {
      const r = await request<{ token: string; user: Session["user"] }>("/auth/signup", {
        method: "POST",
    ...
    async signIn(email: string, password: string): Promise<Result<Session>> {
      const r = await request<{ token: string; user: Session["user"] }>("/auth/login", {
        method: "POST",
        body: JSON.stringify({ email, password }),
      });
      if (r.data?.token) token.set(r.data.token);
    ...
    signOut() {
      token.set("");
      return Promise.resolve({ data: null, error: null } as Result<null>);
    },
