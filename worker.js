  ...
  for (const st of steps) {
    try {
      if (st.do === "read") {
        ...
        let eUrl, eHeaders, eBody;
        if (st.provider === "sendgrid") { eUrl = "https://api.sendgrid.com/v3/mail/send"; eHeaders = { Authorization: "Bearer " + key, "Content-Type": "application/json" }; eBody = { personalizations: [{ to: [{ email: to }] }], from: { email: from }, subject, content: [{ type: "text/html", value: html }] }; }
        else if (st.provider === "postmark") { eUrl = "https://api.postmarkapp.com/email"; eHeaders = { "X-Postmark-Server-Token": key, "Content-Type": "application/json", Accept: "application/json" }; eBody = { From: from, To: to, Subject: subject, HtmlBody: html }; }
        else { eUrl = "https://api.resend.com/emails"; eHeaders = { Authorization: "Bearer " + key, "Content-Type": "application/json" }; eBody = { from, to, subject, html }; }
        try {
          const resp = await fetch(eUrl, { method: "POST", headers: eHeaders, body: JSON.stringify(eBody), signal: AbortSignal.timeout(12000) });
          await readCapped(resp, 4096);
          data.steps[st.as] = { ok: resp.status >= 200 && resp.status < 300, status: resp.status };
        } catch { data.steps[st.as] = { error: "email send failed" }; }
