...
37b03f01 plus the final connection/schema prerequisite guards. No merge, deployment, paid request, customer-data change or demo repair.
...
- 452 focused tests passed: edit-lock, edit-result-display, edit-poll,
...
- Required unit and site-build CI run on the final candidate. The earlier site-build run was
  superseded after a regression exposed a recorded database with no resolvable
  connection bypassing schema validation. The subsequent shape check also
  distinguishes parsed malformed schema from a valid empty schema. Both were
  reproduced through the real edit route and corrected. Results and run links are recorded in the delivery report.
