  ...
  return (
    <SiteChrome {...CHROME}>
      <div className="mx-auto max-w-2xl px-6 py-16">
        ...
        {member.data && <NotesArea name={member.data.name} onSignOut={() => logout.mutate()} />}

        {!member.isPending && !member.data && (
          <>
            <h1 className="text-3xl font-semibold tracking-tight">Your notes</h1>
            <p className="mt-2 text-muted-foreground">
              Sign in to keep private notes on your practice — what felt good, what to work on next time.
            </p>
...
function NotesArea({ name, onSignOut }: { name: string; onSignOut: () => void }) {
  ...
  return (
    <>
      ...
      <div className="mt-8">
        ...
        {notes.isPending && <Skeleton className="mt-4 h-24 rounded-xl" />}
        {notes.isError && <p className="mt-4 text-sm text-destructive">Couldn't load your notes. Refresh and try again.</p>}
        {notes.data?.length === 0 && (
          <div className="mt-4"><Empty title="No notes yet" description="Save one above after your next class." /></div>
        )}
        {!!notes.data?.length && (
