        ...
        {!member.isPending && !member.data && (
          <>
            ...
            <p className="mt-2 text-muted-foreground">
              Sign in to see studio announcements and keep your own class notes.
            </p>
  ...
  return (
    <>
      ...
      <Card className="mt-8">
        ...
        <CardContent>
          {announcements.isPending && <Skeleton className="h-20 rounded-md" />}
          {announcements.isError && (
            <p className="text-sm text-destructive">Couldn't load announcements.</p>
          )}
          {announcements.data?.length === 0 && (
            <Empty title="Nothing posted yet" description="Check back for studio updates." />
          )}
      ...
      <Card className="mt-6">
        ...
        <CardContent>
          ...
          {notes.data?.length === 0 && (
            <Empty
              title="No notes yet"
              description="Jot down anything you want to remember before your next class."
            />
          )}
          {!!notes.data?.length && (
            <ul className="grid gap-3 motion-stagger">
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="mt-6 grid gap-4">
              ...
              />
              <Button type="submit" className="motion-press w-fit" disabled={create.isPending}>
