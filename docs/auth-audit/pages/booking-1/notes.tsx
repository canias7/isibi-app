  ...
  return (
    <SiteChrome {...CHROME}>
      <div className="mx-auto max-w-2xl px-6 py-16">
        ...
        {!member.isPending && !member.data && <SignInPrompt />}
  ...
  return (
    <>
      ...
      <p className="mt-2 text-muted-foreground">
        Sign in to keep private notes on your practice — poses to work on, how a class felt, anything
...
function SignedInNotes() {
  ...
  return (
    <>
      ...
      <p className="mt-2 text-muted-foreground">
        Signed in as {member.data?.email}
      </p>
      ...
      <div className="mt-8">
        ...
        {open && (
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-4 rounded-xl border bg-card p-5">
              <FormField
                control={form.control}
                name="title"
                render={({ field }) => (
      ...
      <div className="mt-8 grid gap-4">
        ...
        {notes.data?.length === 0 && (
          <Empty
            title="No notes yet"
            description="Add one after your next class — how it felt, what to work on next time."
          />
        )}
        {notes.data?.map((n) => (
          <Card key={n.id}>
