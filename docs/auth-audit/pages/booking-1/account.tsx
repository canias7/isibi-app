...
import { activityFeedPlaceholder } from "@/components/ui/activity-feed";
    ...
    <>
      ...
      <Card className="mt-6">
        ...
        <CardContent>
          ...
          {notes.data?.length === 0 && (
            <Empty title="No notes yet" description="Jot down what worked in a class, or what to try next time." />
          )}
          {!!notes.data?.length && (
            <ul className="grid gap-3 motion-stagger">
              {notes.data.map((n) => (
