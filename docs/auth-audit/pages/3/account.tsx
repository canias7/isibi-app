        ...
        {!member.isPending && !member.data && (
          <>
            ...
            <p className="mt-2 text-muted-foreground">
              Sign in, or make an account to keep your practice notes.
            </p>
...
function SignedIn({ name, role, onSignOut }: { name: string; role: string; onSignOut: () => void }) {
  const announcements = useRows<Announcement>("announcements", { order: "title", dir: "asc" });
        ...
        <CardContent>
          ...
          {role === "admin" && (
            <p className="mt-4 text-xs text-muted-foreground">
              You have admin access — announcements are managed inside the studio's isibi dashboard.
            </p>
          )}
        </CardContent>
      </Card>
