...
**19/20 samples clean.** 4 briefs × 5 samples, one call each.
...
- output 1546 tok/sample · fresh in 4725 · cache read 386156 · write 20324
- 0.670 credits for the run
...
- **capacity 1** — clean
- **capacity 2** — clean
- **capacity 3** — capacityFn (no functions declared at all); tablesSurvive (no tables)
  - why: `tables` was a string that is NOT valid JSON — a stringified list would have been recovered [stop=tool_use, out=1820 tok]
- **capacity 4** — clean
- **capacity 5** — clean
