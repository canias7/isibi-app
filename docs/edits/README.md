...
| 21 | 2026-09-02 | `slug` did not run in run 16. The lanes box said `kind,slug.` — a full stop after the last name — and the harness dropped the name it did not know without a word, so the log read as a complete pass of one lane. Fixed in the tree: a name that is not a lane refuses before sign-in, and `slug.` reads as `slug`. Still unrun: `crookes-guitar.gofarther.app` answers 404 and there is no alias row. | (no capture) |
