...
1. **Bug log** â things the owner found broken or not-as-wanted, tracked to resolution.
2. **How the owner likes things done** â durable preferences/patterns so a fresh
...
## 2026-07-21 â BACKEND ROADMAP: building the "100 more" list (AI parked)
...
- **Batch 1 â validation & integrity** (PR #533, live 16/16): column `default`,
  `enum`, `min`/`max` (numeric bounds / text length), `pattern` regex â all enforced
  server-side (bad value â 400). **Atomic column increment** `POST /rows/<t>/<id>/incr
...
- **Batch 2 â soft-delete / trash / restore** (PR #535, live 17/17): table `"trash":true`
  â `deleted_at`; DELETE soft-hides (`?hard=1` = permanent), reads exclude trashed by
  default (`?trashed=1` / `?withTrashed=1`), `POST /rows/<t>/<id>/restore`. Threaded
  through list + get-by-id + stats + scoped delete/restore.
- **Batch 3 â query power + tags** (this PR): multi-column sort (`sort=-price,name`),
  where operators `startswith|endswith|in|nin|isnull|notnull`, and **Tags** (`_tags`:
  add/remove/list per row, `?tag=` filter, `?tags=1` attach â same write scope as the row).

Progress: ~8 of 93 roadmap items shipped. ð-flagged items (OAuth, SMS, web push,
Stripe extensions) will need the owner's provider keys for live verification â batched
toward the end.

**Batches 4â8 (all shipped + verified live, no keys):**
- **Batch 4 â bulk update/delete by filter** (PR #538, 14/14): `PATCH`/`DELETE
  /rows/<t>?where=â¦` acts on every matching row â `{updated/deleted:N}`. Rails: a
  filter is REQUIRED, capped 1000/call, scoped to own rows (feed/user), trash-aware.
- **Batch 5 â password strength + block/ban** (PR #539, 15/15): signup rejects weak
  passwords; `_users.blocked` (owner endpoint `/api/site/backend/member` block|unblock|
  make_admin|remove_admin) â blocked can't log in + kicked on next `/auth/me`.
- **Batch 6 â in-app notifications** (PR #540, 14/14): per-app `_notifications`, member
  reads own inbox `GET â¦/notifications[?unread=1]` + `/read` + `/read-all`; created
...
- **Batch 7 â invite-only signup** (PR #541, 13/13): owner endpoint
  `/api/site/backend/invites` enable|disable|create(count,uses)|list|revoke; signup
  requires a valid `invite` code (atomic redeem) except the bootstrap admin â
  403 `{need:'invite'}`.
- **Batch 8 â cursor pagination** (PR #542, 7/7): keyset `?after=<lastRowId>` / `?before=`
  for stable "load more" on big lists (alongside limit/offset).
- **Batch 9 â JSON/array/object columns** (PR #543, 14/14): declare `type:"json"`
  (aliases `array`/`object`) â the platform JSON-stringifies on write & re-parses on
  ...
  the inner rows).
- **Batch 10 â faceted filters + counts** (PR #544, 9/9): `GET â¦/rows/<t>/facets?fields=
  color,size` â `{facets:{color:[{value,count}],â¦}}` (top 100/col by count) for filter
  sidebars ("Red (12)Â·Blue (4)"). Respects the SAME where/q/tag/trash + read visibility,
  so counts drill down as filters tick. Pure read, no write-path risk.
- **Batch 11 â schema-evolution column backfill** (PR #545, 8/8): fixes the footgun below.
  `applySiteSchema` now ALTER-adds `owner_id`/`deleted_at` (idempotent) after the
  `CREATE IF NOT EXISTS`, so revising a table's access (displayâfeed/user) or turning on
  `trash` backfills the required columns instead of silently breaking scoped writes.
- **Batch 12 â auto-slugs / pretty URLs** (PR #546 + fix #547, 8/8): table-level
  `"slug":"title"` â the platform fills a UNIQUE url-safe `slug` column on insert
  ("My First Post!"â`my-first-post`, `-2/-3` on collision, batch-deduped, diacritics
  folded, emptyâ`item`). Wired into ALL insert paths (single/batch/upsert); edits never
  re-slug; apps route via `?where=slug:eq:`. **Fix #547:** `normalizeSchema` was
  stripping the new `slug` table key (it only forwarded unique/oncePerUser/trash) â the
  column was created but never populated; caught by the live test, now threaded through.
- **Batch 13 â custom roles / RBAC** (PR #548, 10/10): an `admin`-access table can declare
  ...
  Note: the FIRST signup on any built app auto-becomes `admin` (bootstrap) â expected.
- **Batch 14 â per-row sharing / ACL** (PR #549, N/N): on a `user`-access table the row
  OWNER shares one record with specific members â `POST â¦/rows/<t>/<id>/share
  {user:email|id, perm:'view'|'edit'}`, `DELETE â¦/share/<memberId>`, `GET â¦/share`.
  ...
  (no leak) â sharing is purely additive.

- **Batch 15 â optimistic concurrency / no lost updates** (PR #550): table `"version":true`
  â a `_version` col that auto-bumps on each single-row PATCH and is returned on reads
  (`row._version`). Client sends `?ifVersion=N` (or `If-Match`) â stale write refused with
...
`normalizeSchema`/`coerceTable` (line ~2712) AND in the persisted `norm.push({â¦})`
...
`def` is loaded from `_meta` and won't carry the flag unless `norm` includes it) â
column-level keys survive via `coerceCol`, table-level ones don't.

## 2026-07-21 â CONTINUING THE ROADMAP (session picked up "keep going until 93")

Owner: finish the backend roadmap (~31 â 93). Original 100-item artifact was never
...
(near-zero builder-prompt cost) first, then client-facing, then ð key-gated (Stripe/
...
**NEW: offline backend harness** (`scratchpad/bh/` this session â consider committing to
...
fetch mock for the `site_backends` slugâuuid ledger and owner auth. So `ensure`â`schema`
âsignupâauth'd insertâlistâpatch all run the actual route code end-to-end offline. This
is the quality gate now; the owner still does the real live pass before merge.

## 2026-07-21 â Batch 16: edit-tracking + read-shaping (offline 17/17 + reg 5/5) â built, PR

Three additive, mostly-automatic primitives (PR on `claude/chat-session-xy6jwe`):
- **`"timestamps":true`** (table flag; aliases updatedAt/updated_at/timestamp) â auto
  `updated_at` column, set on insert (= created) via CREATE default, **bumped to now on
  EVERY edit** â single PATCH (via `applyVersionedUpdate`, so it also covers the
  ...
  first edit â fresh tables are perfect).
- **`?fields=a,b`** â sparse field selection on any list/get: returns only those columns
...
- **`?count=<child>`** â attaches `row._counts.<child>` = how many children reference each
...
## 2026-07-21 â Batch 17: write integrity â immutable fields + computed defaults (offline 13/13) â built, PR

Two cohesive write-integrity primitives (same PR/branch):
- **`"immutable":true`** (column flag; aliases readonly/readOnly/writeOnce) â write-once: a
  field is settable on INSERT but any later edit that includes it â 400 "can't be changed".
  ...
  â `rules[col].immutable`. Use for an email, a created price, an order number.
- **Computed default tokens** â a column `"default"` of `"@now"`/`"now"`/`"timestamp"` â
  `datetime('now')`, `"@today"`/`"today"` â `date('now')`, `"@uuid"`/`"uuid"` â a random
  uuid-shaped id. Emitted as a SQL DEFAULT *expression* in CREATE TABLE, so the DB fills it
  when the writer omits the field â NO insert-path plumbing. A provided value still wins;
...
## 2026-07-21 â Batch 18: NEW LAYER Follows (member social graph) (offline 12/12) â built, PR
...
- `POST /api/db/<slug>/follow/<userId>` (auth) â toggles, `{following, followers}`
  (`{on:true|false}` forces). Can't follow yourself. Idempotent (PK).
- `GET â¦/follow/<userId>` â `{followers, following, mine}` (Bearer â `mine`).
- `GET â¦/follow/<userId>/followers` and `/following` â arrays of public profiles
...
## 2026-07-21 â Batch 19: query power II â `between` + `sort=random` (offline 7/7) â built, PR

Both live inside the shared `buildD1Filter`/`buildD1List`, so EVERY read path (list/stats/
facets/bulk, all access modes) gets them for free â one-place, low-risk changes:
- **`where=<col>:between:lo:hi`** (also `lo,hi`) â inclusive range `col >= lo AND col <= hi`
...
- **`sort=random`** (aliases `rand`/`shuffle`) â `ORDER BY RANDOM()`, for discover/featured/
...
## 2026-07-21 â Batch 20: NEW LAYER Ordered lists / manual positions (offline 11/11) â built, PR

Drag-to-reorder (todos, kanban, playlists, galleries):
- Table flag **`"ordered":true`** (aliases position/sortable/reorderable) â adds a `position`
  REAL column, auto-assigned to the END of its scope on insert by an **AFTER INSERT trigger**
  (`trg_<t>_pos`, per-owner on user/feed via `owner_id IS NEW.owner_id`, global otherwise) â
  zero insert-path plumbing. Existing rows backfilled `position=id`.
- **`POST â¦/rows/<t>/<id>/move`** with `{after:<id>}` / `{before:<id>}` (midpoint between the
  neighbour and the next row on that side â REAL, so no renumber) or `{to:<n>}` (absolute).
...
- Apps read with `?sort=position`. **Bonus fix:** auto-managed columns (`position`,
  `updated_at`, `_version`) weren't in the sortable/filterable set â added a `listExtras`
...
## 2026-07-21 â Batch 21: NEW LAYER Expiring rows / TTL (offline 8/8 + trash reg 4/4) â built, PR

Flash sales, 24h stories, one-time invites, temp share links:
- Table flag **`"expires":true`** (aliases ttl/expiry/expiring) â adds `expires_at` (app-set,
...
- **Mechanism refactor:** folded expiry into the trash-visibility path â `trashClause` +
  new param-free `expiresClause` compose into `visClause`; `withTrash` â **`withVisible`**
...
## 2026-07-21 â Batch 22: pinned/featured rows + per-table defaultSort (offline 8/8) â built, PR

Both are list-ordering defaults, localized to `buildD1List` (via a new `opts` param):
- **`"pinnable":true`** â adds a `pinned` INTEGER (app-set, added to the writable/queryable/
...
- **`"defaultSort":"-created_at"`** (sanitized string, same prefix convention as `?sort`) â
  used when the request sends no `sort`; per-query `?sort` still overrides. Threaded through
  coerceTable/norm.
This is the first batch of the post-merge run (16â21 merged to main via #551). Opens a new PR.
Roadmap tally: ~40/93.

## 2026-07-21 â Batch 23: NEW LAYER App settings / config KV (offline 15/15) â built, PR #552

Every app needs a settings/feature-flag store (theme, hero text, toggles). Self-contained
like Counters â new `_config (k,v,updated_at)` table per site D1:
- `GET /api/db/<slug>/config` â `{config:{k:value}}` Â· `GET â¦/config/<key>` â `{value}`
  (PUBLIC â the app renders from it).
- `POST â¦/config/<key> {value}` / `DELETE â¦/config/<key>` â **admin site-user only** (role
...
## 2026-07-21 â Batch 24: NEW LAYER Bookmarks / saves (offline 13/13) â built

Mirrors Follows/Reactions but PRIVATE to each member. New `_bookmarks (user_id, target)`
PK-deduped table:
- `POST /api/db/<slug>/save/<table>:<id>` (auth) â toggle `{saved, count}`; `{on}` forces.
- `GET â¦/save/<target>` â `{count, mine}` (Bearer â mine).
- `GET â¦/saves[?table=<t>&limit=N]` (auth) â the caller's saved items `[{target, table, id,
  created_at}]` newest-first; the app then loads rows via `?where=id:in:â¦`.
Helpers ensureBookmarks/toggleBookmark/bookmarkState/bookmarkList. One BACKEND_RULES line.
Roadmap tally: ~43/93.

## 2026-07-21 â Batch 25: Scheduled publish / drafts (publish_at) (offline 7/7) ✅ built

Inverse of TTL, reuses the visibility mechanism: `"publishable":true` (aliases scheduled/
publish_at) → adds `publish_at` (app-set, in the writable/sortable allow list). Reads HIDE rows
whose publish_at is in the FUTURE; NULL/past = live. `?scheduled=1` = drafts view,
`?withScheduled=1` = all. `publishClause` folds into `visClause` alongside trash + expiry.
Roadmap tally: ~44/93.

## 2026-07-21 â NEW LAYER: Uniqueness constraints (race-free) â live
...
- Table-level `"oncePerUser":["product_id"]` (on `user`/`feed`) â UNIQUE over
...
- Real UNIQUE INDEX in the site D1 â race-free. A violating write returns a clean
...
Shipped PR #530, deployed. **Verified live 13/13** â oncePerUser is genuinely
...
## 2026-07-21 â NEW LAYER: Reactions (one-per-user likes/upvotes/RSVPs) â live
...
- `POST /api/db/<slug>/react/<target> {type}` (auth) â TOGGLES the caller's reaction
  (default type `like`), returns `{reacted, count}`; `{on:true|false}` forces a
  direction (idempotent). `GET â¦/react/<target>` â `{counts:{type:n}, mine:[types]}`
...
- **`?reactions=1`** on any list read inlines `row.reactions = {counts, mine}` for
  target `<table>:<id>` (batched, caller-aware) â a feed renders like/upvote buttons
...
- CONVENTION: react to `<tableName>:${id}` (e.g. `posts:${id}`) so manual reacts and
  the `?reactions=1` shortcut agree â BACKEND_RULES now states this explicitly (a
  singular-vs-plural mismatch would split the count).

Shipped PR #528, deployed. **Verified live 14/14** â toggle on/off, auth guard, cross-
...
## 2026-07-21 â NEW LAYER: Profiles (public member identity) â live
...
- `GET /api/db/<slug>/profile/<id>` â PUBLIC, safe columns only (never email/hash).
- `GET`/`PATCH /api/db/<slug>/profile/me` â read/edit own (auth); avatar must be a
  URL, name non-empty, bio bounded. `GET â¦/profiles?ids=1,2,3` â batch.
- **`?authors=1`** on any list read inlines `row.author = {id,display_name,avatar_url}`
  by `owner_id` (batched, no N+1) â feeds show the poster with NO author_name column.
- Owner member read now shows `display_name`; BACKEND_RULES teaches profiles+`?authors`.

Shipped PR #526, deployed. **Verified live 17/17** â public read (no email/hash leak),
...
## 2026-07-21 â NEW LAYER: Counters (atomic likes/views/reactions/polls) â live

Built apps had no race-free way to tally likes, view counts, reactions, or poll
options â a read-modify-write on a row column races under load, and a full row per
...
- `POST /api/db/<slug>/count/<name>[?by=N]` â atomic increment (default +1, floored
  at 0; negative to undo a like), returns the new `{value}`.
- `GET /api/db/<slug>/count/<name>` â `{value}`; `GET â¦/count` â `{counters:{â¦}}`.
- PUBLIC + atomic (anonymous likes/views, no login, no races), rate-limited
  (300/min read Â· 120/min write). Stored in the site's own D1 `_counters` (dropped
  with the DB â no orphans). Surfaced to the owner in `/api/site/backend/analytics`
...
Shipped PR #524 (+#525 trailing-slash tolerance), deployed. **Verified live 12/12** â
increment/by/floor, read single+all+unknown, owner analytics surfacing, method guard,
and the property that matters: **25 concurrent increments â exactly 25 (race-free)**.
All test backends provisioned bare (ensure+schema, $0) and deleted after.

## 2026-07-21 â End-to-end platform test + a real bug caught (backend wiring) â ï¸ââ

**Drove a real Haiku-built app ("Townsquare" community board, $0 fal, photo-free)
end-to-end as a real user** â signup/login, feed posts, comments (relations),
search, the AI suggest-title function, analytics â then checked every owner panel.

**Platform backend: 22/22 checks PASSED.** Auth (+ first-userâadmin role), feed
writes (auth-gated, anon rejected), relations (`?children=comments` nested 3,
`?expand=post_id` inlined the parent), search (`?q=coffee`â1; multi-term AND),
**AI-as-a-primitive** (suggest-title â owner charged EXACTLY 1 credit, zero provider
...
perfect UI but showed "Could not load posts" â because it wired EVERY backend call
**without the site slug**. The rule mandates `const API = '/api/db/' +
location.pathname.split('/')[2]`; Haiku ignored it, wrote its own helper
`fetch(\`/api/db${'{'}t${'}'}\`)` and called `w("/rows/comments")` â `/api/db/rows/comments`
(slug-less â 404). The string `townsquare-e7c3eb` appeared **0 times** in the whole
bundle. Auth, posts, comments â all dead. **Why nothing caught it: the build +
...
sent NO model field â production builds were **already Sonnet-only**. The dead app
only happened because the *test* forced Haiku via the raw API. So live users were
almost certainly unaffected â but the Haiku code path existed and was reachable.

**FIX â shipped + DEPLOYED (PR #523 merged to main + live on isibi.ai 2026-07-21):**
1. **Haiku REMOVED from the builder** (owner's call 2026-07-21 â "just take haiku
   out"). `RB_MODEL` in `/api/site/react-build` is now hardcoded `claude-sonnet-5`;
   the `model:'haiku'` API path is gone. (Haiku is untouched everywhere else â it
   still writes the cheap director steps. This only removes it from whole-app builds.)
2. **Louder API-base rule** (`react-gen.mjs` BACKEND_RULES) â deriving the slug is now
   a hard, explicit directive ("NEVER hardcode `/api/db/auth`â¦ NEVER `/api/db${'{'}x${'}'}`â¦").
3. **Post-build wiring guard + repair pass** (`worker.js`, mirrors schema-repair) â a
...
same brief) on the live builder with no model field â came back `model: claude-sonnet-5`,
`backend:true`, `fixed:0`. Inspected the served bundle: **4/4 wiring checks pass** â
derives the slug via `split("/")[2]`, ZERO slug-less `/api/db/auth|rows` literals, ZERO
`/api/db${'{'}x${'}'}` templates. Drove it end-to-end: signup â post (auth-gated feed) â
public read â comment â `?children=comments` nested = all â. (Also re-confirmed the
...
transient "came out incomplete" (generic message, no provider leak â good); a retry
...
reset. (This note is a follow-up commit after the #523 squash-merge â not new code.)

---

## 2026-07-20 â Phase 4a: React builder STREAMS (live code view) + auto-fix loop â

`/api/site/react-build` is now an NDJSON stream (like the classic builder):
- `{ev:"code"}` â the source as Sonnet writes it (the "watch the code" live view)
- `{ev:"phase"}` â generating â images â compiling â (fixing) â publishing
- `{ev:"done"|"error"}` â URL / slug / cost / balance / brand / fixed-count
...
Vite compile error the exact error + current files go back to Sonnet (â¤2 tries),
...
â¦ untouched): fired a plant-shop + a lemonade brief, watched phases + code stream
live, terminal `{done}` â "Little Squeeze", compiled React, 3 files, buildMs 8436,
...
success bar). The UI is a design preview â NOT yet wired into the chatbox.

**Deploy gotcha logged:** the first Phase-4a deploy (873b79b) FAILED on a transient
Cloudflare API **522** during `wrangler deploy` (nothing to do with the code â
bundling + secrets succeeded). Re-deployed via a no-op change; also renamed
`instance_type` `standard` â `standard-1` (same 4 GiB tier) to clear a wrangler
deprecation warning. **Lesson: always confirm the deploy run's conclusion=success
(GH Actions) before testing â a green *merge* is not a green *deploy*.**

**Still open in Phase 4 â the CUTOVER (big, needs owner's steer):** wiring the React
...
## 2026-07-20 â â FIRST LIVE REACT BUILD SUCCEEDED (owner reloaded Anthropic credits)

Once the Anthropic API balance was topped up, the React pipeline ran clean
end-to-end on the first try:

- Prompt: one line ("small-batch coffee roaster called Ember & Oakâ¦").
- Result: `ok:true`, live at **`/s/a-clean-landing-page-for-a-s-7a2c10/`** â a real
...
- The model self-branded it "Ember & Oak â Small-Batch Coffee Roasters" (title +
...
- Rendered headless with ZERO page errors. (Screenshotting note for future
  sessions: the headless browser can't reach isibi.ai through the agent proxy â
...
that's borderline against the 180s Anthropic AbortSignal and any client timeout â
...
async (kick off â poll), which also feeds the live code-streaming view the owner
wants.

---

## 2026-07-20 â Phase 4c: FULL CUTOVER â the chatbox builder now IS the React engine â

Owner's call: "full cutover now" + "collapsed rows" + "not the open-site card". Done.

**What changed in the chatbox (`chat.js`/`styles.css`):**
- New project OR any React site â the streaming React engine (`/api/site/react-build`
  Â· `/api/site/react-revise`). Legacy static sites keep the old `/api/site` path.
- **Live view IS the chat** â Claude-Code-style collapsible step rows (`.st-step`):
  during a build "Writing the code â¾" streams the source live; when done the rows
  collapse to â summaries stored ON the message (Wrote the code / Generated images
  â opens thumbnails / Compiled / Published), each opened by the â¾. Rows default
  COLLAPSED. NO "Open site" result card (owner cut it â redundant w/ the preview +
  the top-bar "Live â"). Output text restyled to match.
- Stage: compile placeholder during a first build, then the compiled site from
  `/s/<slug>/` (cache-busted per revise). `Publish`â`Live â`; Code tab + Download-
...
**Verified live** (throwaway, real â¦ untouched, deleted after): built "El Fuego"
(5 images, published), then **revised by chat** ("change the hero headline to Tacos
That Bite Back") â same URL, rebuilt in 14s, headline changed, rest preserved.
Screenshotted for the owner.

**Bug caught + fixed in-flight:** revise first threw a generic error â `react-revise`
...
unimported reference â it becomes a global lookup). Added the import (PR #445).
...
**Deferred (React sites, follow-ups):** the MoreâCloud panels (Analytics/Inbox/
...
â revisit per-panel when the owner wants them on React. R2 has a few orphaned test
sites (little-squeeze/ember/el-fuego) from throwaways â harmless, unlinked.

---

## 2026-07-20 â Per-site backends: the two-layer decision + Phase A (D1 plumbing)

Owner's direction for giving built sites their own backend (like Lovable):
- **Layer 1 = isibi's OWN login** (people who log in to build sites) â **stays on
  Supabase, no Clerk.** Untouched. (Clerk was considered + priced â good for L1
...
- **Layer 2 = each BUILT site's own backend** (its visitors' logins + its data) â
...
have â 25B row reads / 50M writes / 5GB storage per MONTH included, then pennies.
...
Cloudflare account + one Worker + many databases** â NOT a separate CF project per
...
**Phase A shipped (plumbing):** `site_backends` table (slugâD1 uuid, RLS own-read,
...
`CF_D1_API_TOKEN` (owner created a Custom token, AccountÂ·D1Â·Edit, 2026-07-20).

**Build order (each testable):** A plumbing â Â· B schema (AI declares tables) Â·
C built-site auth (signup/login/me, users in the site's own DB) Â· D data read/write
(access-controlled) Â· E builder UI (Data/Users panel). All D1, $0 until scale.

---

## 2026-07-20 â Per-site backend Phases B + C (schema + built-site auth) â

**Phase B â schema:** `applySiteSchema(uuid, spec)` turns an AI-declared JSON table
spec into safe DDL (strict identifier validation + double-quoting â SQL-injection
...
**Phase C â built-site visitor auth:** public, slug-scoped endpoints
`/api/db/<slug>/auth/{signup,login,me}`. Each site's visitors' accounts live in that
site's own D1 `_users`; passwords hashed **WebCrypto PBKDF2 (100k iters â the Workers
...
the site's own DB (`_meta.auth_secret`) â a token for site A is invalid on site B (no
platform-wide auth secret). Brute-force lockout (8 fails â 15 min), generic login
...
were already taken by the OLD static-site member auth â don't collide).
Verified live: signupâmeâlogin all work; wrong-pw 401, dup 409, **cross-site token
rejected 401**.

**Still ahead (per-site backend):** D â data read/write API (access-controlled:
collect / display / per-user buckets) Â· E â builder Data/Users panel Â· plus the
...
applySiteSchema). Until that wiring lands, AâC are server capabilities proven by
endpoint tests, NOT yet triggered by a normal user build. **Orphan test D1 DBs still
accumulate in the CF dashboard (no delete path yet â that's part of D/E + account
deletion).**

---

## 2026-07-20 â Per-site backend Phase D + WIRING: LIVE end-to-end (Lovable-style) â

**Phase D â data API:** schema now records each table's access mode + column
...
BACKEND protocol â the model emits `isibi.schema.json` (tables + access) and wires
forms to `/api/db/<slug>/â¦` (slug from `location.pathname.split('/')[2]`) ONLY when
the site needs data/auth. react-build/revise `parseSchemaSpec` (never shipped) â
...
REACT_RULES in react-gen.mjs â const TDZ, not hoisted.)

**LIVE END-TO-END PROOF:** built "FocusFlow" from one sentence ("a waitlist page
where emails are actually saved") â model declared `waitlist_signups` (collect),
backend auto-provisioned, a visitor POSTed an email (public) â saved, owner read it
back. Full Lovable-style "describe an app with data â it works" is LIVE. Screenshot
sent.

**So the per-site backend (AâD + wiring) is COMPLETE and live.** Remaining polish:
Phase E â a builder **Data/Users panel** (front-end) so the owner sees rows in-app
...
## 2026-07-20 â Per-site backend Phase E: builder Data/Users panel â (feature COMPLETE)
...
**â The per-site backend is now feature-complete: A (provision) Â· B (schema) Â· C
(built-site auth) Â· D (data API) Â· E (Data panel) + generation wiring, all live.**
A user builds an app that needs data â its own DB + tables + login + working forms,
and the owner views the data in the builder. Lovable-style, done.

**Only real remaining item:** a **D1 delete path** â deleting a site or an account
...
## 2026-07-20 â Full site delete (the Ã on a Your-Sites card now wipes everything) â

Owner: "if user deletes it from there, that's it, it's deleted." Done. The card Ã
used to only drop the site from localStorage â the live page + D1 db lingered.
Now: `POST /api/site/backend/delete` (auth, owner-scoped via the React source uid
and/or the `site_backends` row) deletes the **D1 database** (`cfD1Delete` â CF DELETE
API) + its mapping row, and the R2 **published dist + uploads + generated source**.
Frontend card Ã confirms (permanent) then calls it before removing locally; drafts
with no slug still delete locally only. Verified live: owner delete â 404 "no
...
- Nit: the non-owner block returns 401 ("sign in required") instead of a cleaner
  403 â security is correct, message is off. Cosmetic.
- **Still pending:** ACCOUNT deletion (`delete_account` + the client wipe) doesn't
  ...
  iterate `sitesCache` slugs â call `/api/site/backend/delete` for each first.

---

## 2026-07-20 â Test pass on the builder + per-site backend (found + fixed 1 real bug)
...
- **Login app (user-tier), "DayList" to-do:** PASS. Model declared a `user` table
  (`tasks`: title/done), wired signup/login/rows + Bearer token; 2 visitors â
  each sees ONLY their own tasks, logged-out â 401, re-login works, data saves +
  reads. App renders as a real login-gated landing page.
- **Plain portfolio ("Mira Voss"), no data:** PASS â correctly `backend=False`,
...
- **Revise-adds-backend (add a contact form to the portfolio):** found a **REAL
  BUG** â the model declared the schema in an alternate shape
  ...
  fields (object or array), bare-string cols, and missing accessâ'collect' before
  applying.** Re-verified: alternate shape now creates the table + the form
  saves + owner reads the message.
- **Full site delete on real sites:** PASS â both test sites deleted via the
  endpoint â backend 404 + live page 404 (R2 wiped).
...
## 2026-07-20 â Test round 2 (feed + display): 1 fixed, 1 known-limit noted

- **Community feed (Campfire, "post publicly, everyone reads"):** was **BROKEN** â
  ...
  **FIXED: added a 4th access mode `feed`** = public GET, logged-in POST/PATCH/
  DELETE scoped to the author (owner_id). Verified: 2 users post â public read
  sees BOTH; logged-out post â 401. Rules now steer the model to `feed` for
  boards/comments/reviews/public-guestbooks (vs collect which hides, display
  which blocks). The four modes: **collect Â· display Â· user Â· feed.**
- **Shop (Clayware, "products from a database"):** the model **hardcoded** the 4
  mugs (`backend=False`) instead of a `display` table â correct, because `display`
  ...
  catalogs. **RESOLVED 2026-07-20 â owner content editor SHIPPED:** the Data panel
  ...
  column) and per-row Edit/Ã controls. `_users`/`_meta` are blocked; columns are
  allow-listed from the schema (minus id/created_at/owner_id). "The AI builds the
  storefront; you stock the shelves." **So `display` is now first-class** â the
  model can use it for products/posts/menus and the owner fills the content in.
  Live-verified end-to-end: owner adds 2 menu rows â owner read sees them â PATCH
  edits price â PUBLIC data API (what a visitor sees) reflects it â DELETE removes
...
- **Cross-user isolation audit (2026-07-20) â data was safe, responses lied.**
  ...
  read/edit/delete Alice's rows â **no leak, no breach.** BUT a wrong-owner (or
...
- **Schema-merge bug (2026-07-20) â a revise could break existing tables.**
  ...
  it, stripping pre-existing tables from the data API's allow-list â their data
...
- **Boolean columns didn't round-trip (2026-07-20).** Posting `active: true` into
  ...
  treat a `false` row as true â silently broken todo/checkbox/published flags.
  **FIXED:** `d1Params` coerces every bound param (trueâ1, falseâ0, undefinedânull)
...
- **Also audited clean (no bugs), 2026-07-20:** cross-OWNER isolation (a different
  ...
  isolation (a login on site A can't touch site B â separate HMAC secret + slug
  ...
  valid-length wrong passwords count toward a lockout (fine â a <8 pw can't be right).

- **Built-app visitor password reset SHIPPED (2026-07-20, #476).** The per-site D1
  visitor auth had signup/login/me only â a forgotten password = locked out forever.
  ...
  so the generated app never needs its own reset route â the model just wires a
  "Forgot password?" request form (BACKEND_RULES updated). Email via `sendPlatformEmail`
  â Go Farther. Tested 11/11 (page renders, reset-request always-ok for existing/
  unknown/honeypot/bad-email, reset rejects garbage + wrong-purpose tokens, orig pw
  still works). Happy path (real emailed tokenâchange) unverifiable without the key +
  a mailbox, and the token secret is correctly unforgeable.
  **â ï¸ SETUP NEEDED for email to actually send:** add a GitHub Actions secret
  `GO_FARTHER_API_KEY` (the same gf_live_â¦ key the Supabase send-email fn uses), then
  ...
  found") â always create the secret first, or the deploy is dead.
- **BACKLOG (owner's call 2026-07-20):** cap built-site DB creation via CREDITS
  (charge per provision) rather than a hard per-user limit â "set that up later."
  No cap today; acceptable while the user base is small.
- **Robustness/fuzz pass (2026-07-20) â all clean (30/30), no bugs.** Threw malformed
  schemas (tables as string/number/null, columns as string, access as number, garbage
  types, nested junk, whole-body null/string) â all handled, never a raw 500
  (normalizeSchema coerces or 400s). ensure with empty/missing/huge/symbol slugs â
  sanitized or 400. Non-JSON bodies â 400. Data API: PUTâ403, non-JSONâ400, unknown
  slugâ404. `/s/<slug>/` serving: crafted traversal paths (`../../etc/passwd`,
  `..%2f`, `....//`, 300-char) ALL 404 â safe by construction (R2 object keys are
...
- **FULL-STACK security sweep (2026-07-20) â ALL AIRTIGHT, zero findings.** Went
  beyond the D1 backend to the whole app, testing as real authed users (anon key +
  JWT) against Supabase + the Worker:
  Â· **Money RPCs** â a normal user CANNOT call `add_credits`/`set_plan` (404 PGRST202,
    ...
    403 (42501 â authenticated role has NO write grant on money tables; only the
    SECURITY DEFINER RPCs touch them, minting is service_role-only).
  Â· **RLS** â user A reads 0 rows of B's credits/purchases/user_plan/chats/
    user_memory/user_assets/usage_log/gen_charges; can't write into B's data.
  Â· **SSRF** (`/api/import/fetch`) â every internal target blocked (127/10/172.16-31/
  ...
  Â· **Storage** (`media` bucket) â INSERT/DELETE/SELECT policies are path-scoped to
    `media/<auth.uid()>/`; A uploading into B's folder = 403 RLS "new row violates
    row-level security policy"; own folder works.
  Â· **Stripe webhook** â unsigned AND forged-signature events both rejected 400 "bad
    signature"; no credits minted.
  Â· **Generation gates** (`/api/{image,video,audio}`) â unauthed 401; a non-allowlisted
    model 400 "unknown model" BEFORE any fal call/charge; error bodies never say "fal".
  All test users/data deleted.

- **PLATFORM STACK â building out the layers every app needs (2026-07-20).** Model:
  ...
  Â· **QUALITY PASS (owner 2026-07-21, "do that better") â 3 things, not more breadth:**
    ...
    tokens) with ZERO capability lost â kept every endpoint/exact syntax + the critical
    ...
    **Cascade delete (#516/#517) â VERIFIED live 11/11:** deleting a row (data API feed/
    user/admin + owner editor) now also deletes its direct children (tables whose `ref`
    points at it) so no orphans; ownership checked FIRST (gone/not-yours â 404, no phantom
    ...
    are NOT available for dynamic per-site DBs â cascade runs SEQUENTIAL children-first
    ...
    UI (#519):** wired 3 new Moreâ¸Cloud cards + on-brand glass modals â **Insights**
    ...
    shown + approved. Endpoints were already live/verified â this is the missing UI.
    **Export/Import UI (#521):** added under snapshot/restore in the Backups panel â
    pick a table â Export CSV/JSON (authed blob download) or Import CSV (file â rows).
    So EVERY backend data endpoint now has an owner control. Only non-UI'd surface left
    is the AI endpoint (no owner need â it's an in-app feature). **QUALITY PASS COMPLETE.**
  Â· **AI-as-a-primitive (#508) â built apps call an LLM, no key, VERIFIED live 15/15.**
    A built app adds real AI (chatbot, summarize, suggest-a-reply, categorize) with
    `POST /api/db/<slug>/ai {prompt,system?}` â `{ok,text}`, or an `{do:"ai"}` function
    step â `{{steps.r.text}}`. Platform runs Claude Haiku server-side; each call is
    ...
    sees provider internals). **Proof:** 9/9 free (empty prompt â no charge + friendly
    ...
    call, 20â19â18). Owner approved the ~$0.006 test spend. Builder rules document both
    forms. **This is the AI-native differentiator â apps ship AI features out of the box.**
  Â· **Visitor analytics (#512, accuracy fix #513) â page views + events, VERIFIED live
    ...
    analytics?slug=&days=` â `{total, byEvent, byPath, byDay, rows}`. Stored as tiny
    (day,event,path) counter upserts in the site's own D1 `_analytics` â never raw per-
    ...
    _metrics but that under-counted at low traffic (13/25 â events stranded in sub-
    threshold isolate buffers); fixed to write EACH event through immediately (per-event
    upsert via waitUntil, table ensured once/isolate) so view-counts are EXACT â re-test
    12/12 total=25, view=20/signup=4/purchase=1, /pricing=11, no-authâ401, XSS event/
    path sanitized. Rate-limited 300/min per visitor. Builder rules document /track.
  Â· **Event triggers (#511) â on-insert â function, VERIFIED live 6/6.** A function can
    declare `"on":{"insert":"<table>"}` (sibling of steps) to run AUTOMATICALLY after a
    row is inserted into that table (new order â email me Â· new signup â notify Slack Â·
    new comment â AI-screen), with the new row as `{{input.<field>}}`. Fires on single-
    ...
    /ai endpoint always worked) â now the step survives + runs metered to the owner.
    **Proof (zero AI/fal): 6/6** â an order insert fired the fn â wrote a `logs` row
    templated from `{{input}}`; a 2nd order â 2nd log; a bulk import of 3 orders fired
    ZERO triggers. Seeded the fn straight into site_functions (no build); cleaned up.
  Â· **Data import (#509) â CSV/rows â a table, VERIFIED live 14/14.** Owner endpoint
    `POST /api/site/backend/import {slug,table,csv|rows}` â the mirror of export. Parses
    a CSV string (RFC-4180-ish: quoted fields, embedded commas/newlines, `""` escapes)
    with case-insensitive headerâcolumn mapping, or a rows array; cap 2000; only declared
    columns written (unknown dropped, `_users`/`_meta` blocked, empty cell â NULL).
    **Proof (zero AI/fal): 14/14** â `"Gadget, deluxe"` + `""quotes""` parsed, Nameâname/
    Priceâprice mapped, `ignored` col dropped, empty priceâNULL, rows-array path, header-
    onlyâ400, _users refused, no-authâ401, and export round-trips it. Owner-facing.
  DB â Â· Auth+reset â Â· **Files â (#484)** Â· **Server functions + Secrets â (#486)
  â this also delivers Payments + Email for React apps** (via a checkout/email
  function step + a secret) Â· **Search/Query â (#490)** Â· **Stats/Aggregations â
  (#491)** Â· **Roles/permissions â (#492)** Â· **Email verification â (#493)** Â·
  **Realtime â (#494)** Â· **Relations/expand â (#495)** Â· **Reverse relations â
  (#496)** Â· **Upsert â (#497)** Â· **Batch insert â (#498)** Â· **Data export â
  (#499)** Â· **Rate limiting â (#500)** Â· **Validation â (#501)** Â· **Multi-word search
  â (#502)** Â· **Observability â (#503)** Â· **Backup/restore â (#504)**. Custom domains
  LAST (owner's call 2026-07-21 â "all of them, leave custom domains for last"; it needs
  the owner to enable Cloudflare custom hostnames + SSL at the account level, so it's
  gated on infra, not code).
  Â· **Backup/restore (#504) â data DR for built apps, VERIFIED live 11/11.** Owner
    endpoints: `POST /api/site/backend/backup` snapshots ALL declared data tables into
    one JSON in R2 (`backups/<slug>/<ts>.json`); `GET â¦/backups` lists them; `POST â¦/
    ...
    AI/fal): 11/11** â backup counted 4 rows, listed, deleted all posts, restore brought
    3 posts + 1 comment back with **id 1 preserved** so the comment still expand-joins
    its post; no-authâ401, foreign-slug keyâ400. Cleaned up.
    **NOT done (honest):** *build* versioning/rollback (keep last N published builds for
    one-click rollback) â that's a publish-pipeline change, separate from this DATA
    backup/restore. NOW BUILT as #506 (below).
  Â· **Build rollback (#506) â revert a bad deploy, safety VERIFIED live 7/7.** Every
    React publish (build + revise) archives its whole dist as one JSON snapshot in R2
    (`builds/<slug>/<ts>.json`), keeps the newest 5. Owner endpoints: `GET â¦/builds`
    lists snapshots newest-first; `POST â¦/rollback {key}` rewrites that snapshot's dist
    to `sites/<slug>/` â the live site reverts in one call. Owner-scoped via the
    ...
    (edit-after-rollback = roll forward â documented caveat). **Free safety proof: 7/7**
    (builds/rollback on a non-owned or absent site â 401 before any R2 read; foreign-
    slug key â 400; missing slug/key â 400; no-auth â 401). The FULL archiveârollback
    cycle needs one real published build to seed a snapshot â owner chose NOT to spend
  ...
  Â· **GAP PASS COMPLETE (2026-07-21).** After the stack-category review, closed every
    backend gap except DNS (owner's skip): rate-limiting #500, validation #501, search
    #502, observability #503, backup/restore #504, build-rollback #506 â all verified
    live (rollback: safety only, per owner's no-spend call). Remaining knowns: custom
    domains (infra), email key (owner sets GO_FARTHER_API_KEY â verify/reset emails go
    live), true FTS5 / Durable-Object realtime / Analytics-Engine metrics (all "scale
    upgrades", current versions work). **The backend stack is ~complete â flagged to
  ...
  Â· **Observability (#503) â per-app request/error counts, VERIFIED live 9/10.** Every
    ...
    as daily `{reqs,errs}` totals â durable usage WITHOUT a write-per-request. Owner
    endpoint `GET /api/site/backend/metrics?slug=&days=` â the daily series + totals
    ...
    threshold buffer are missed until it flushes). **Proof (zero AI/fal): 9/10** â
    ...
    grew with more traffic, no-authâ401, and the public API can't reach `_metrics`
    (404). For exact counts we'd need Analytics Engine / a durable counter (scale
    upgrade). Cleaned up.
  Â· **Search upgrade (#502) â multi-word + relevance ranking, VERIFIED live 10/10.** `q=`
    was a literal substring match; now it splits into words and requires EVERY word to
    appear in SOME declared column (AND of terms, OR across columns) â `q=miami beach`
    matches rows with both words in any field/order. With `q` and no explicit sort,
    results rank by relevance (how many termÃcolumn pairs hit), best first. True FTS5
    (stemming/prefix) stays the scale upgrade (like Durable Objects for realtime).
    **Proof (zero AI/fal): 10/10** â "miami beach"âonly the both-words row, "beach
    austin"â0, reversed "loft miami" still matched, a city-only "miami" match ranked
    LAST, `sort=title` overrode ranking, and it combined with `where`+`total`. Cleaned.
  Â· **Validation (#501) â server-side write validation, VERIFIED live 14/14.** A schema
    ...
    fal): 14/14** â missing/empty required â 400, bad email/number/url â 400, over-`max`
    â 400, fully-valid + optional-omitted rows â 200, a batch with one bad email rejected
    WHOLE (nothing landed), 100k field capped. Cleaned up.
  Â· **GAP-CLOSING PASS (owner 2026-07-21, "build all of them, skip dns"):** after the
    stack-category review, closing the real gaps â rate limiting â (#500), then
    validation, full-text search, observability, backups/rollback. Email-key + custom
    domains stay owner-gated. Each free to verify.
  Â· **Rate limiting (#500) â built apps' API now throttled, VERIFIED live.** Per-site +
    per-visitor-IP burst limiter on `/api/db/*`: reads 300/min, writes 60/min, auth
    30/min, email-send (reset/verify request) 10/min (dropped neutrally â no reveal, no
    inbox flood). 429 + Retry-After over the limit. Fixed 60s window, in-isolate counter
    with stale-window eviction â ZERO new infra (a strict GLOBAL limit would need KV/
    Durable Objects; per-isolate still meaningfully caps abuse). Transparent to apps (no
    build-prompt change). **Proof (zero AI/fal):** auth capped at EXACTLY 30/min (30Ã401
    then 15Ã429), writes at EXACTLY 60/min (60Ã200 then 20Ã429), 20 quick reads all 200,
    throttle body friendly ("please wait a minute", no internals). (The lone red check
    in the harness was a TEST-REGEX false positive â its no-leak guard matched "fal"
    inside the JSON word "false"; not a real leak.) Cleaned up.
  Â· **Data export (#499) â owner downloads a built-app table, VERIFIED live 13/13.**
    ...
    escaping. **Owner-facing â ZERO builder-rules cost** (doesn't touch the build
    prompt; this is the "add capability without taxing every build" strategy from the
    bloat note). **Proof (zero AI/fal): 13/13** â CSV with header + escaped tricky value
    (comma/quote/newline â doubled quotes, wrapped), JSON array intact, `_users` export
    carried role/verified but NO pass_hash/pass_salt, no-auth â 401, unknown table â
    404. Cleaned up. **TODO (tiny, needs a screenshot):** add a â¤ Export button to the
    Data panel in the Cloud UI â endpoint's live, just no button yet.
  Â· **PROMPT-BLOAT NOTE (flagged to owner 2026-07-21):** every layer appends to
    BACKEND_RULES (fed to the AI on EVERY build), which is now long â costs input tokens
    ...
    (parked option).
  Â· **Batch insert (#498) â many rows in one call, VERIFIED live 12/12.** `POST {rows:
    [{â¦},{â¦}]}` (array under `rows`, â¤100) to `/rows/<table>` inserts them all in one
    statement (CSV import, seeding, bulk) â `{inserted:N}`. All write modes, same auth +
    owner stamping; union of declared cols across the batch (missing â NULL). **Proof
    (zero AI/fal): 12/12** â collect batch of 3, ragged rows (missing email â NULL), a
    `user` batch stamped owner_id per row with A/B isolation, boolean coerced, and a
    150-row payload capped to 100. Cleaned up.
  Â· **Upsert (#497) â create-or-update by a key, VERIFIED live 14/14.** `POST /rows/
    ...
    like/vote toggles, saved progress â any "one row per X." For user/feed tables the
    ...
    14/14** â a user's `settings?upsert=key` kept exactly one row per key and updated
  ...
  Â· **Relations (#495 forward + #496 reverse) â linked tables, joined in ONE call,
    ...
    (â¤4) attaches each row's PARENT under the fk name minus `_id` (post_id â `post`).
    REVERSE (#496): `?children=<child_table>` (â¤3) attaches each parent's CHILDREN as an
    array under the child table name (`posts?children=comments` â each post has a
    `comments` array). Both batched (one grouped query each, no N+1; children capped
    500 total / 50 per parent). FKs are metadata-only (D1 FKs stay off) â the platform
    ...
    leak. Forward proof 10/10 (commentsâposts correct, single-row expands, no-expand
  ...
  Â· **Realtime (#494) â live updates with NO new infra, VERIFIED live 13/13.** No
    ...
    the instant a new row lands â near-instant chat/feeds with ~10Ã fewer requests, no
    ...
    **Proof (zero AI/fal): 13/13** on feed+user+collect â since=0 tail, incremental
  ...
  Â· **Roles/permissions (#492) â VERIFIED live 17/17.** Built-site users now carry a
    ...
    only an admin site-user can create/edit/delete (a real in-app CMS â blog/catalog/
    events the owner manages from a logged-in admin screen, unlike `display` which has
    no in-app editor). Non-admin write â 403 'admins only'; anon write â 401; everyone
    ...
    **Proof (zero AI/fal):** two signups â admin + user; admin POST/PATCH/DELETE ok,
    user + anon blocked (403/401), edits/deletes stuck, public reads worked. Cleaned.
  Â· **Email verification (#493) â flag + endpoints live; happy-path flip pending the
    email key (same gate as reset).** On signup the platform auto-emails a signed 24h
    'confirm your email' link â the worker-hosted `/verify` page flips the user's
    `verified` 0â1 (idempotent, on-brand). Apps get a `verified` flag on the `user`
    object; they can gate features on it + offer a 'Resend' button â `POST /auth/
    ...
    API_KEY is set as a Worker secret** (still pending â SAME blocker as password
    ...
    verify-request returns neutral ok. The tokenâflip happy path goes live with the
    email key. Builder rules document the flag + resend.
  Â· **Why we hand-build these (owner asked 2026-07-20):** Supabase (which powers
    ISIBI's OWN backend) ships auth + a query API + storage pre-assembled â but a
    Supabase project is heavy + costs per-project, so it can't be "one backend per
    built app Ã thousands." The built sites run on Cloudflare (D1 + R2 + Worker),
    which is built for database-per-tenant (cheap, instant, ~unlimited) but gives
    RAW primitives â no batteries. So we build the convenience layer ONCE on CF and
  ...
  Â· **Stats/Aggregations (#491) â dashboards computed server-side, VERIFIED live.**
    `GET /api/db/<slug>/rows/<table>/stats` â `{count}` + optional `sum`/`avg`/`min`/
    `max=<col>` (declared numeric cols, repeatable) + optional `group=<col>` for a
    per-value breakdown (`{groups:[{value:'paid',count:3,sum:{amount:600}},â¦]}`,
    ...
    cards/charts instead of pulling rows and reducing in JS. **Proof (zero AI/fal â
    seeded an `orders` display table directly): 16/16** â count=6, sum=875, min50/
    max300, avgâ145.83; group=statusâpaid(3,600)/pending(2,200)/refunded(1,75) sorted
    desc; `where=paid`â(3,600) and `where=amount>=150`â(3,650); safety: a bogus agg
  ...
  Â· **Search/Query (#490) â server-side filter/sort/paginate on the data API,
    VERIFIED live.** Before this, a list read returned newest-N and the built app
    had to fetch everything and filter in the browser â fine for a demo, useless for
    ...
    table's declared columns), `sort=<col>&order=asc|desc`, `limit`(â¤200)+`offset`,
    ...
    value is parameterized (`buildD1List` helper) â no injection surface; `user`
    ...
    **Proof (zero AI/fal spend â provisioned a display table directly and hit the
    endpoint):** seeded 6 listings across 3 cities; 17/17 checks passed â
    `where=beds:gte:3`â3 rows, repeatable AND (`beds>=3 & price<=3M`)â2, `q=miami`â3,
    `sort=price asc`âascending, `limit=2&offset=2`âclean pagination with `total:6`,
    ...
    deleted (0 left).
  Â· **Functions + Secrets (#486) â the big one, VERIFIED live end-to-end.** The
    ...
    `site_functions` by slug (no published_sites coupling) â already React-ready.
    ...
    `isibi.functions.json`, `persistSiteFunctions` writes it â done event reports
    ...
    **Proof:** Haiku-built "Northwind contact â Slack via webhook" app declared a
    ...
    secret (listed by name only, value never returned) â fn ran {ok:true}. Then a
    deterministic echo fn PROVED server-side secret injection â a known secret value
    ...
    payments (Stripe checkout step), and email â keys stay server-side.
  Â· **Files (#484):** the upload primitive (endpoint + R2 + `/u/<slug>/<file>`
    ...
    FILES section to the React BACKEND_RULES (wire `<input type=file>` â upload â
...
- **HAIKU builds are viable (2026-07-20, #483 added `{model:'haiku'}` to react-build).**
  Built "Rosterly â team directory w/ login + photo upload" on Haiku 4.5: compiled
  **first try** (no fix loop, `fixed:0`), phases generatingâimagesâcompilingâ
  publishingâdatabase, wired BOTH the D1 backend AND the new /api/site/upload, and
  `backend:true`. **Cost 7 credits vs ~31 on Sonnet â ~Â¼ the price.** Full live
  chain worked: signup â upload a PNG (served back 200 image/png) â create a
  `team_members` (user-mode: name/role/photo) row with the photo URL â read back,
...
- **ð´ BIG ONE â real end-to-end build bug (2026-07-20, #481), found by actually
  ...
  `/rows/tasks`, BUT never emitted `isibi.schema.json` â `backend=false`, no D1
  provisioned â the shipped app's login AND tasks both 404 "this site has no backend
  ...
  RULES`) asks the model to emit JUST the schema â inferred from its own code â then
  ...
  **Re-verified live end-to-end:** rebuilt same prompt â `backend=true`, phase order
  now ends `â¦publishing â database`; on the LIVE site signup works, task create+read
  ...
  incomplete" (flaky/truncated gen â the "try again" message is correct; retry
  succeeded) â worth watching if it recurs often. (2) Auto-fix loop ran (fixingÃ1-2)
  ...
  gated on "no backend exists yet" â not in #481.

- **Cloud panel was reading the OLD static-site store for React sites (2026-07-20).**
  A React site's Moreâ¸Cloud cards (Members/Submissions/Database/Secrets/Functions/
  Files) called the legacy Supabase endpoints, which resolve via a `published_sites`
  row that React builds NEVER create â so every card silently returned `[]` (200,
  not an error) even when the site had real data in D1. **FIXED (#472, owner chose
  "rewire to D1"):** for React sites w/ a backend, MembersâD1 `_users`, Submissionsâ
  D1 `collect` tables, Databaseâopens the Data panel. Secrets/Functions/Files/Emails/
  Payments have no equivalent for a static React SPA (no server-exec surface) â shown
...
- **Schema dedupe bug (2026-07-20, #474).** A schema with DUPLICATE column names,
  ...
  two same-named columns â D1 error 7500 â backend provision 502'd. react-build
...
- **Deep security audit (2026-07-20) â all AIRTIGHT:** the public data API cannot
  reach the internal tables â `/api/db/<slug>/rows/_meta` (holds the auth signing
...
- **TWO OPEN DESIGN GAPS (flagged, not yet decided):** (1) **No per-user cap on
  `/api/site/backend/ensure`** â an authed user could loop it and provision unlimited
  D1 databases (account cap 50k) â resource-exhaustion vector. Needs a cap or rate
  limit (what number is the owner's call). (2) **D1 visitor auth has NO password
  reset** â `/api/db/<slug>/auth/` is signup/login/me only; a visitor who forgets
...
- Small, surgical changes â don't restyle or refactor beyond what's asked.
- One thing at a time. Owner prefers reviewing/fixing bugs one-by-one over big batches.
- Ship flow: change â commit â open PR â squash-merge to `main` (auto-deploys).
...
## Parked (owner said hold â do not build until re-opened)
...
- **Home preset system (2026-07-12):** the 8-preset lineup (Blitz Motion +
  ...
  Shelf Wars / Week One) is ON HOLD per the owner â "forget about them for
  ...
  (Seedance 2.0 Â· 12s Â· 9:16 Â· 1080p). Presets = director instruction
...
- **Voice/audio lane on the landing filmstrip (built + REMOVED twice, 2026-07-13):**
  ...
  on the "Made with isibi" strip. Round 1: compact (186Ã76) â owner "came out
  very ugly, delete that" â reverted. Round 2: re-added bigger (300Ã118),
  merged, then owner said "wrong chat, delete the voice thing" â reverted off
  `main` again. **Not a design rejection the 2nd time â it was merged to the
  wrong project/chat.** Fully off `main`. If it's genuinely wanted here later,
  confirm scope with the owner first; the bigger 300Ã118 version is what they'd
  approved visually.

- **Declined (2026-07-12):** Luma **Reframe** (video + Photon image outpaint-to-new-ratio,
  on fal) â offered, owner said no. Don't re-pitch unless they bring it up.
  Runway integration also discussed: not on fal (needs its own API pipeline) â neither
...
- **Studio DROPPED (owner: "drop the studio and the video editor thing", chose
  ...
  on-device trims). **KEPT `public/ffmpeg-edit.js`** â the QR burn depends on it
...
- **AI video editing wired into the Builder (done 2026-07-12).** The pattern:
  attach a Video clip to an editing model â the worker routes to that model's
  ...
  - **Gemini Omni Flash** â `google/gemini-omni-flash/edit` (prompt + video_url;
    conversational swap/relight/stabilize/bg). Regional note: fal blocks editing
    uploaded videos for EEA/Switzerland/UK users.
  - **Kling o3 Pro** â `.../o3/pro/video-to-video/edit` (prompt + video_url +
    optional style `image_urls` â¤4 + keep_audio; elements/shot_type not exposed).
  - **Veo 3.1** â `fal-ai/veo3.1/extend-video` (prompt + video_url; continue/lengthen).
  ...
  in VIDEO_USD) â PROVISIONAL, verify on the fal sweep. Not-in-roster editors
  still available if wanted: Happy Horse 1.0, Kling o1, VOID (object removal).

## 2026-07-20 â React builder: the "code -1 / code 400" wall = Anthropic account out of API credits (NOT a code bug)
...
> Plans & Billing to upgrade or purchase credits." (request_id req_011CdDâ¦)
...
max_tokens 16000, system+messages â all valid; 128K output cap, no beta header
...
â the React builder, the classic Sonnet builder (`/api/site`), the AI Orchestrator
(`/api/direct`), and the security scan. (Generation of images/video via fal is on a
separate provider/balance and is unaffected.)

**Owner action needed:** top up API credits at console.anthropic.com â Plans &
...
**Code cleanup shipped (PRs #436â#437):** the react-build gen-error path now logs
the real upstream reason server-side (`console.error`) and returns a generic
"builder is busy â try again" to the client (no provider/billing text leaked).

Test hygiene: throwaway account (166c40d9â¦, rb-â¦@example.com, 500 test credits via
SQL â never touched the real â¦ balance) fully deleted after the diagnosis.

---

## In progress â awaiting owner sign-off (NOT merged to main)

### Website builder â real EDGE FUNCTIONS (Path A, 2026-07-18)
- **Status:** â SHIPPED to main + deployed + live-tested 2026-07-18 (owner said
...
- **The decision (why Path A):** the owner wanted the model to build "edge
  functions" like Lovable (describe backend logic in chat â model builds it â
  appears in the Cloud panel â runs live). Walked the owner through the real
  ...
  Three paths â **A: declared function-specs on the shared backend we already
  own** (â$0, no per-site infra, nothing to sandbox); **B: a Supabase project
  ...
  Owner picked **A** ("do the shared") â and it's not throwaway: the spec layer
...
- **How it works:** the model emits a bounded **function SPEC** (a triggerâsteps
  recipe), never code. The generator declares one as
  `<script type="application/isibi-fn" data-name="X">{"steps":[â¦]}</script>` in
  ...
  **read** (a public collection), **save** (to a collection), **fetch** (an
  ...
  resolves ONLY inside a fetch request (server-side) â in respond/save it
  ...
  â¤8 steps, â¤2 fetch/run, 8s per network op, 32 KB response reads, plus a
...
- **Where:**
  ...
  - `public/chat.js` + `styles.css`: Cloud â **Edge functions** card is live;
  ...
  - Supabase: `site_functions` table (owner-scoped RLS, mirrors `site_secrets`).
    **Also patched `delete_account()`** to clear every `site_*` owner table
    (secrets/collections/functions/submissions/domains/visitor accounts) â those
    ...
    files are still NOT wiped on deletion â separate follow-up (R2 can't be
    reached from Postgres; needs a Worker/client purge).
- **Live test (2026-07-18, all pass, throwaway data cleaned up after):**
  respond+input templating â Â· saveâreadâcount (records landed, count flows) â Â·
  external fetch from the deployed Worker (GitHub zen, 200) â Â· **SSRF block** â
  a fetch at the cloud-metadata IP (169.254.169.254) returns status 0 / empty,
  safeFetch refused it â Â· unknown fn â 404 â Â· **secret injection** â a real
  vault secret decrypts on the Worker and lands in the outbound header the echo
  service reflects â Â· **secret isolation** â the same secret returned BLANK when
  a function tried to leak it via respond â Â· **encrypted at rest**
  (`leaks_plaintext:false`) â. Local: 16/16 logic tests + encrypt/decrypt
  round-trip â. (Test note: `site_collections.owner_id` has an FK to auth.users,
  so a save only works under a real owner â always true for real functions.)
- **Known v1.1 nits (not blocking):**
  1. Template paths don't span hyphens â `{{steps.h.body.headers.x-secret}}`
  ...
  3. `email` action deliberately not shipped â decide the abuse posture first.
...
### Website builder â DETECT & FIX errors (2026-07-18)
- **Status:** â shipped to main + deployed. The Lovable feature the owner
  singled out (image 9: "app detects errors, click fix, it fixes").
- **What:** the live preview now watches the built site for REAL runtime bugs â
  uncaught JS errors + unhandled promise rejections â via an error shim injected
  ...
  red "N issues detected Â· Fix with AI Â· Ã" chip at the bottom-left of the
...
- **Low false-positive by design:** generated sites load no external scripts
  ...
  undefined function on load â error caught through the sandboxed cross-origin
  iframe â badge appears "1 issue detected".
...
- **Blank-preview follow-up (2026-07-18, owner hit it live):** owner built a
  ...
  hid all content on load (scroll-reveal) then broke â black. Two fixes shipped:
  (1) **generator never-blank rule** in SITE_RULES â content MUST render with CSS
  alone, JS enhancement only, animations degrade to visible, try/catch around
  risky JS (fixes NEW builds). (2) **blank-detection in the preview shim** â after
...
### Website builder â function TRIGGERS: webhook + scheduled (2026-07-18)
- **Status:** â shipped to main + deployed + live-tested. Owner picked this from
  the "more technical, like edge functions" backlog.
- **What:** edge functions gained two triggers beyond the site's own JS calling
  `/api/site/fn`:
  1. **Webhook** â `POST https://isibi.ai/api/site/hook/<slug>/<name>`; the
     ...
     (`invokeSiteFunctionByName`). Same bounds + per-slug rate limit. The Cloud â
     Edge functions panel shows each function's copyable webhook URL.
  2. **Scheduled** â spec `"schedule":{"everyMinutes":N}` (clamped 5â¦43200). Runs
     on the EXISTING 2-min cron (`scheduled()` â `runScheduledSiteFunctions`),
...
- **Live test (throwaway data, cleaned up):** webhook â POSTed
  `{event:"payment.succeeded",data:{amount:4999}}` â function got it, nested
  `{{input.data.amount}}` resolved to 4999 â; unknown fn â 404 â. Scheduled â
  a 5-min function fired on the real cron (`cron_runs:1`, `last_run` stamped),
  did not double-fire â.
- **Next backlog (owner's picks):** FILE UPLOADS (visitor uploads â R2 â
  collection URL) is the next build. Queryable DB was DECLINED for now â the
...
### Website builder â FILE UPLOADS (2026-07-18)
- **Status:** â shipped to main + deployed + live-tested. Owner's 2nd pick.
- **What:** published sites can accept visitor uploads (listing photo, avatar,
  resume). `POST /api/site/upload` (public, fail-soft) takes a base64 data URL â
  mime-allowlisted to images (PNG/JPG/WebP/GIF) + PDF (NO svg/html, so no
  stored-XSS), â¤6 MB, per-slug count cap (300), only for a real site â stored in
  R2 under `uploads/<slug>/` â returns a permanent URL served from
  ...
  protocol (file input â FileReader data URL â POST â persist res.url).
...
- **Live test (throwaway site, DB row cleaned):** upload a 1Ã1 PNG â permanent
  URL â; served back = 70 bytes, `content-type: image/png`, nosniff, valid PNG
  â; HTML-masquerade data URL â 415 â; bogus slug (no site) â "not live yet" â.
  (One 70-byte test PNG remains in R2 â wrangler isn't authed in the session to
  delete it; immaterial.)
- **v1.1 gap â CLOSED 2026-07-18:** built FILE MANAGEMENT (Cloud â Files).
  ...
  `uploads/<slug>/` and delete one. `siteFiles()` grid modal â image thumbnails /
  PDF chip, size, delete. Live-tested end-to-end (upload â owner lists it â delete
  â gone â unauth = 401), throwaway user + the R2 object both cleaned up.

### Website builder â QUERYABLE DATABASE (2026-07-18)
- **Status:** â shipped to main + deployed + live-tested. (Earlier I'd said the
  model could do client-side filtering; owner asked to build the server side, so
  now both are possible â server query is better for real datasets + paging.)
- **What:** `GET /api/site/data` now filters/sorts/searches/paginates. Done
  ...
  collection (â¤500 storage cap) and queries in-memory â no raw query is ever
  ...
  `sort=<field>&order=asc|desc`, `limit`(â¤100)+`offset`. Response gains `{total}`
...
- **Tested:** 12/12 unit tests (multi-clause filter, numeric range, in/contains,
  free-text, sort asc/desc incl. string, pagination, injection-ish value â safe
  no-match). Live on real listings: Miami under $2M â 1 result; bedsâ¥3 sorted by
  ...
  unevenly for ~30-60s, so a query can briefly hit old code â settles quickly.)
...
### Website builder â ATTACH IMAGES to the builder (2026-07-18)
- **Status:** â shipped to main + deployed. UI + routing verified; the full
  build path (host â vision â embed) NOT live-tested yet (a real build spends the
  owner's credits â offered to run one on request).
- **What:** the builder composer (BOTH the home "What are we building" box and the
  workspace revise box) can attach up to 3 images (â¤5 MB, png/jpg/webp/gif). On
  build/revise they ride in `body.images` (base64 data URLs). The engine, for each:
  (1) hosts it in R2 under `assets/<siteId>/` â a public URL served from
  ...
  LOGO/product photo â embed a real `<img src="<hosted url>">`; if it's a design
  REFERENCE â match palette/layout but don't embed; decide from the brief. Vision
  ...
  captured by the metered billing (usageMetadata) â no separate charge logic.
...
### Website builder â PAYMENTS (Stripe, owner's own key) (2026-07-18)
- **Status:** â shipped to main + deployed. Security gate live-tested; the actual
...
- **Model:** each SITE OWNER uses their OWN Stripe account (no Connect, no platform
  fee, no onboarding) â they paste their Stripe secret key into Cloud â Secrets as
  STRIPE_KEY. Built on the functions/secrets/webhooks stack.
- **What:**
  1. **checkout action** â a new function step `{do:"checkout", secret:"STRIPE_KEY",
  ...
  2. **Signature-verified order webhooks** â a function with `"verify":"stripe"`
     ...
     event is 400'd and never runs â can't record a fake paid order. Owner pastes
...
- **Tested:** 7/7 unit (valid/wrong-secret/tampered/replay/malformed sig) + live:
  no-sig webhook â 400, valid-sig â runs+saves order, tampered-body+same-sig â
  400, checkout-without-key â refuses (empty url, no Stripe call). Throwaway
  user/site/secret/functions all cleaned up.
- **Not yet:** a live end-to-end paid checkout (needs a real Stripe test key).
- **Payments setup card â DONE 2026-07-18:** Cloud â Payments card (credit-card
  icon) opens `sitePayments()`, a 3-step guide (Stripe key in Secrets â ask the
  builder for a Buy button â optional signature-verified order webhook). Mirrors
  the Emails guide. With this, EVERY Cloud card is now Live (no "Soon" left):
  Members Â· Submissions Â· Database Â· Emails Â· Secrets Â· Edge functions Â· Payments
  Â· Files.

### Website builder â EMAIL (owner's own provider) (2026-07-18)
- **Status:** â shipped to main + deployed. Guard live-tested; a real send needs
  the owner's provider key. Owner asked for this ("so user can bring their email")
  â same bring-your-own-key model as payments.
- **What:** new `email` function action sends through the SITE OWNER's OWN email
  provider â **Resend** (default), **SendGrid**, or **Postmark** â using their key
  ...
  {ok,status} is captured. Guards cleanly when unconfigured (no key/from/to â
  error, no send). from/to/subject/html support {{templates}}.
- **Where:** worker.js (`email` action in runSiteFunction + normalizeFnSpec +
  FN_ACTIONS, SITE_RULES EMAIL paragraph â welcome/notify/receipt patterns, and
  "wire a contact form to a FUNCTION to email the owner"). chat.js/styles.css:
  the **Emails** Cloud card flipped from Soon â live, opens `siteEmails()` â a
  2-step setup guide (add provider key in Secrets â ask the builder).
- **Tested:** unconfigured email step â "email not configured" error, no send â
...
- **Note:** this is the SITE's transactional email (owner's provider). It's
  ...
  Function) â those are unchanged. (Visitor-auth password reset now DOES ride on
  this same owner-provider email â see the next section.)

### Website builder â VISITOR PASSWORD RESET (2026-07-19)
- **Status:** â shipped to main + deployed. Owner asked for "Visitor password
  reset." Builds on the visitor-auth backend (site_users) + the owner-provider
  email above â a site's visitors can now reset a forgotten password by email.
- **What:** two new backend endpoints (bring-your-own-email; no platform sender):
  - `POST /api/site/auth/reset-request` `{slug,email}` â **ALWAYS returns
    ...
    "reset"`) and emails a link `https://isibi.ai/s/<slug>/reset?token=â¦` through
    the **site owner's own** provider (`sendSiteEmailByConvention` â EMAIL_FROM +
    RESEND/SENDGRID/POSTMARK key from the vault). Sent via `ctx.waitUntil` so the
    response is instant. No email configured â request still succeeds silently,
    nothing sent (bring-your-own, no fallback). Honeypot + body-size guarded.
  - `POST /api/site/auth/reset` `{token,password}` â verifies the token
    (purpose+expiry+signature), re-reads the user, and checks the token's `pv`
    still equals the CURRENT hash â **single-use** (a used or superseded link is
...
- **Email helper refactor:** pulled provider-send out of the `email` function
  action into a shared `postProviderEmail(provider,key,from,to,subject,html)` â
...
- **SITE_RULES:** new PASSWORD RESET paragraph teaches the generator to build (1) a
  forgot-password form â `/api/site/auth/reset-request`, always showing the SAME
...
- **No new schema** (reuses site_users + site_secrets). No owner-facing UI change â
  the Emails/Secrets cards already cover the one setup step (add email creds).

## 2026-07-19 â Builder: conversational gate (chat vs build)
...
- **Fix:** a cheap intent classify runs at the top of /api/site (build + revise).
  Greeting / thanks / small talk / question / too-vague â returns
...
- **Live-verified** (throwaway acct, deleted): "hey" â "Hey there! What kind of
  site are we making todayâmaybe a landing page for a cozy coffee shop?" (1
  credit, no build); "can you make online stores?" â "Yes, absolutely! What kind
  of productsâ¦" (1 credit). Real briefs still build (bias + the earlier full
  build verification). Note: first hit timed out on edge-propagation lag, fine
  after.

## 2026-07-19 â LIVE VERIFICATION of the new builder pipeline â
Owner OK'd the spend. Ran a real build + edit on a throwaway account (800 credits
set, fully deleted after via delete_account â which also re-verified that wipe).
- **Build:** "modern real estate agency like Zillow" â 4 pages (Home, Properties,
  ...
  - **Composed chrome CONFIRMED:** header + footer BYTE-IDENTICAL across all 4
    ...
    Verified visually too â Home + Listings share the exact same header.
  - **Favicon + OG meta** present on all 4 pages.
  - **On-subject images CONFIRMED:** downloaded a generated image â a stunning
  ...
  - Design quality is genuinely upscale (editorial serif hero "Where Space Becomes
    Art", real copy, filter bars) â not AI-slop.
- **Surgical edit CONFIRMED:** "change hero to 'Find Your Forever Home' + make the
  Browse Portfolio button forest green" â **5 credits, 6.8 sec** (vs 209/3min for a
...
- Net: surgical edits, validate/auto-fix, composed shared-chrome, brand pin,
  on-subject images, favicon/OG, retry, no-op â all confirmed working on a real
  build. The pipeline is no longer just unit-tested; it's observed end-to-end.

## 2026-07-19 â Builder: polish + no-op edits + Stop button
...
- **No-op edits ("check before editing", borrowed from a competitor prompt the
  ...
  satisfies the instruction â returns `{edits:[],done:true}` â Worker returns the
...
- **Stop generation:** while a build/revise runs, the send button becomes a red
  pulsing Stop (â ). Click aborts the in-flight `/api/site` fetch (AbortController)
  â workspace freed, "Stopped" note. Caveat surfaced: charge-after model means a
...
- **Note on the "chatbox edits the site" question:** confirmed the flow is fully
  wired â the "Ask isibiâ¦" composer â `siteSend()` â `/api/site` revise â the new
...
## 2026-07-19 â Builder RELIABILITY: surgical edits + validate/auto-fix
Owner: the builder feels "weak and not reliable" vs Lovable â wants code-gen +
...
- **#1 Surgical edits (biggest win):** the revise step no longer regenerates the
  ...
  untouched byte is identical â no drift, faster, cheaper. Falls back to the old
  ...
  anchorâfallback, partial, whole-doc, malformed).
- **#2 Validate â auto-fix loop:** `validateSiteHtml()` â cheap, high-precision,
  no JS execution â flags truncated docs, leftover 'lorem ipsum', hotlinked
  ...
  Clean pages skip it â no extra cost. Validator unit-tested 7/7.
- **#3 Shared chrome (one source of truth) â SHIPPED:** the plan now emits the
...
- **All three (surgical + validate + shared-chrome) are DEPLOYED but the
  AI-dependent behavior is NOT yet observed on a live build** â deterministic
...
## 2026-07-19 â Builder: collapsible chat rail + cross-page consistency
Owner flagged three things on the real-estate site: (1) the chatbox should be
hideable; (2) design is inconsistent â logo/brand name changes per page
...
- **Collapsible chat rail (frontend):** topbar toggle (panel icon) hides/shows
  `.st-rail` via a CSS class on `.st-ws` â NO re-render, so the preview iframe
...
- **Brand drift + "isibi" leak (worker prompts):** root cause â the plan defined
...
- **Off-subject images (worker prompts):** DESIGN_BAR photography now requires
  ...
  STATIC `<img data-gen>` per item â not JS-built with a hardcoded/placeholder src
...
## 2026-07-19 â FIXED: blank workspace preview (CSP inheritance)
...
per page w/ inline <style>+<script>; Lovable = full React/TS codebase â noted for
the owner, no action).
- **Root cause (reproduced, not guessed):** the preview loaded the page via a
  ...
  is `script-src 'self'` (NO 'unsafe-inline', by design â app XSS protection), so
  the generated site's **inline `<script>` was blocked** â no JS â JS-built content
  never rendered â black. Styles were fine (style-src has 'unsafe-inline'); scripts
...
- **Fix (no change to generated sites):** the workspace iframe no longer uses a
  ...
  `/preview/` like `/s/` â serves it with the **website CSP** (inline scripts run),
...
- **Verified live:** POST â `/preview/<uid>/<nonce>`; GET â 200 with
...
- **Note:** the errShim `blankCheck` ("page renders blank on load") was firing a
  false "your JS hides content" hint for pages that were actually fine â because
...
- **Workspace restructure â Builder is home, other views float (2026-07-15):**
  ...
  **Workspace nav** (the 6-item Home/Builder/Gallery/Products/Avatar/Media Agent
  ...
  and the only thing `enterApp` opens. The **sidebar stays slim** â chats only
...
- **Public marketing landing (2026-07-12, redesigned 2026-07-13, owner approved
  "main" 2026-07-13 â merged):** logged-out
  ...
  changes). Boot: signed-in â `enterApp`; logged-out â `showMarketing()`; CTAs
  (`data-mkt`) â `openAuthFrom()` opens the gate; gate "â Back" (`#authHome`) â
  back to landing. **Design = Morphic style** (owner: "i kinda want it like that
  one" â https://godly.design/site/morphic/): dark cinematic, compact
  ...
  close, ghost "isibi" wordmark in the footer â all in isibi's pinkâamber.
  **Media slots (owner will supply the images/videos):** drop files at
  `public/mkt/f1.jpg` â¦ `f14.jpg` (filmstrip: row 1 = f1âf7, row 2 = f8âf14,
  16:9, ~600px wide is plenty) and `public/mkt/p1.jpg` â¦ `p8.jpg` (preset
  cards, 16:10). Styled placeholder gradients show until a file lands â no
...
- **Auth is a popup now (owner request, 2026-07-13):** the full-screen sign-in
  ...
  modal OVER the marketing landing (dimmed + blurred backdrop). Closes via â
  (top-right, was "â Back"), backdrop click, or Esc â all return to the landing
...
_Status key: ð´ open Â· ð¡ in progress Â· â fixed_
...
### Video-model schema audit round 3 (2026-07-16) â 2 fixed, 6 catalogued
- **Status:** â fixes shipped; the catalogued items await owner decisions
- **Reported:** 2026-07-16 â owner: "now that we checked the image models, we
...
- **Fixed 1 (money):** Seedance's schema `duration` default is **"auto"** (model
  picks the length, up to 15s) â a duration-less submit (tampered client; the
  real UI always sends one) would render up to 15s while billing fell back to
  the 5s base (~3Ã undercharge, worst case ~$16 at 4K). Worker now pins
  `duration:"5"` whenever none is given, so the render always matches the bill.
  Every other family's schema default already equals its billing base â checked.
- **Fixed 2 (feature):** **Kling multi-shot now works with a start image /
  ...
  (the old code comment claimed t2v-only â the fresh schema disproved it). New
  shared `shotsApply()` gate client-side (a clip still disables shots â the o3
...
- **Verified clean:** every model's durations/ratios/resolutions match schema
  exactly (incl. Veo 4s/6s + 4K, Ray 21:9/3:4/4:3, Kling 3â15s, Gemini 3â10s);
  ...
  Seedance vref 0.6Ã(in+out)); Seedance `generate_audio` confirmed free in
...
- **Round-3b (same day, owner: "add that stuff"): four knobs wired**, all
  ...
  extras rail (sanitizeExtras â body â worker re-validates):
  1. â **Seedance `bitrate_mode:"high"`** (full+fast; mini's schema lacks it) â
  ...
  2. â **Kling `shot_type:"intelligent"`** â the model auto-directs the cut
  ...
  3. â **Kling v3 `cfg_scale` 0â1** â prompt-adherence dial ("follow it
     exactly" ~0.8 / "go loose" ~0.2). o3 has no such field â gated off there.
  4. â **Ray v2v per-signal `controls`** (pose/depth/normals/trajectory/face,
     each with its dial) â set when the user says what to keep/free on a clip
...
- **Round-3c (same day, owner: "pretty sure there are still missing stuff"):
  the owner was right â a CATALOG probe (not just param-diffing endpoints we
  already used) found 12 unwired endpoints; all wired:**
  1. â **Veo 3.1 Fast** â the whole family (t2v/i2v/first-last/reference/
     extend), ~2.7Ã cheaper than full Veo ($0.15/s audio-on, $0.10 off;
     4k $0.35/$0.30). New picker entry; all generic Veo code paths apply.
  2. â **Kling o3 Standard** â cheaper o3 (t2v/i2v/reference/edit; $0.112/s
     audio-on, $0.084 off, edit $0.126/s). New picker entry.
  3. â **Kling o3 reference-to-video (pro + standard)** â â¤4 reference images
     bound as native **@Image1â4 prompt tags** (Seedance-style), optional
  ...
  4. â **OmniHuman v1.5** â $0.16/s, 720p/1080p picker (billed the same),
     optional typed text guides motion/emotion. New picker entry next to v1.
  5. â **Kling LipSync text-to-video** â attach a clip and just TYPE the words
     (no audio upload): Kling voices them itself. Curated 7-voice English picker
     (Narrator default; the schema's other ~39 voices are Chinese â skipped).
  ...
  - **Found but NOT wired:** `o3/pro/video-to-video/reference` (re-render a clip
    WITH reference images, $0.168/s output) â its `duration` input is nullable
...
- **Round-3d (2026-07-16, owner: "Kling elements + bitrate sanity check"):**
  - â **Kling character elements SHIPPED (v1)** â a new **Characters** attach
    row on all four Kling models (o3 pro/std, v3 pro/std): up to 4 characters,
    each ONE frontal image, badged **@Element1â4** with tap-to-cite chips in
    the composer (same rail as @ImageN). Identity holds across the video.
    Routing: o3 + characters (no clip) â reference-to-video (characters and
    style refs can combine, start/end frames ride along) Â· o3 + clip â
    edit endpoint (fal caps characters+refs at 4 COMBINED â characters get the
    slots first, pre-send guard blocks over-cap) Â· v3 â i2v only, so a start
    ...
    **v2 later (if wanted):** per-character angle shots (fal takes 1â3
    `reference_image_urls` per element) â needs a per-slot "+ angles" UI.
  - ð¡ **Seedance bitrate_mode billing** â three independent FREE signals say
    high bitrate is not billed (pricing page has no bitrate dimension; the
    token formula hÃwÃdurÃ24/1024 has no bitrate term; web sweep finds zero
    mention). Final 100% = one live job, folded into the fal-balance live sweep.
- **Still NOT wired (deliberate):**
  1. **Veo `seed` + `safety_tolerance` (1â6)** â defaults kept: seed has no
     reproducibility story in the chat flow; safety_tolerance is a policy knob.
  - Seedance "auto" duration as a UI pick: skipped on purpose â can't price an
    unknown output length.

### Model-wide fal input-validation audit (2026-07-14) â most fixed, 2 deferred
- **Status:** â main gaps fixed; two low-risk items intentionally deferred (below)
- **Reported:** 2026-07-14 â after a v2v edit 422'd ~50Ã (root cause: clip was 15.10s, over Kling's strict **15.05s** cap; our attach check had a 0.5s grace so it slipped through). Owner: "check that now for every model."
...
- **What / Fixed:**
  - Clip duration tolerance now matches fal's exactly (0.05s, was 0.5s) â `CLIP_LIMITS`/`clipIssue` in chat.js.
  - **Veo reference-to-video** locks duration to `"8s"`; we were sending the user's 4/6/8 â 422. Worker now forces `"8s"` for that endpoint.
  - **Audio limits** added (`AUDIO_LIMITS`/`audioIssue`, chat.js): Kling LipSync (.mp3 â¤5MB 2-60s), OmniHuman (â¤30s), Seedance ref audio (MP3/WAV â¤15MB â¤15s) â validated at attach + send.
  - **Ray prompt cap** was defaulting to 20000 on t2v/i2v; real cap is 6000 (all `luma/`). Worker clamp fixed.
  - Clips are staged to **fal storage** (hosted URL) before submit (`falUpload`, worker) â data URIs worked for the duration probe, but hosted URLs are universally accepted and keep request bodies small.
...
- **Formerly-deferred items â BUILT 2026-07-14 (owner: "sure do that"):**
  1. **fps auto-conform** â `sbFFProbeFps`/`sbFFFps` (ffmpeg-edit.js) probe the attached clip's real fps via the on-device engine; `normalizeClipFps()` (chat.js) re-encodes out-of-range clips to the nearest bound (e.g. 23.98 â 24 for Kling o3) automatically, free, with a chat note. Runs at attach + on model switch.
  2. **Image dimension checks** â `imgMeta`/`imageAttachIssue()` measure every image slot (image/end/first/last) and bounce Kling-bound images under 300Ã300 or outside aspect 0.40â2.50 at attach, re-checked on model switch.
  3. Also: image models' min prompt length (nano-banana-pro = 3 chars) guarded in raw mode.

### Pricing audit â fal bills per ENDPOINT, not per model (2026-07-15)
- **Status:** â fixed
- **Reported:** owner noticed the successful Kling o3 v2v edit billed **$2.52** on fal while the app charged 263 credits ($2.10) â the edit endpoint bills $0.168/s, a 20% premium over t2v's $0.14/s. Root cause of the class: price tables keyed rates by MODEL while fal prices each ENDPOINT separately. Swept all 31 endpoints' pricing pages.
- **Fixed (both `VIDEO_USD`/`VIDEO_PRICE` worker+client tables):**
  - Kling o3 v2v edit â own `v2s` rate $0.168/s (15s edit now quotes/charges 315 credits).
  - **Veo extend** outputs a const 7s clip â billed at 7s regardless of the duration picker ($2.80/350 credits, audio-on rate).
  - **Ray i2v** is priced BELOW t2v (5s 720p $0.30 vs $1.00) and 10s is unavailable from a start image â new `i2s` tier + duration forced to 5s (was overcharging ~3Ã).
  - **Kling LipSync** bills the INPUT VIDEO's seconds rolled up to 5s steps ($0.014/s) â we billed per audio seconds (could undercharge 10Ã). Now billed from the client-measured clip length (clamped 2â10s; unknown bills the 10s max).
  - **gpt-image-2** is token-billed; High 1024Â² â $0.211 â flat rate raised $0.12 â $0.22.
  - Seedance per-second nudges: std 720p 0.304 / 1080p 0.682; fast 480p 0.135 / 720p 0.242; mini 480p 0.0725.
- **Verified correct:** Veo tiers (audio-on rates), Ray t2v/v2v + HDR 2Ã/EXR 3Ã, Gemini ~0.13/s, Kling t2v/i2v all tiers, nano-banana $0.15 (4K would be 2Ã â we only render 1K), all ElevenLabs rates.
...
- **Status:** ð¡ one-time cleanup done 2026-07-14; root fix deferred (owner: "later, just note it for audits")
- **Reported:** 2026-07-14 (owner deleted their `aniascristian@gmail.com` test account and asked to verify)
- **Where:** Postgres `public.delete_account()` â its `delete from usage_log` and the storage clause `owner=uid`; also the client Delete-account flow in `public/chat.js`.
- **What:** Audit after the deletion found the auth user + identity, chats, credits, user_memory, user_plan, video_editor_plan, and all GoTrue child rows (sessions/refresh_tokens/identities) GONE â but **3 `usage_log` rows** and **1 storage object** (`media/test/hello.txt`, owner = the deleted uid `144474c8-bb38-4ffc-a867-d9fb54c31bcd`) were left behind. No `purchases` existed (those are intentionally KEPT as a financial record anyway).
...
- **What:** fal endpoint `luma/agent/ray/v3.2/text-to-video` (+ `/image-to-video`
  ...
  reference mode, no audio. fal pricing â per-sec: 540p $0.10 Â· 720p $0.20 Â·
  1080p $0.40 (t2v rates used for both paths â never undercharge). HDR/EXR and
  keyframes exist on the API but are NOT exposed yet (HDR doubles cost).
- **Untested on a real render** â fal balance still empty; verify on the live
  sweep when topped up.
- **HDR toggle added (owner request):** Settings panel shows an "HDR Â· 2Ã price"
  Off/On section for models with `opts.hdr` (Ray only). Guardrails both ways:
  turning HDR on bumps 540pâ720p and 10sâ5s; picking 540p/10s turns HDR off.
  Price quote doubles live; worker validates independently (`wantHdr` â wrong
  combos are neither sent to fal nor charged) and `creditCost` bills 2Ã.
- **Loop / EXR / video-to-video added (owner: "add all of those", 2026-07-12):**
  (1) **Seamless loop** â free Off/On in Settings; 5s SDR only, exclusive with
  HDR, dropped server-side alongside end frames/keyframes. (2) **EXR** â the
  HDR section is now Off / OnÂ·2Ã / On+EXRÂ·3Ã; sidecar link is delivered as a
  chat message after the render (fal links expire in days). (3) **Video-to-video**
  â attaching a Video clip on Ray routes to `/video-to-video` (re-render the
  ...
  framing wins); billed at its own higher rates (`v2s`: 540p $0.144 Â· 720p
  $0.216 Â· 1080p $0.432 per sec) in both quote and charge. Granular v2v
...
- **Keyframes added (owner request, full 64):** new "Keyframes" attach row,
  Ray-only (`caps.kf: 64`) â up to 64 images, numbered tiles in a 2-up grid,
  ...
  across the clip (24fps: 0â120 for 5s, 0â240 for 10s). No timeline UI yet â
...
### Builder: reference images must be visibly defined as @Image1, @Image2â¦
- **Status:** â done
- **Reported:** 2026-07-11
...
- **What:** For models that take reference images, the owner wants the chat to
  ...
  into Seedance prompts; worker appends "Feature @Image1â¦" to raw prompts and
  strips dangling tags) â but the UI never told the USER the tags exist.
- **Fix:** Each reference thumbnail now wears an @ImageN badge, and the ref-row
  ...
  apply to ALL reference-capable models â so badges show wherever a Reference
  row exists, and the worker translates @ImageN â "reference image N" for
...
- **In the chat too:** sending a message with references now drops a thumbnail
  strip under the user bubble (right-aligned), each thumb tagged @ImageN â the
  thread records which image each cited tag pointed at. Thumbs are downscaled
  (â¤168px JPEG) before persisting so the localStorage chat budget + Supabase
  chat sync stay small; strip type is `{t:'refs', imgs:[â¦]}` in chat msgs.
...
- **What:** First pass widened only YouTube â Videos (PR #352). Owner then said
  every tab â including all Instagram tabs â should use that wide size.
...
- **Status:** â fixed
- **Reported:** 2026-07-11
- **Where:** `worker.js` â `youtubeVideos()` (~line 1180) + the CSP `img-src` (~line 382)
...
- **Fix:** Filter tombstones server-side (title "Deleted video" AND no
  ...
  largest thumb available (maxres â standard â high â medium â default). (PR #___)
...
- **Cause:** `.sec-tabs` sets `overflow-x: auto`; per CSS rules that makes
  ...
  couple px â browser painted a tiny default (white) vertical scrollbar.
...
- **What:** A composer on the Home page (same panel style as the Builder's),
  docked at the BOTTOM (owner: "put it on the bottom" â sticky, pins low on
  ...
  normal send path (orchestrator included) â the user lands mid-conversation.
- **Preset cards re-wired (2026-07-12, supersedes the display-only interim):**
  ...
  (reference: a "3D object generation Ã" pill the owner showed) + switches mode
  to the card's kind. The user types just their idea; on send the preset's
  prompt rides along as "Creative direction â follow this preset: â¦" for the
  director. Ã unpins; chip clears after send and on view re-render. Sending
...
- **Cards fully wired (owner: "actual thing behind the screen", 2026-07-12):**
  every one of the 19 presets is now a real RIG â a director-grade prompt
  ...
  testimonial â Seedance 2.0 Â· 9:16 Â· 10s; Sale announcement â GPT Image 2 Â·
  1:1; Floating product â Ray 3.2 Â· 1:1 Â· 5s; Epic establishing â Veo 3.1).
...
- **Two new Marketing cards (owner's references, 2026-07-12):**
  (1) **Product Animation** â exploded-view rig: components separate in
  ...
  (2) **From product URL** (`urlScan: true`) â pin chip, paste a store link,
  ...
  director, and the ad preset runs 9:16. Chip shows "Reading the pageâ¦"
...
- **QR burned into product-URL videos (owner request, 2026-07-12):** videos
  generated in a "From product URL" chat get a scannable QR (â the product
  page) burned into the bottom-right corner BEFORE saving â real pixels, so
  downloads/re-shares carry the link forever. Pipeline: vendored MIT
  `qrcode-generator` (`public/vendor/qrcode.js`, CSP-safe) â `qrPngFor()`
  canvas PNG â `sbFFQr()` in ffmpeg-edit.js (on-device ffmpeg overlay via
  scale2ref at ~22% of video height, same encode args as the caption burner) â
  ...
  normal unburned save on ANY failure (free tier/no storage â temp link, no
  QR â burned copies can't persist without gallery storage). Untested against
  a real render (fal balance) â verify in the sweep.
- **Conversational QR control (owner request, 2026-07-12):** the user can now
  direct the QR in plain chat â "put a qr code from second 3 to 4", "add a QR to
  ...
  returns â Seedance/Veo/Ray all identical). Want-but-no-URL â asks for the link.
...
- **QR position by voice (owner request, 2026-07-12):** the user can also say
  where â "qr top-left", "bottom-left corner", "center". `parseQrDirective`
...
- **QR is BURNED CLIENT-SIDE, not by the model or server:** the generation model
  (Seedance/Veo/Ray) never sees "QR" â the instruction is stripped from the
...
- **â  Model picks are PROVISIONAL:** owner said (2026-07-12) they will dictate
  the right model per preset later â treat the current assignments as
...
- **Status:** â done (interim behavior)
- **Reported:** 2026-07-11
- **Where:** `public/chat.js` `renderPresetsInto()` / `usePreset()` (~line 934)
- **What:** Clicking a starter card on Home ("Product hero ad", "UGC testimonial"â¦)
...
### Attachments cleared when switching to a non-supporting model â NOT a bug (owner's call)
- **Status:** â working as intended â do not change
- **Reported:** 2026-07-11
- **Where:** `public/chat.js` `updateAttachVisibility()` (~line 384)
- **What:** Attach an image â switch to a model without image support â switch
...
### Messages leak across all chats â thread never repaints on switch
- **Status:** â fixed
- **Reported:** 2026-07-11
- **Where:** `public/chat.js` â duplicate `renderThread` (1592 + 5112)
...
- **Cause:** Two top-level functions were both named `renderThread` â the real
...
### Model capabilities wired up â @Video refs, Kling shot-lists, Gemini i2v, GPT ratio fix
- **Status:** â shipped
- **Reported:** 2026-07-15 â owner: "add everything a model supports â¦ lock in."
- **What (four things, after auditing every model's live fal schema):**
  1. **Seedance `@Video1` reference** â you can now drop a video clip alongside
     your image refs on Seedance (all 3 tiers) â it rides into reference-to-video
     ...
     "no ref endpoint" note was stale â verified on fal).
  2. **Kling `multi_prompt` shot-lists** â Kling o3/v3 t2v can render a CUT
  ...
  3. **Gemini image-to-video** â a whole fal endpoint we never wired. Gemini now
     has a start-frame (image) slot â `google/gemini-omni-flash/image-to-video`.
  4. **Bug fix: GPT Image 2 aspect ratio** â it has no `aspect_ratio` field
     (sizes via `image_size`), so a picked ratio was silently dropped and every
     render came out landscape 4:3. Now maps ratio â `image_size` enum.
...
- **Deferred on purpose (need fal cost-deltas or lower value):** `generate_audio`
  toggle (enabling audio where it's OFF by default â e.g. Kling o3 â risks
  ...
  `quality` â price levers, need repricing), Kling "elements" character mode
  (@Element multi-angle consistency â bigger feature), ElevenLabs voice tuning
  (stability/style/speed â no price impact, marginal). All catalogued from the
  per-model schema audit.

### Round-2 audit (owner asked: "check the missing stuff again, check credits too") â 33 confirmed findings, fixed 2026-07-15
- **Status:** â fixed (the fix batch below); rest documented
- **Money bugs found & fixed (quote == charge re-verified, 27-case parity test):**
  1. **Kling o3 was billing audio-ON ($0.14/s) for SILENT renders** â o3's
     `generate_audio` defaults false (every other family: true). Fix: o3 t2v/i2v
     now send `generate_audio:true` â the video you pay for has sound.
  2. **o3/Gemini clip edits billed the duration PICKER; fal bills the CLIP's
     length** (the owner's $2.52 15s edit proved it â we'd charged for 5s).
     Fix: bill the clip's server-measured length (o3 â¤15s; Gemini capped at 30s
     â our product cap, attach-validated).
  3. **Veo reference-to-video always renders 8s but billed the 4/6s pick** â
     fix: bill 8s (both sides).
  4. **Seedance @Video reference pricing** â fal prices video input at 0.6Ãrate
     over (input+output) seconds; we billed flat t2v Ã output. Fixed to the
     0.6Ã(in+out) basis (covers both readings of fal's page; relax later if a
     live job bills less).
  5. **Auto-mode multi-shot could show â¦79 and charge â¦788** â Auto now posts
     "ð¬ Multi-shot: N shots Â· Xs total â â¦YYY" in the chat before billing
     (Plan card was already exact).
  6. **/api/direct kept the fee when the AI call failed** â every terminal
     ...
     amount)` RPC (SECURITY DEFINER, **service_role-only EXECUTE, â¤10 credits
     per call** â worker-authorized only, not client-callable).
  7. **gen_charges insert was fire-and-forget** â a failed insert after a
  ...
  8. **Compose could truncate a big multi-shot answer** (max_tokens 4000, and
     thinking shares the budget) â user paid, got local fallback. Raised to 8000.
...
- **Free upgrades wired:** nano-banana now renders **2K** (same $0.15 as 1K on
  fal â verified; 4K is 2Ã and stays unwired). Veo `auto_fix:true` normalized on
  all Veo endpoints (i2v defaulted false â content-policy trips failed instead
  of self-healing).
- **Director-driven knobs wired (owner's call: AI sets them, no new UI):**
  - **"silent/no sound"** â `sound:false` â `generate_audio:false` (Seedance/
    Kling/Veo; o3-edit `keep_audio:false`) â AND bills the cheaper audio-off
    tier where fal has one (Veo halves; v3 pro 0.112, v3 std 0.084, o3 0.112).
  - **"no people / avoid text"** â `negative` â `negative_prompt` on Kling v3
    (APPENDED to fal's quality-guard default, never replacing it) and Veo.
  - **"say it slower / more expressive"** â ElevenLabs `speed`/`stability`/
    `style` (v3 model: stability only). Price-neutral.
  - All ride the same pendingâreview-cardâgenerateMedia lifecycle as shots.
- **Seedance @Video clip band:** fal caps reference clips at ~0.41-0.93MP pixel
  AREA (1280Ã720 fits; 1080p doesn't). Attach now auto-downscales oversized
  clips on-device for free (`sbFFScale`, same pattern as the fps conform);
  under-480p clips are rejected with the reason.
- **nano-banana full ratio list** (3:2, 2:3, 4:5, 5:4, 21:9 added â per-model
...
- **Round-1 wiring re-verified correct** by schema re-fetch: multi_prompt rules
  (prompt omitted when shots sent â v3 requires that), Gemini integer duration,
...
- **Known exposures documented, NOT charged for yet (need one live job each to
  verify):** Kling v3 "voice control" third price tier ($0.196/$0.154 â
...
- **Deliberately skipped (reasons on file, don't re-flag):** Kling v3
  ...
  `output_format`, Kling **elements** mode (character consistency â real
  feature, deferred to its own pass: needs elementsâimage_urls live testing
  and the voice_id half trips the voice-control price tier).

### Post-round-2 free checks (2026-07-15, same day)
- **Supabase security advisors run:** one real WARN actioned â the six dead
  ...
  **leaked-password protection** (HaveIBeenPwned check) in Supabase Auth
  settings â one toggle, dashboard or Management API.
- **`sbFFScale` verified headless in a real browser engine:** a 1920Ã1080 H.264
  clip conformed to 1278Ã718 (917,604 px â inside Seedance's reference band),
  ...
  in-browser playback of the result can't be asserted there â real
...
- **Bug the owner hit:** saved a dog image, later deleted the chat it was in,
  and it vanished from the Gallery â but Storage still showed 1.2 MB used.
- **Root cause:** `galleryItems()` rebuilt the whole gallery by scanning
  ...
  a chat message still pointed at it â delete the chat, lose the card, and the
...
- **Fix:** the Gallery now reads what's ACTUALLY in the caller's storage.
  - New RPC `list_media()` (SECURITY DEFINER, auth-only, mirrors
    `storage_status()`'s `media/<uid>/` prefix scope) â returns each object's
    name/size/created_at.
  - New `GET /api/gallery` â maps rows to `{url, kind (from extension), size,
...
- **Recovers the owner's dog automatically** (it was always in storage â
...
- **Two causes, both fixed:**
  1. `/api/save` uploaded to Supabase Storage with NO cache-control header â
     the browser re-downloaded every multi-MB original on every refresh.
     Filenames are unique (`<ms>-â¦`) and files immutable, so uploads now send
     `cache-control: max-age=31536000` â after first view, media comes from
     the browser cache instantly. **Only NEWLY saved files get this** â
  ...
  2. Big JPEGs paint top-down while downloading. All saved-media `<img>`s
     ...
     fade-in â images appear whole, never half-painted. Plus
     `decoding=async` + `loading=lazy`.
- Headless-verified on all three surfaces (fade class applies, ready fires,
  no page errors). Product thumbs skipped on purpose â they're inline data
  URIs (instant already).

### Avatar page went invisible â CSP vs inline handlers (2026-07-16)
- **Owner's report:** "AVATAR NEVER RENDERED" â blank card where the avatar
...
- **Root cause (mine):** the avatar fade was wired with an inline
  ...
  scripts, so in production the handler never ran â `.img-fade` stayed at
...
- **Fixes:** avatar thumbs + creator result rewired via addEventListener
  ...
  (`onerror="this.remove()"` on Builder preview photos â silently dead under
...
- **Mechanics** (one stored file, reference-counted by location):
  - Gallery delete, file still shown in a chat â new `POST /api/media/unlist`
    MOVES it to `media/<uid>/chat/<file>` (service key â bucket RLS has no
  ...
  - Gallery delete, nothing references it â hard delete (frees space), as before.
  - Chat delete of a chat-only file's LAST reference â hard delete (it's
...
- **Owner's report:** "PRODUCT IS NOT CACHING IT" â Molly's Suds product card
  saved with a name but the ð¦ placeholder. Balance dropped exactly 3 credits
  â it was the AI lookup path (walled Walmart listing).
- **Why images went missing:**
  1. AI path: web_search returns TEXT â Claude's "direct image links" are
     often stale or guessed, so all candidates 403/404'd â name, no image.
  2. Normal path: only ONE extracted image was tried, and stores like Shopify
     ...
     page fetch â single candidate dies, no picture.
- **Fixes (worker):** `extractProduct` now returns `images` (â¤4 deduped
  ...
  `report_product` gains `page_urls` (â¤2 alternate product PAGES, brand site
  first, never the blocked store) â when every direct image link fails, the
...
- Owner's existing pictureless card: remove + re-run the lookup (or scan the
  brand-site link directly â free path, no wall).
...
- **Chain of failure:** the new rescue-pages path â Claude offered an
  amazon.com page (despite "never the blocked store") â Amazon 200s its
  captcha wall â extractor's og:image on that page IS the Amazon logo â
  junk filter only covered the last-resort <img> scan, not og:image â logo
  inlined as "the product".
- **Guards added (worker):** `JUNK_IMG_RE` (logo/sprite/icon/captcha/â¦) now
  filters EVERY candidate â extractor's og/JSON-LD list AND Claude's direct
  links; `WALL_RE` hoisted + applied to rescue pages (skip walled pages
  outright); rescue-page title must share â¥1 real word with the product
...
- **Owner caught both live.** "EDIT IMAGE 5, TRANSPARENT BACKGROUND" â the
...
- **Cause 1 (wrong image):** `directorImage()` sent the director ONLY the
  main image â with 5 attached it literally couldn't see 2-5, so any "image
  ...
  1024px â¤2 imgs / 640px â¤6 / 512px above, â¤14, ~12M char server cap), the
  worker labels each block "Image 1"â¦"Image N", and ask/compose prompts say
...
- **Cause 2 (fake transparency):** no model in the lineup can output real
  alpha (checked NBP + GPT Image 2 fal schemas â no background param;
...
- **Proper follow-up (not built):** fal hosts dedicated background-removal
  models (BiRefNet / rembg, ~cents) that return REAL alpha PNGs â wiring one
  as a "remove background" edit path would make this an honest yes. Needs
  the fal balance + owner's go.

### "Codes not sending" â was Sign UP with an existing account (2026-07-16)
- Owner hit Sign Up with an already-registered email â GoTrue's
  anti-enumeration returns 200 + a FAKE user (empty `identities`) and sends
  NO email â the UI went to "check your email" and nothing ever came.
...
- **Fix:** `Auth.signUp` now detects the empty-identities response and throws
  "That account already exists â sign in instead." â shown on the form
...
- NB the send-email hook returns 200 BEFORE the background Go Farther send
  (5s hook deadline) â a mailer failure is invisible to GoTrue by design;
  it lands in the edge function's console logs only.

### Avatars + products now follow the account (2026-07-16)
- **Owner's find:** avatars/products showed on the PC but not the laptop â
  they were localStorage-only ("stored locally for now").
- **Built:** `user_assets` table (avatars jsonb, products jsonb, updated_at;
  RLS own-row select/insert/update; FK cascade â account deletion cleans it).
  Client mirrors the memory sync: saveAvatars/saveProducts â touchAssets â
  debounced pushAssets upsert; pullAssets at boot with whole-object
  last-writer-wins. Images compacted to â¤800px JPEG q.82 before push (small
  ones and hosted URLs pass through untouched; scheme-filtered on pull) â the
...
### 14-image edit refused by Gemini â director now sends only the images it uses (2026-07-16)
- **Owner's test:** "EDIT IMAGE 10, ADD A WHITE BACKGROUND" with 14 attached â
...
- **Two causes addressed:**
  1. All 14 images went as inline data URIs â a huge request, and one stray
     attachment (the batch included a Messi photo â real-person content) can
  ...
  2. New `useImages` field on the compose/revise tool: the director lists the
     ...
     Worker safety net: >4 images or >5MB inline â stage data URIs on fal
     storage (falUpload, per-image fallback to inline) before submitting.
- Verified headless: useImages [3,1] of three attached â body carries exactly
  3rd-as-main + 1st-as-extra. Retest the 14-image edit live.

### Products feature removed entirely (2026-07-16)
- **Owner's call:** "I DONT THINK PRODUCT SHOULD A THING BRO" â the whole
...
- **Kept on purpose:** `downscaleImage` (avatar import), QR-burn (still fires
  from any URL typed in a message â only the preset's auto-stamp of
  ...
  â those are creative presets, not the library), and the `.pr-head`/`.pr-or`/
...
- **Owner's ask:** "WE NEED TO ADD LIKE AN IMPORT THING IN GALLERY" + "ADD THE
  FETCH THING IN GALLERY" ("same thing we had for product" â the URL box).
- **Built (gallery header, right side):** an â¤ Import button (device pick,
  multi-file, â¤12 per batch, image/video/audio with 8.5/29/14 MB caps) and an
  import-from-link box with the gradient â go button. Both go through the
  normal `/api/save` gates: no plan â pricing sheet client-side / 402
  server-side, GB cap enforced, magic-byte validation, free-user watermark
  path untouched.
- **Worker:** new `/api/import/fetch` â server-side fetch of the pasted URL
  ...
  (MP3/WAV/OGG/M4A by magic bytes, â¤20M chars) so audio imports work.
- **Found while wiring it: `sbToast` never existed.** Three call sites were
  guarded with `typeof sbToast === 'function'` â the storage-full avatar
...
- Headless-verified: 3-kind batch posts base64 per file, oversized file
  ...
  opens pricing instead of the file picker, link flow pipes fetchâsave and
  clears the box, junk input never leaves the page, server errors toast.

### Import-from-link: Walmart fix + the â¦3 AI rescue (2026-07-16)
- **Owner's report:** Walmart link â "no image found on that page" ("FIX THE
...
- **Why it failed:** Walmart's PerimeterX wall returns 200 with a "Robot or
  human?" page â no og:image â and the v1 fetch used a bot-ish UA with only a
...
- **Fixed (worker, /api/import/fetch):** Chrome UA + Accept-Language on every
  ...
  `pageImageCandidates` (JSON-LD any-@type â og/twitter â microdata â link
  image_src â lazy-load/srcset <img> scan), candidates tried in order with a
...
- **AI rescue (the 3 credits):** when the free path still comes up dry, the
  route auto-falls-back to the product scanner's escape hatch â Sonnet 5 +
  ...
  big-box walls), each tried through the SSRF guard. â¦3 charged up front,
  ...
  button now reads "â â¦3" (worst-case price up front, old product-box style).
- Live-tested extraction: Wikipedia â real image (was false-walled in v1),
  Walmart + IMDb â wall detected, would route to the AI rescue. 11 unit tests
  on pageImageCandidates against the shipped code. NOT live-tested: the paid
  Claude lookup itself (real money â needs an owner test with a Walmart link).

### Gallery audio cards redesigned + scroll-lag fix (2026-07-16)
- **Owner:** "FOR THE AUDIOS â¦ I NEED BETTER DESIGN" + "FEELS KINDA LAGGY
  SCROLLING UP AND DOWN ON THAT PAGE, WHY?"
- **Audio cards:** native `<audio controls>` rows (grey browser chrome, dead
  0:00/0:00) replaced with a custom card â pinkâamber equalizer bars that
  ...
  only for now; the chat's audio messages still use the native player â ask
  before touching those).
- **The lag:** every card on the page stayed fully live â a dozen `<video>`
...
- Headless-verified: cards render (no native controls), duration paints,
  ...
  ranges â Chromium clamps seeks to 0 on it; real Supabase storage serves
  ranges fine).
...
- **Root cause:** the grid was downloading + decoding multi-MB FULL-RES
  originals at ~300px card size (one sampled PNG: 1.6MB, 1179Ã2556).
- **Fix:** Supabase image transformations are ENABLED on this project
  (verified live) â grid cards now load
  `render/image/public/media/â¦?width=560&height=350&resize=cover&quality=75&format=webp`
  (same sampled image: 14KB, >100Ã lighter). Lightbox + downloads still use
...
- **Billing note:** Supabase counts UNIQUE origin images transformed (cached
  ...
  images/month, then $5 per 1000 â trivial at current volume, keep in mind at
  scale.
- Headless-verified: grid uses render URLs, 400 â per-image fallback to the
...
- **Owner's find:** "when I switch resolutions the price doesn't change, only
  quality" â all their test shots were on `auto` ratio, where the resolution
...
- **Fix:** the RESOLUTION row now dims + blocks clicks while ratio is `auto`,
  with a note ("On auto ratio the model picks the size â billed as 1K.
  ...
  "auto Â· high Â· 4K" â misleading).
- Verified: lock/unlock toggles with ratio picks, price flips â¦29 (auto,
  1K tier) â â¦52 (16:9 Â· high Â· 4K), summary chip drops the size at auto.
- Price audit the same session: ALL 264 GPT+Nano combos (ratio Ã quality Ã
  size Ã 1-4 images) swept through estimatePrice() â every one matches the
  fal sheet; totals are ceil'd once (not per image), same as the worker
  charges.

### GPT 1K resolution removed from the picker (2026-07-16)
- **Owner:** "1K and 2K cost the same â then delete 1K." Correct read: fal
  bills GPT Image 2's 1K and 2K identically (per quality tier), so 1K was a
  strictly-worse pick. The picker is now 2K/4K with 2K the default â same as
  Nano's row.
...
- "DELETE THE AUTO THING" â asked with the tradeoff spelled out (auto kept a
  ...
  **Edits now reframe the source to the picked ratio** â if a user complains
...
- **Owner (testing Veo):** "THERE'S NO OPTION TO HAVE AUDIO ON AND OFF" â
  ...
  sound-capable families (SOUND_MODELS_RE = seedance | kling v3/o3 | veo â
  mirrors the worker's gate).
- Off sends `sound:false` (the existing worker path: generate_audio=false +
  the aoff billing rate where fal lists one â Veo, Seedance, Kling v3; o3
  has no discount, just silence). The manual toggle wins over the director's
  inference; the chip shows "Â· Silent"; per-chat composer state persists it;
  non-capable models never see the toggle and reset it to On.
- Verified: Veo Fast 8s 720p â¦150ââ¦100 and Veo 4k â¦600ââ¦400 on toggle, wire
  body carries sound:false, Sora shows no section.

### "Image to image" â "Edit image" (2026-07-16)
...
- Owner's call. Per-model title via #titleClip (like titleImage): /veo/ â
  "Extend clip"; every other family keeps "Video clip" â on Ray / Kling o3 /
...
- **Owner:** "the 4 of them can't be activated at the same time" â right in
...
- **Enforced, Veo only:** the clip now joins the existing image/flf/ref
  exclusivity web in BOTH directions (attach a clip â the other rows clear;
  attach anything else â the staged clip clears). Other families keep their
...
- Checked every Veo cap against fal's OpenAPI schemas: durations (4s/6s/8s),
  resolutions, ratios, ref 8s lock, extend 7s output â all matched. Two
  fixes from the sweep:
  1. Extend INPUT cap was 8s, which blocked re-extending an already-extended
     video. fal allows extending up to 30s total â input cap is now 23s
     (30 â the fixed 7s output). Billing unchanged (extends bill 7s output).
...
- **Owner:** "make sure you show users the exact error." Two paths were
  ...
  1. Submit-time failures (friendlyFail) bucketed everything into canned
     lines with the raw error console-only â now the friendly line carries
     fal's exact detail appended: â¦ (exact error: "duration: must be one of
     4s, 6s, 8s"). Quota/balance/unknown-model lines stay clean (no useful
     upstream detail there).
  2. Mid-render FAILED/ERROR showed a generic "couldn't finish" â the client
     now fetches the failed job's response payload and quotes fal's reason:
     â ï¸ The model couldn't finish â exact error: "â¦" + the refund note.
...
- **Owner (Veo test #1):** the Extend clip slot showed a generic "ð¬ clip"
  chip. Now readClipMeta captures the clip's first frame (seek â canvas â
  jpeg) after validation passes and the slot renders it full-width like the
  image slots, with a small ð¬+duration tag (duration hidden when the file
  doesn't report one â recorded webms often don't). The <video> src is
...
- **Owner:** "when I refresh the page it loses the attached stuff."
  ...
  localStorage" â a clip data URI can be 25MB+).
- **Built:** each chat's staged snapshot mirrors into IndexedDB
  ...
  MaskState â so every attach, clear, exclusivity eviction, and thumbnail
...
- Verified headless: stage â reload â restored (counter intact); clear â
  reload â stays empty.
...
- Follow-up hardening on the refresh persistence (owner's go-ahead): every
  ...
  along on â and re-route â a fresh send. A boot sweep also garbage-collects
  expired rows and rows whose chat no longer exists, so forgotten 25MB clips
  don't sit in the browser forever.

### Wordmark â back to the landing page (2026-07-16)
- Owner's ask: the top-left isibi logo now returns to the marketing landing
  in its signed-in form (goLanding â enterLandingAuthed + showMarketing):
...
- Owner: "the moving frames are not moving" when returning via the logo â the
  ...
  always â it just looked like a new static one because of the frozen loops.

## âââ SESSION RECAP: image-model testing â Veo testing (2026-07-16/17) âââ
...
- Price audit: owner's 8 Nano + GPT screenshots verified, then ALL 264
  ratioÃqualityÃsizeÃcount combos swept through estimatePrice() â all match
  fal's sheet; totals ceil'd once, same as the worker charges. (no code)
- GPT resolution row locked while ratio=auto with a note (abe4979) â then
  superseded by the next two:
- 1K deleted from the GPT picker (fal bills 1K/2K identically â strictly
...
- Charge audit: worker's real creditCost extracted and compared to the UI
  quote on all 120 GPT combos + a captured wire body â identical; tamper
  edges (bad quality/size/num, stale auto) all fail safe. (no code)
- Renamed "Image to image" â "Edit image" (row, tooltips, director wording)
  (03c68ca)
...
- Full price table built + verified against fal's LIVE pages (720p/1080p
  ...
  (modelÃratioÃresÃdurÃsoundÃpath incl. the 8s ref lock and 7s extend
  lock) match the worker's charge. (no code)
- Manual Sound On/Off toggle for Veo/Seedance/Kling v3+o3 â Off bills fal's
  audio-off rate, chip shows "Silent", manual beats director (08ee13d)
- "Extend clip" rename (Veo only â Ray/o3/Gemini keep "Video clip", theirs
...
- Extend caps fixed vs the live OpenAPI schema: input cap 8sâ23s (chained
...
- Staged attachments survive page refresh â IndexedDB per chat, all slots
...
- Wordmark â the signed-in landing page (same page as post-signin, no new
  page) (6c6ecd6); landing's autoplay loops resume on return (62578e6)

**Live Veo tests:** A1 exclusivity â Â· A2 square-clip aspect bounce â Â·
next: A3 free price-flips, then B4 (first paid run, â¦50) onward per the
...
- **Owner (test A3):** with a reference attached the duration snapped to 8s
  ...
  (rightly) never moved â billing was already fixed at 8s on both sides,
...
- **Fix:** with refs or an extend clip staged on Veo the Duration chips dim
  + go inert with a note ("Reference runs always render 8sâ¦" / "Extending
  always adds 7sâ¦"); refs snap the value to 8s, extend's summary chip reads
...
- **Owner:** the @Image1 chip bar auto-appeared in the composer the moment a
  reference was attached â "don't make it appearâ¦ only when I go @".
- Now it's mention-style: hidden by default; typing an @token in the prompt
  ...
  refs/elements/@Video1 as chips; clicking one REPLACES the partial "@imâ¦"
  ...
  thumbnails stay â they're how you know which number is which.

### fal balance pre-flight + honest queue status (2026-07-17)
- **Found live by the owner:** fal balance went NEGATIVE mid-testing; fal
  still ACCEPTED the image-to-video job and left it queued forever â the
...
- **Worker:** new falBalanceUSD() (official Platform API: GET
  ...
  submit now pre-flights it: balance < $0.50 â 503 "generations are briefly
  pausedâ¦ (you were not charged)" BEFORE any charge. Fails open on unknown
  ...
  nothing â verify while the balance is negative: a submit should bounce
...
- **Client:** friendlyFail maps the 503 to a clean no-charge message, and a
  job stuck IN_QUEUE >2.5min swaps the eternal "#0â¦" for an honest "Still
  queued on fal â unusually backed upâ¦ you'll see the exact error and get
  your credits back."
...
- Owner's wording call: the pre-flight refusal now reads "Our generation
  servers are temporarily down â we're working on it. Check back soon; you
...
- Full sweep of every user-facing failure message (catalog in the chat log).
  ...
  attempts â a paid render vanishing silently. finishDeadJob now resolves
  terminally: fal says COMPLETED â delivery retried each boot until it
  lands; otherwise â requestRefund (server re-verifies with fal) + a chat
  message with the refund amount, or an apology when fal actually ran it.
- Clarified: fal never pauses renders â "paused" was only ever our tab's
...
- Rewrote the six messages that named it (render no longer available /
  ...
  removed, standalone fal tokens â "the render service" (word-boundary
  safe â false/falcon survive).
- Known residual (not user-visible in the UI, only devtools): the network
  tab shows queue.fal.run inside /api/video/poll?url=â¦ params. Hiding that
  needs worker-side request-id mapping â flagged as optional hardening.
...
- **Owner:** some gallery items only render when the mouse passes over them.
  ...
  decode is forced â our hover-play was the force, so poster-less video
  cards sat blank until hover. Images were fine (thumbnails).
- Fix: on loadedmetadata, seek 1ms in (the standard Safari nudge) â forces
  ...
  the seek to 0 because route stubs can't serve byte ranges â real storage
  does (proved earlier with the audio-card seek against a range server).

## ð AUDIT LIST (owner-flagged, DO LATER â not yet built)
- ~~Attach pickers must ALL offer both sources~~ â â BUILT 2026-07-17 (see
  entry below). Original note: in video mode
  the pickers (Reference to video, Extend clip, First & last frame, image
  slot, characters/keyframesâ¦) go STRAIGHT to the device file dialog â no
  ...
  asks for both â check every single model." Sweep EVERY attach slot on
  EVERY model (13 video + 2 image + audio): each should open the source
  chooser (gallery + device; avatar where it makes sense; kind-filtered â
...
- The orchestrator's error step wrote "You've hit your balance limit on Fal
  â head to fal.ai/dashboard/billingâ¦" straight into a user chat. The
...
- Context of the failure itself: a submit was rejected for balance right
  after the top-up â if it recurs with balance present, check fal's
  dashboard for a separate SPENDING-LIMIT setting.

### Video references: director now sees (and uses) ALL of them (2026-07-17)
- **Owner's E11 test:** 3 refs attached on Veo Fast, open prompt â the
...
- **Fix (mirrors the image-mode default-to-all rule):**
  - Client: reference runs send EVERY ref (â¤9, downscaled) + imageCount.
  - Worker: imageCount accepted for video; new video multiImgLine â labels
    Image 1..N, LOOK at each, write ONE scene citing @Image1â¦@ImageN, USE
...
- **Owner (hit it live during Veo testing):** video-mode pickers went
  ...
  audio â every model) opens the source menu: isibi gallery + Your device,
...
- Kind-aware library: the clip slot lists gallery VIDEOS (video tiles with
  the first-frame nudge), audio slots list gallery AUDIO (âª tiles), image
...
- A gallery pick is fetched into a File and pushed through the SAME hidden-
  input change path as a device pick â so every existing validation (size
  caps, magic bytes, Veo exclusivity, clip thumbnail + fps conform, billing
  measurement) applies identically. Gallery-header â¤ Import stays a direct
  dialog on purpose.
...
- **Owner's screenshot:** a render failed and the chat said "Your account
  has run out of credits" â while the owner had â¦867. The raw failure was
...
- **Fix, two layers:**
  - chat.js: known platform-side errors (briefly paused / servers
    ...
    ("our render serversâ¦", never "your account").
  - worker.js error-step prompt: explicitly forbidden from claiming the
    ...
    says "not enough credits" â platform balance problems are OUR
...
- **Root cause of the failure itself (owner asked "I have $8 in fal, so
  ...
  after topping back up (fal-ai/fal issue #922 â "Account locked despite
...
- **Owner hit it live:** attached a clip for extend, orchestrator started
  "writing the prompt", they switched chats â and the flow died silently.
...
- **Fix:** the pipeline snapshots the origin chat's composer state (kind,
  ...
  approval card â switching back shows it priced and ready to run. This
...
- Verified headless both ways: switch-away mid-compose â review card
  persisted + renders on return, no generation fired, other chat clean;
  stay-put in Auto â generates immediately as before.

### Chatbox settings are authoritative â director's sound override removed (2026-07-17)
- **Owner rule (stated while planning the G14 test):** "the orchestrator has
  ...
  with exactly what the chatbox shows â sound toggle, duration, resolution,
  ratio â the director's words never silently change a setting (or a price).
- Sound was the ONE chatbox setting the director could override (an
  earlier design: "make it silent" in words â extras.sound=false â cheaper
...
- Instead the ASK step now tells the user: silent video = flip the Sound
  switch in the settings (and that it costs less) â then proceeds with the
  creative request. The director advises about settings; it never drives them.
- Everything else in extras stays director-drivable on purpose â negative
  prompt, Kling cfg/auto-cuts, Seedance bitrate, Ray controls, voice
  delivery â none of those have chatbox controls and all are price-neutral.
- Verified headless: compose returning sound:false â price quote unchanged
...
- **Owner's 4 AM tentacle saga:** Veo 422'd the "guy with tentacles falls"
  ...
  - Gap 1: the auto-reword (error step's fixedPrompt) only ran on SUBMIT
    failures â a rejection on the poll/result path (where content-filter
    ...
    approval card lands in the origin chat â "approve to try again".
    Applied on both the FAILED-status and 4xx-result paths, video+image,
    only when the error is content-filter-flavored.
  - Gap 2: the COMPOSER was writing filter-bait ("grotesqueâ¦ wet, slimyâ¦
    grafted"). The video prompt-writer now has a craft rule: strict content
    checkers reject whole renders on trigger words â phrase visceral/impact
    ideas neutrally or comedically.
  - The lesson: on an EXTEND, the checker also scans the SOURCE CLIP's
    frames. The tentacle-man clip itself is what kept tripping it â no
    wording passes. That's a provider-side hard block, not an app bug;
    the refunds fired correctly every time (3 Ã â¦132 back).
- Verified headless: 422-with-checker-detail on the result path â exact-
...
- **Owner's screenshot:** an approved extend prompt re-narrated the ENTIRE
  source clip (the tentacle grafting, the slide, the landing â all footage
  that already exists) and closed with "for the 8s clip" on a +7s extend.
  Result quality was "a little buggy" â re-describing makes the model try
  to REPLAY events that already happened, which is exactly what glitchy,
  morphing extensions look like.
- Cause: the worker had no extend writer â a Veo clip attach fell into the
...
- Kling o3 / Ray clip edits keep the edit writer; Seedance @Video1 keeps
  the reference writer â this only splits Veo extends out.

### Veo 3.1 Lite added (2026-07-17)
- Owner spotted fal's new budget tier and asked for it. Third Veo variant in
  the group flyout: "Veo 3.1 Lite Â· Google Â· cheapest Â· audio".
...
- Pricing (verified verbatim on fal's page): 720p $0.05/s sound Â· $0.03
  silent; 1080p $0.08/s Â· $0.05 silent. UNLIKE Standard/Fast, 1080p costs
  more than 720p â the resolution picker moves the price on this tier.
  Credits at 8s: â¦50 (720p) / â¦30 silent / â¦80 (1080p) / â¦50 silent.
- Verified headless: in the Veo flyout, all four price points, res list has
  ...
  when the owner wants: one â¦50 t2v).
...
- The 4:48 AM refusal investigation, concluded: the worker's FAL_KEY was
  API-scoped and could NOT read billing (probe returned null) â so the
  low-balance guard had never fired (fails open on null by design), and the
  502s were fal REJECTING submits at the door with money visible ($4.95) â
...
- Owner swept fal's Veo catalog (13 endpoints) against ours right after the
  ...
  exists (fal's docs search had hidden it). caps.flf flipped on â rows,
...
- Owner swept fal's Ray catalog (4 endpoints). t2v/i2v/v2v were wired;
  **Reframe** (generative outpaint to a NEW aspect ratio) was missing â now
  built with zero new UI: attach a clip on Ray, pick a DIFFERENT ratio in
  settings â the run routes to .../reframe; keep the clip's own ratio â
...
- Pricing (verified on fal's page; billed per started SOURCE second, 30s
  schema cap): 540p $0.06/s Â· 720p $0.12/s Â· 1080p $0.36/s â r2s tier in
...
- Director: reframe runs get a dedicated rule â describe ONLY what fills
...
- The owner's "keyframes not wired" report checked out fine headless: the
  ...
  â worker â fal i2v with spaced indexes (stale tab suspected).
- Verified headless end-to-end: native ratio â v2v â¦135; flip to 9:16 â
  reframe â¦45 (3s clip Â· 720p), duration locked, note shown, body carries
...
- Both tiers (1.0 + 1.5) pulled everywhere: model picker + group flyout,
  ...
  keeps the entire audio-length billing pipeline (byte-measured, â¤30s) â
...
- A user whose saved composer still points at OmniHuman degrades safely:
  no rows, no price, and a send answers "that model isn't available â pick
...
### Catalog sweeps: Seedance â 9/9 Â· Gemini has a missing endpoint (2026-07-17)
- Seedance: all 9 fal endpoints covered (t2v/i2v/reference Ã Std/Fast/Mini).
  ...
  slots) + @VideoN/@AudioN tags + billing basis for multiple clips â noted
...
- Gemini Omni Flash: t2v/i2v/edit wired; **reference-to-video MISSING**
  ...
  3-10s; pricing not on the API page â needs the model page check before
  wiring).
...
- The missing 4th Gemini endpoint (owner's catalog sweep). References row
  (â¤6 â fal documents no cap; ours) with @ImageN badges; the worker
  ...
  image refs ONLY (no video/audio params â verified 2026-07-17).
- Pricing: same ~$0.13/s def rate as Gemini t2v (verified on the model
  page) â the existing price entry covers it, â¦130 at 8s.
- Clip â refs are mutually exclusive on Gemini (its edit endpoint takes no
  refs, its ref endpoint no clip) â same both-directions clearing as Veo.
...
- Owner green-lit closing the capability gap from the catalog sweep. Slot #1
  ...
  attach: videos MP4/MOV, each ~480-720p area band, combined 2-15s, â¤50MB
  total; audios combined â¤15s, â¤15MB per file. Extras aren't auto-downscaled
  (slot #1 is) â out-of-band ones bounce with the fix instructions.
- Chips: typing @ offers @Video1..N and (new) @Audio1..N; tags reconcile in
  ...
  every reference.
- Billing: the 0.6Ã (input + output) seconds basis now uses the COMBINED
  byte-measured input seconds, server-side; ANY unmeasurable clip â the 15s
...
- Verified headless end-to-end (real files through the real file inputs);
  ...
  hit webm metadata flakiness â worth one live poke when convenient.
...
- What went with it (noted before removal, owner confirmed anyway): the
  ...
  keyframes (â¤64), HDR/EXR, per-signal edit controls.
- Ray-only machinery (keyframes row + lists, HDR/loop pickers, edit dial,
  reframe helpers, worker isRay routing/billing) left DORMANT in code â
  unreachable behind the allowlist / caps, flagged with comments â so a
...
- **New standard (owner's rule: "check fal first, detail by detail"):**
  ...
  (fal.ai/api/openapi/queue/openapi.json?endpoint_id=â¦) â the same source
  their live validation runs on â never just the rendered docs page.
- Machine-schema audit of everything shipped today:
  - Lite t2v: duration enum 4s/6s/8s â (all free)
  - Lite i2v: duration enum 4s/6s/8s â (all free â first fix over-locked
    it; corrected)
  - Lite first-&-last: duration CONST "8s" â picker locks to 8s (the veo
    ref-lock mechanism, note included), worker forces "8s" and bills 8.
  - Gemini reference-to-video: image_urls REQUIRED, maxItems 10 â our cap
    raised 6â10 (was a guess); prompt cap 20k matches.
...
- Verified headless: Lite t2v/i2v free at 4s (â¦25), flf locked (â¦50, note,
  snap), Gemini cap 10.

### Full machine-schema audit: all 33 endpoints, one billing fix (2026-07-17)
- Following the Lite flf lesson, every wired endpoint (Veo Std/Fast/Lite Ã
  their sub-endpoints, Seedance Ã6, Kling Ã12, Gemini Ã3, LipSync Ã2) was
...
- FIXED â extend overbilling at 4k: Veo extend-video (Std + Fast) has
  resolution CONST 720p (output is always 720p; we never sent resolution â
  correct) but billing used the PICKED tier: 4k selected + extend charged
  â¦525 for a $2.80 render. Both quote and charge now use the 720p tier on
...
- Owner sent a reference screenshot: per-variant rows with the provider
  ...
  tier pill â PREMIUM (purple) / CLASSIC (blue) / BASIC (teal), check on
...
- Owner's follow-up references: the Veo rows now use Google's official
  ...
  the same tier treatment â Standard PREMIUM 4K Â· Fast CLASSIC 720p Â·
...
- Active-variant chips removed from all three grouped rows (Veo/Seedance/
  Kling) â owner's call; the flyout â carries the selection.
- LATER (owner: "i will do kling and gemini later"): tier badges for the
  Kling five and Gemini â waiting on the owner's tier labels; one-line
...
### LATER â Seedance price re-verification (owner, 2026-07-17)
- fal's Seedance endpoint pages don't show cost cards, so today's money
  ...
  stored rates (per second, no audio discount): Std 480p $0.14 Â· 720p
  $0.304 Â· 1080p $0.682 Â· 4k $1.59; Fast $0.135/$0.242; Mini
  $0.0725/$0.155; video-ref runs 0.6Ã rate Ã (combined input + output
  seconds). These predate the machine-schema standard â when fal surfaces
...
- Also open from the Veo sweep: whether Fast REFERENCE at 4k bills the 4k
  ...
  charge the 4k tier â house-safe; one live 4k Fast ref settles it.

### Veo money audit COMPLETE â owner-verified (2026-07-17)
- The owner walked every fal cost card across all 13 Veo endpoints
  (Std/Fast/Lite Ã t2v, i2v, first-&-last, reference, extend where they
  ...
  include 5s â schema remains the only trustworthy source.
...
The owner will verify these on the frontend later â every combo the app
should quote AND charge:

Veo 3.1 (Standard) â t2v / i2v / first-&-last (4s Â· 6s Â· 8s):
  720p/1080p sound  â¦200 Â· â¦300 Â· â¦400
  720p/1080p silent â¦100 Â· â¦150 Â· â¦200
  4K sound          â¦300 Â· â¦450 Â· â¦600
  4K silent         â¦150 Â· â¦225 Â· â¦300
  Reference (8s lock): â¦400 / â¦200 Â· 4K â¦600 / â¦400
  Extend (+7s, 720p pin): â¦350 / â¦175

Veo 3.1 Fast â t2v / i2v / first-&-last (4s Â· 6s Â· 8s):
  720p/1080p sound  â¦75 Â· â¦113 Â· â¦150
  720p/1080p silent â¦50 Â· â¦75  Â· â¦100
  4K sound          â¦175 Â· â¦263 Â· â¦350
  4K silent         â¦150 Â· â¦225 Â· â¦300
  Reference (8s lock): â¦150 / â¦100 Â· 4K â¦350 / â¦300 (tier-vs-flat open)
  Extend (+7s, 720p pin): â¦132 / â¦88

Veo 3.1 Lite â t2v / i2v (4s Â· 6s Â· 8s):
  720p sound  â¦25 Â· â¦38 Â· â¦50     720p silent  â¦15 Â· â¦23 Â· â¦30
  1080p sound â¦40 Â· â¦60 Â· â¦80     1080p silent â¦25 Â· â¦38 Â· â¦50
  First-&-last (8s lock): 720p â¦50 / â¦30 Â· 1080p â¦80 / â¦50
  (No reference / extend / 4K on Lite.)

Fractional-looking cells are correct ceilings (â¦113 = $0.90, â¦263 =
$2.10, â¦38 = $0.30, â¦23 = $0.18) â rounding is always up.
...
- Owner (after seeing fal's Seedance form): ONE "Image to video" row with
  the start slot + an optional "End frame Â· optional" slot, replacing the
  separate First-&-last row â applied across every video model that had
  both (Veo Ã3, Seedance Ã3, Kling Ã4). Row header counts n/2.
- Zero rewiring risk by design: the slots are a re-skin over the proven
  ...
  imageâffirst (+flast); removing the end demotes back to a plain start;
...
- The rowFlf DOM stays (hidden) â no model shows it anymore.

### Seedance: one combined "Reference to video n/12" row (owner's design, 2026-07-17)
- Owner: instead of three separate rows (Audio 0/3 Â· Video clip 0/3 Â·
  Reference 0/9), Seedance now shows ONE "Reference to video" row counting
  n/12 (fal's cross-modal total), with three labeled groups inside â
...
- Zero-rewire implementation: the existing controls (clip slot, @Video2-3
  ...
  Seedance model is selected, and moved back for every other model â same
  ids, handlers, validators; LipSync's rows verified intact round-trip.
- NEW enforcement fal always had but our split rows never checked: â¤12
  files TOTAL across modalities (9+3+3=15 possible) â attach-time toast
...
### Reference row groups â 3-icon modality toggles (owner refined, 2026-07-17)
...
- Relocating the audio/clip controls out of their rows detached them from
  their #rowAudio/#rowClip-scoped CSS â the empty audio slot's waveform
...
- The fal limits used to surface only as bounce toasts on violation; each
  reference tab now shows its caps line permanently: images "up to 9 Â·
  @Image1â¦ Â· 12 files max total", videos "up to 3 Â· MP4/MOV Â· 2-15s
  combined Â· near 480-720p", audio "up to 3 Â· MP3/WAV Â· 15s combined Â·
...
- The just-added per-tab caps hint was removed same-day (owner: "don't put
  the cap there â whenever it exceeds, just reject it and tell the user
...
### OVERNIGHT PLATFORM AUDIT (2026-07-17, owner asleep â findings only, nothing fixed)
...
1. Landing filmstrip 404 spam â /mkt/f1â¦f14.jpg don't exist, so every
...
2. CLAUDE.md drift â it still describes the floating logo menu
...
3. (Standing, already noted) Seedance Fast/Mini + reference price cards
   unverified; Fast-reference-at-4k tier question; 0.6Ã video-ref billing
...
- Every durationÃresolutionÃsound price combo on all 12 video models
...
- fal admin-key note: the key lives in Worker secrets (not readable from
  the dev box) â deep checks ran against fal's public machine-schema API;
  the owner-only /api/fal-balance probe remains the live-balance window.

### DEEP MULTI-AGENT AUDIT (2026-07-17 overnight, findings only â NOTHING fixed)
159 agents Â· 10 dimensions Â· every finding double-verified by adversarial skeptics.
74 raw â 61 CONFIRMED (survived both skeptics) Â· 9 plausible Â· 4 refuted.
Breakdown: 9 high Â· 19 medium Â· 33 low. Full evidence per finding below.


#### HIGH (9)
- **worker.js:2244** <billing-parity> â Veo extend with the Sound toggle off is billed at the audio-off (aoff) discount, but generate_audio:false is never sent to the extend endpoint (bareEdit excludes it) â the render is produced with audio at fal's audio-on price while the user is charged roughly half of it.
  - evidence: worker.js:2244 only applies the silent flag when !bareEdit: `else if (soundOff && !bareEdit && (isSeedance || isKlingV3 || isVeo)) input.generate_audio = false;` â Veo extend sets bareEdit=true (worker.js:2050-2054), so the request never asks for a silent render. Yet worker.js:2460 passes `soundOff` to creditCost unconditionally, and creditCost (worker.js:227) then picks the aoff tier: Veo extend bills aoff 720p $0.20/s x 7s = $1.40 (175 credits) instead of the audio-on $0.40/s x 7s = $2.80 (350 credits) the render actually costs. Same for fal-ai/veo3.1/fast ($0.10 vs $0.15/s). The client quot
- **public/chat.js:304** <state-machine> â Merged Image-to-video row: the image<->ffirst promote/demote conversion is dead code, so attaching the End frame silently deletes the start image and strands an invisible `flast` that skews pricing and rides the send as a start-less `last` frame.
  - evidence: Image kinds take the readImageConformed path and `return` at line 307, so the conversion written for them at lines 338-352 (`kind === 'flast' && mergedFlf() && !attachments.ffirst && attachments.image` -> promote image to ffirst) sits inside the clip/audio-only `reader.onload` (line 309, gated by `kind === 'clip' || kind === 'audio'` at 287) and can never run. The live flast path instead hits line 304 `clearImageInputsExcept('flf')`, which nulls `attachments.image` (line 1585). Repro on any merged model (Veo 3.1 x3, Seedance x3, Kling v3/o3 â all have `caps.image`+`caps.flf`, lines 33/40/67/74
- **worker.js:2814** <error-paths> â Stripe webhook swallows set_plan failure: a paid membership's storage tier/plan silently never activates, and Stripe is told the delivery succeeded so it never retries
  - evidence: In the invoice.paid handler, add_credits failure correctly returns 500 so Stripe retries (line 2799), but the set_plan call (lines 2804-2813) is wrapped in `try {...} catch {} // credits already granted; a plan-set hiccup shouldn't fail the webhook` and the handler then returns `Response.json({ received: true })` (200). If set_plan fails (Supabase blip, 10s timeout), the user has just paid $24.99-$99.99 for a membership whose gallery-storage benefit (user_plan tier + plan_until) is never recorded â /api/storage returns cap 0, every save 402s with reason 'free', and the UI tells this paying cus
- **public/chat.js:7314** <error-paths> â Avatar generation poll loop never checks for terminal fal failure and never refunds: a FAILED job leaves the user staring at a spinner for the full 5 minutes, then shows 'Timed out' with the credit silently kept
  - evidence: acGenerate's poll loop (lines 7314-7318) is `while (Date.now() < deadline) { ... if (state === 'COMPLETED') break; await sleep(3500) }` â unlike the main flow (pollAndDeliver line 5260, which breaks on FAILED/ERROR/CANCELED, fetches the exact error, and calls requestRefund at 5272) and even the voice preview (line 2634), there is no FAILED/ERROR/CANCELED check. A fal-side failure spins the 'Creating your avatarâ¦ this takes a few seconds' stage for 5 full minutes, then fails as 'Timed out â please try again' (7319) with no reason. requestRefund is never called anywhere in acGenerate (grep confi
- **public/chat.js:7327** <error-paths> â Avatar creation silently persists an expiring temporary fal URL when the gallery save fails or is blocked â the avatar (and its cross-device sync copy) rots dead within days with no warning
  - evidence: `try { const saved = await saveOutput(url, 'image'); if (saved && saved.url) finalUrl = saved.url; } catch (e) {}` then the avatar is persisted with `image: finalUrl` (7332) â on any save failure or 402 block, finalUrl stays the temporary fal URL. For EVERY free account this is the guaranteed path (cap 0 â 402 reason 'free', trySave returns {url:null, block:'free'}), yet unlike the chat flow â which explicitly delivers 'â¹ï¸ Saving to your gallery is a paid feature â this one is a temporary link' (line 5415) â the avatar flow says nothing. Worse, saveAvatars â touchAssets â pushAssets syncs the 
- **worker.js:3587** <director-prompts> â The user-facing ask step is handed the raw fal-branded model id ('target model: fal-ai/...') with no provider-nondisclosure instruction - only the error step forbids naming fal - so a user asking 'which model will you use?' can get 'fal-ai/kling-video/...' streamed back verbatim, violating the never-show-fal rule.
...
- **worker.js:2159** <director-prompts> â Tag-protocol mismatch on Kling o3 clip edits with style refs: refLine tells the director to cite @Image1..@Image4 tags, but the reconciler only counts image tags on reference-to-video endpoints, so on the /video-to-video/edit endpoint every @ImageN is stripped - producing a mangled paid edit instruction with the uploaded refs left uncited.
...
- **public/chat.js:6082** <ux-deadends> â Plan-mode review card's 'Generate â¦N' price is frozen at card-build time and is never re-quoted when the user changes model/duration/resolution/sound afterwards â and on re-render it prices with the CURRENT composer mode instead of the card's own mode, so the quote can be wildly wrong while approval charges the real (different) amount.
  - evidence: buildReviewCard sets the price once: `allow.textContent = 'Generate ' + (estimatePrice(m === 'audio' ? prompt : undefined, shots, extras && extras.sound) || 'â¦')` (chat.js:6082). estimatePrice branches on the GLOBAL `mode`/`model`/`duration`/`quality` (chat.js:4480-4572), and nothing repaints the thread on a settings change â setMode (chat.js:2227-2241) and every Settings pick call only buildOptMenus/updateSendPrice, never renderThread (renderThread is only called on chat switch/boot/sync, chat.js:3374). Reachable path A (stale quote): Plan mode â send request â card shows e.g. 'Generate â¦63' 
- **public/chat.js:5371** <refresh-resume> â Refresh during the save/deliver phase silently strands a completed, charged render for up to 1 hour: the delivery claim is taken by the now-dead tab, and the boot-resume that hits the claimed key bumps tries, pauses with autoResume=false, and shows NO message.
  - evidence: pollAndDeliver claims delivery (line 5371 claimDelivery(statusUrl)) BEFORE the save phase â which can run for minutes (saveOutput retries, burnImageWatermark, saveVideoWithQr's ffmpeg burn, lines 5379-5411) â and only clears the record at endGen (5413). If the tab dies in that window, the claim (keyed by TAB_ID, blocking for 3600e3 ms per claimAt check at line 4065) belongs to a dead tab. On the next boot, resumeOne re-polls, sees COMPLETED, fetches the result, then line 5371 returns false â `jobBumpTries(origin); pauseGen(origin, false); return;` â no deliverAgent call, no scheduleResume (aut

#### MEDIUM (19)
- **public/chat.js:4550** <billing-parity> â Gemini clip-edit quote uses the browser-measured clip duration, but the worker can only byte-measure mp4/mov â any other container (e.g. webm) makes clipSecondsReal 0 and bills the 30s maximum, so the user sees a quote for the real length and is charged up to ~6x more.
  - evidence: Client quote (chat.js:4548-4550): `clipEditSecs = Math.min(clipEditMax, Math.ceil((clipMeta && clipMeta.dur) || clipEditMax))` â browsers decode webm fine, so a 5s webm quotes 0.13*5 = $0.65 (82 credits). Worker: videoDurationFromDataUri (worker.js:270-279) parses only mp4/mov moov/mvhd, so a webm clip yields clipSecondsReal=0 and worker.js:2418 bills the max: `Math.min(clipEditMax, Math.ceil(clipSecondsReal || clipEditMax))` = 30s -> 0.13*30 = $3.90 (488 credits). CLIP_LIMITS has no format restriction for Gemini (chat.js:442, only maxDur:30; the o3 edit is protected by its mp4/mov formats lis
- **public/chat.js:4978** <provider-leak> â Error-step output is only half-scrubbed: data.reply goes through scrubProvider but data.prompt (fixedPrompt, written by Claude from the RAW fal error) is rendered into review cards unscrubbed â in explainFailure and offerReword.
...
- **public/chat.js:8420** <dom-consistency> â Media Agent chat (agent Q&A) is fully orphaned: agentRenderThread targets #maThread, which no HTML or JS template creates, and agentSend is only reachable from buttons agentRenderThread itself renders â the whole feature (AGENT_SUGGESTIONS, /api/social/agent call) is unreachable dead code.
  - evidence: chat.js:8420 `const thread = document.getElementById('maThread'); if (!thread) return;` â grep for maThread across public/ finds a producer only in the unused demo folder public/demo-hero-2/chat.js:6109, never in public/index.html or public/chat.js templates. renderMediaAgent (chat.js:7393) builds only #appSwitch/#appMain, and renderSection (chat.js:7473+) routes to analytics/posts/dms/comments/autoreply with no agent-chat section. agentRenderThread is called only from agentSend (8446, 8461), and agentSend only from the .ma-suggest buttons agentRenderThread renders (8428) â a closed loop with 
- **public/chat.js:670** <state-machine> â awDecode has no swap/identity guard (unlike readClipMeta line 372 and measureAttachedImage line 592): a slow decode of a replaced or cleared audio clip stamps the OLD clip's awDur/awPeaks onto current state, which can wrongly auto-reject the newly attached valid audio and mis-quote lip-sync pricing.
  - evidence: awDecode (lines 654-686) unconditionally sets `awPeaks`/`awDur = audio.duration` (669-670) and then runs `audioIssue()` (678) with no check that `attachments.audio` still equals the dataUrl it was decoding â the exact guard readClipMeta uses (`attachments.clip !== dataUri`, line 372). Race: attach clip A (e.g. 70s, over Kling LipSync's 60s cap), immediately re-pick valid clip B; onAttach resets awDur=0/awDecoding=true (line 316, whose comment states the invariant: 'a send in this window must not bill the old length') and starts decode B, but decode A (decodeAudioData of a large mp3 takes secon
- **public/chat.js:5427** <state-machine> â Post-delivery 'inputs were consumed' cleanup clears attachments/extraImages/refList/elList but omits kfList, vxList and axList (and leaves clipMeta set), so Ray keyframes silently ride the NEXT prompt at the i2v price tier, and Seedance leaves orphaned @Video2-3/@Audio2-3 entries whose debounced persist immediately resurrects the state the code just deleted from IndexedDB.
  - evidence: Lines 5426-5436: only `attachments` keys, `extraImages`, `refList`, `elList` are cleared. Leftovers: (a) kfList â next send in the same chat posts `keyframes: kfList.slice()` (5097) and prices as startImg (4523) even though the comment at 5421 says inputs must not 'ride the next prompt'; (b) vxList/axList survive with slot #1 (`attachments.clip`/`attachments.audio`) now null â an impossible state the attach flow prevents (clearAttach lines 874-875 drops extras with slot #1): renderVxList still paints tagged @Video2-3 slots with no @Video1, srTotal (1722-1725) counts them against the 12-file ca
- **public/chat.js:295** <state-machine> â Async image-attach callbacks are neither slot- nor chat-scoped: a slow readImageConformed resolving after a chat switch writes the old chat's image into the NEW chat's staged attachments (cross-chat leak into the next send), and on quick same-slot re-picks the last-RESOLVED file wins over the last-attached one.
  - evidence: onAttach's `.then` (293-306) does `attachments[kind] = uri` with no guard; readImageConformed is multi-second for oversized files (>8MB triggers Image decode + iterative canvas re-encodes, lines 261-275 â the code's own comment says 15-25MB PNGs from the app's 4K outputs are the normal case). switchChat (3634-3648) is freely clickable meanwhile and runs restoreStaged for the new chat â the late resolve then lands the previous chat's image into the live `attachments` of the new chat, gets picked up by the debounced stashStaged under the new chat's id, and rides its next generation. The list att
- **public/chat.js:4280** <error-paths> â retryPendingSaves silently drops queued gallery saves after the chat explicitly promised 'It'll land there on its own' â the media stays on a temp URL that expires, with no follow-up message
  - evidence: When a mid-session save fails transiently, the user is told 'â³ Still saving this to your gallery â big files can take a minute. It'll land there on its own.' (5418). But in retryPendingSaves: line 4276 `if (Date.now() - (p.at || 0) > 6 * 24 * 3600e3) continue;` silently discards saves older than 6 days, and line 4280 `else if (block) { /* paid gate (free/full) â retrying won't help, drop it */ }` silently discards a save that later hits the 402 gate (plan lapsed or cap filled between generation and retry). In both cases the chat message keeps its temporary fal URL, which expires â a paid rende
- **public/chat.js:9468** <error-paths> â galleryDelete removes the card from the UI before the server operation and surfaces nothing on failure â both the unlist path (catch {}) and the hard-delete path (Auth.storageDelete result ignored) fail silently
  - evidence: Line 9446 `el.remove()` runs before any network call. Referenced path: the /api/media/unlist POST is in `try {...} catch {}` (9448-9468) and a non-ok response leaves j null so nothing happens â the code's own comment admits 'the card comes back on the next gallery load', but the user is shown a successful delete now and a resurrected card later with no explanation; sbToast (9172) exists and is unused here. Unreferenced path (9471): `try { await Auth.storageDelete(m[1]); } catch {}` â auth.js:182-190 shows storageDelete returns res.ok, and this boolean is ignored, so a failed DELETE (expired to
- **public/chat.js:5575** <error-paths> â pushAssets (avatar cross-device sync) never checks the response and never requeues on failure â a 4xx (expired token, RLS) or network error silently loses the sync, unlike pushChats which requeues both
  - evidence: pushAssets wraps its upsert in `try { await fetch(ASSETS_ENDPOINT...) } catch {}` (5569-5580) with no `res.ok` check and no retry/requeue â contrast pushChats, which on !up.ok re-adds ids to syncDirty and calls scheduleSync (3234) and requeues on network error (3244-3248). A single failed push means avatars edited on device A never reach device B until the NEXT avatar edit happens to fire touchAssets; worse, the LWW timestamp (assetsAt, already bumped at 5543) makes device A also ignore the stale server row on future pulls (5597), so the divergence is permanent and invisible. pushMemory (5504-
- **worker.js:2445** <worker-routes> â GPT Image 2 2K/4K billing guard for 'auto' ratio is dead code â a request with ratio absent or 'auto' plus size '4K' is billed the 4K tier ($0.41) while no explicit dimensions are ever sent, so fal renders/bills the ~1K default (~$0.22): a ~2x overcharge the code explicitly says must not happen.
  - evidence: Line 2445: `gptSize: gptSize && ratio === "auto" ? "1K" : gptSize` â but `ratio` (lines 1875-1878) is validated by `/^\d{1,2}:\d{1,2}$/` and is otherwise null, so it can NEVER equal the string "auto"; body.ratio='auto' yields ratio=null. With ratio null, line 2357-2359 `gptSizePx(ratio, gptSize)` returns null (no image_size sent) and the generic ratio branch at 2298 is skipped, yet the un-downgraded gptSize ('2K'/'4K') still reaches creditCost â GPT_PRICE['4K'] (line 109-115). The comment at 2443-2444 states "At 'auto' ratio there are no explicit dimensions â 2K/4K can't apply, so they must no
- **worker.js:4276** <worker-routes> â /api/import/fetch AI-rescue path ignores useCredits' -1 insufficient-balance return: on a race the â¦3 lookup runs without ever debiting, every failure path then mints +3 credits the user never paid via creditBack, and on success the client pill is sent balance:-1.
  - evidence: Lines 4275-4277: `let newBalance = null; try { newBalance = await useCredits(auth, AI_CR); } catch {...}` â only the throw (ledger down) is handled. Per the function contract (worker.js:384-385, "Returns the new balance, or -1 when the balance is too low"), a concurrent spend between the readCredits pre-check (4272-4274) and this debit returns -1, which is never checked. Contrast the generation route, which checks `if (!(balanceAfter >= 0))` and cancels (4524-4527). Consequences: (a) the paid Sonnet+web_search lookup runs with no charge; (b) `refund()` (4278) / `creditBack(env, user.id, 3)` (4
- **worker.js:4394** <worker-routes> â /api/save â the route that legitimately receives the largest client payloads (up to ~40MB base64 video) â has no tooLargeBody backstop: request.json() buffers and parses an arbitrarily large body before any size check runs.
  - evidence: Lines 4390-4396: `if (url.pathname === "/api/save" ...) { const user = await authUser(request); ... body = await request.json(); }` with no Content-Length guard; the per-kind caps (b64.length > 40_000_000 at 4414, 20M at 4430, 12M at 4448) apply only AFTER the whole body is buffered and JSON-parsed. Every other body-heavy route has the backstop: generation 100MB (line 1696), /api/direct 60MB (line 3318), stripe webhook 256KB (line 2713) â tooLargeBody (line 804) returns a clean 413 by Content-Length. An authed user posting a near-plan-limit body (Workers allows 100-500MB) forces full buffering
- **worker.js:3746** <director-prompts> â multiImgLine (the multi-reference image guidance including the useImages selection protocol) is spliced only into the image-EDIT compose branch, where it can never render because the client makes an edit base and reference images mutually exclusive - and its text even contradicts that branch's own header ('there is no edit base' inside 'this is an EDIT'). The actual multi-reference image compose branch gets no multi-image guidance at all, and the video variant of multiImgLine is used in no template.
...
- **public/chat.js:6103** <ux-deadends> â Approving a multi-shot Plan card after switching to a non-Kling video model silently drops the whole shot list â the user approves a numbered shot breakdown but gets a single plain render with no notice that the plan changed.
  - evidence: The card renders the shot list (chat.js:6066-6076, gated by `shotsApply(model)` evaluated at BUILD time, chat.js:6052) and its approve handler passes the captured shots to `generateMedia(prompt, { announce:false, shots, extras })` (chat.js:6103). generateMedia re-gates with the CURRENT model: `const genShots = (kind === 'video' && shotsApply(model) && sanitizeShots(opts.shots)) || null` (chat.js:5049) â shotsApply (chat.js:5702) is Kling-only. Reachable path: Plan mode on Kling o3 â ask for a montage â card shows 'Here's the shot list â approve to run it' with N shots â open the model menu, pi
- **public/chat.js:9255** <ux-deadends> â Gallery import-from-link with a non-URL string is a silent no-op: the only feedback is written to the input's placeholder, which is invisible while the typed text is still in the box.
  - evidence: importGalleryUrl: `if (!m) { if (inp) inp.placeholder = 'That needs a full link (https://â¦)'; return; }` (chat.js:9254-9255) â the input's value is not cleared, and an HTML placeholder only renders when the field is empty, so the message never shows. Reachable path: floating logo menu â Gallery â paste/type anything without 'https://' (e.g. 'molly's suds detergent' or 'www.example.com/photo.jpg' â the regex at chat.js:9254 requires the scheme) into #galImportUrl â press Enter (KEYDOWN_ACTIONS 'gal-import-url', chat.js:9620) or click the 'â â¦3' button â the function returns before disabling any
- **public/chat.js:4300** <refresh-resume> â finishDeadJob's 'keep retrying DELIVERY each boot (tries pinned below the cap)' is false: resumeJobs deletes dead records from localStorage (jobsWrite(live)) before finishDeadJob runs, and resumeOne({...j, tries:3}) never re-persists the record â so the final delivery attempt is in-memory only and any interruption loses the completed paid render permanently and silently.
  - evidence: Line 4298-4300: `const dead = jobs.filter((j) => (j.tries || 0) >= 4); jobsWrite(live);` â dead records are removed from storage. finishDeadJob's COMPLETED branch (line 4318) then calls `resumeOne({ ...j, tries: 3 })` with the comment 'keep retrying DELIVERY each boot â¦ until the file actually lands', but nothing writes the record back (jobRecord is only called in generateMedia and recoverJob). If that attempt fails â network pause path calls jobBumpTries/pauseGenâscheduleResume, both of which operate on jobsLoad() where the record no longer exists (scheduleResume's find at 4384 returns nothin
- **public/chat.js:4385** <refresh-resume> â scheduleResume requires rec.statusUrl, so a provisional (idem-only) record from a reply lost mid-submit is never recovered in-session â the user is told 'checking whether that render went throughâ¦' but no check runs until the next full page reload or a manual re-send.
  - evidence: generateMedia's catch (line 5198) delivers 'â ï¸ Connection dropped â checking whether that render went throughâ¦' and calls pauseGen(origin), whose autoResume path calls scheduleResume (line 4031). But scheduleResume (line 4385) is `if (rec && rec.statusUrl) resumeOne(rec);` â the provisional record written at line 5080 has an idem and NO statusUrl, so the timer fires and does nothing. resumeOne itself handles the idem-only case correctly (line 4341 â recoverJob), so the filter is the only blocker. The maybe-charged job (worker charges after fal accepts) sits unrecovered â and its loader gone â 
- **public/chat.js:5034** <refresh-resume> â A record whose tries reach 4 mid-session (via the 45s scheduleResume cycle) stalls silently after the app promised auto-pickup, and a new send in that chat then overwrites the dead record (the pending check filters tries<4), so the charged render never reaches finishDeadJob's refund/notice at next boot.
  - evidence: Each failed in-session resume bumps tries (e.g. lines 5234, 5299, 5446) and re-schedules; after the 4th failure scheduleResume's `(j.tries || 0) < 4` filter (line 4384) finds nothing, so the render â whose last user-visible message was 'the app will pick it back up automatically' â silently stops being retried until a reload triggers finishDeadJob. If instead the user sends a new generation in that chat first, generateMedia's guard `jobsLoad().find((j) => j.chatId === origin && (j.statusUrl || j.idem) && (j.tries || 0) < 4)` (line 5034) skips the dead record and jobRecord at line 5080 (which r

#### LOW (33)
- **public/chat.js:4501** <billing-parity> â Audio (TTS) quote caps the billed character count at 2,000 but the worker charges up to 4,000 characters â a director-composed voice script between 2,000-4,000 chars (plan-mode approval bypasses the 2,000-char send guard) is quoted at up to half what is charged.
  - evidence: Client quote (chat.js:4501): `const chars = Math.min(2000, raw.trim().length)` with the comment 'Match the server's cap', and the send() guard (chat.js:6147-6152) claims 'Voice is capped at 2,000 characters server-side'. But the worker slices the prompt at 4000 (worker.js:1706), sends the full text to ElevenLabs with no 2000 cap (worker.js:1909 `input.text = prompt` â only the LipSync text mode slices at 2000, worker.js:1942), and charges `chars: prompt.length` (worker.js:2440), i.e. up to 4000 chars. The plan-mode review card prices via estimatePrice(prompt) (chat.js:6082) and its approve han
- **worker.js:2445** <billing-parity> â The GPT Image 2 'auto'-ratio billing demotion is dead code: ratio is regex-normalized to null before the check, so `ratio === "auto"` can never be true â a request with ratio 'auto' plus size 2K/4K is billed the 2K/4K tier while no dimensions are sent and the render comes out 1K-class (4K high: 52 credits charged for a ~$0.21 render).
  - evidence: worker.js:1875-1878 normalizes ratio with `/^\d{1,2}:\d{1,2}$/` â 'auto' fails and ratio becomes null. worker.js:2445 then evaluates `gptSize: gptSize && ratio === "auto" ? "1K" : gptSize` â never true, so gptSize stays '4K'; meanwhile gptSizePx(null, ...) (worker.js:2358, regex at 120-121) returns null, so no explicit width/height is sent and the image renders at the schema-default 1K class. Charge: GPT_PRICE 4K high $0.41 = 52 credits vs real fal cost ~$0.211 and vs the client quote which prices 'auto' as 1K (chat.js:4485: `GPT_PRICE[ratio === 'auto' ? '1K' : gptSize]` = 29 credits). Current
- **public/chat.js:5919** <provider-leak> â Director replies outside the error step bypass scrubProvider: the ask-step reply (deliverAgent(origin, res.reply)), its live-streamed deltas, and the Media Agent reply are all rendered unscrubbed, despite the 4974 comment framing scrubbing as defense-in-depth for AI output.
...
- **public/chat.js:8308** <dom-consistency> â renderPublish is dead code: its only call site is inside itself (the platform-tab click it wires), so it can never run â the live YouTube/Instagram composers (openYtComposer/openPostComposer) build the #maPublish markup inline and duplicate it.
...
- **public/styles.css:2052** <dom-consistency> â `.sb-toast` is defined three times with conflicting rules: a #sbToasts-scoped variant using .in/.prog modifiers that no JS ever creates or toggles (styles.css:1715-1728), and two competing fixed-position .show toasts (2052-2061 vs 2869-2877, z-index 120 vs 400) where only the last block effectively wins for the live sbToast().
  - evidence: styles.css:1715 `#sbToasts { position: fixed; ... }` â 'sbToasts' appears nowhere in chat.js or index.html, and no JS adds class 'in' or 'prog' to a toast (grep confirms). styles.css:2052 and 2869 both declare `.sb-toast { position: fixed; left: 50%; bottom: ...; opacity: 0 }` + `.sb-toast.show` with different z-index/background/padding; the only creator is sbToast() (chat.js:9172-9184, className 'sb-toast', toggles 'show'), so the 2052 block is shadowed dead weight â edits to it silently do nothing.
- **public/chat.js:3510** <state-machine> â awPlayer is not invalidated on chat switch: restoreStaged swaps awPeaks/awName/awDur to the new chat's staged audio but leaves awPlayer bound to the previous chat's clip, so the play button plays the WRONG chat's audio (and a clip playing at switch time keeps playing over the new chat).
  - evidence: restoreStaged (3501-3518) restores awDur/awPeaks/awName/awSize/awType (3510-3511) and re-renders via renderAttach('audio') (3514), but renderAudioSlot only pauses/nulls awPlayer in its EMPTY branch (762) â when the incoming chat has its own staged audio, the 'has' branch (749-758) keeps the stale player. awToggle (732-746) then sees `awPlayer` non-null and calls `awPlayer.play()` on the previous chat's Audio element while the slot shows the current chat's filename, duration, and waveform. Repro: stage audio X in chat A, press play (or just play/pause once), switch to chat B which has audio Y s
- **public/chat.js:3679** <state-machine> â deleteChat of the active chat calls restoreStaged on the fallback chat but never hydrateStaged, so that chat's refresh-persisted staged attachments don't appear â and the empty in-memory stash created on the next switch-out then deletes the IndexedDB record for good.
  - evidence: Line 3677-3680: `if (wasActive) { applyComposerState(...); restoreStaged(chatStore.active); }` â no hydrateStaged, unlike switchChat (3644-3645) and boot (6560). If the fallback chat had staged inputs persisted before a refresh and hasn't been visited this session (no stagedByChat entry), restoreStaged renders an empty panel over a DB record that still holds them. Worse, the next switchChat AWAY from it runs stashStaged (3638, creating an empty in-memory snapshot) and `stagedDbPut(outId, null)` (3640, since stagedHasContent is false), permanently destroying the persisted stash; hydrateStaged o
- **public/chat.js:9053** <error-paths> â Gallery server-list fetch failure is indistinguishable from an empty gallery: a fresh device with /api/gallery down shows 'Nothing here yet â everything you generate lands in your gallery' to a paying user whose media exists
  - evidence: loadServerGallery (9053-9061) swallows both non-ok responses and thrown errors (`catch {}`), leaving serverGallery null. galleryItems then falls back to the chat-derived local view (9087) â reasonable on the originating device, but on a new device/browser (no local chats yet, pullChats may also have failed silently at 3271) the grid renders empty and renderGallery shows the definitive-sounding empty state at 9323 ('Nothing here yetâ¦') with no 'couldn't load your gallery â retry' distinction. sbToast exists and could disambiguate. The same-device fallback behavior itself is a deliberate, commen
- **public/chat.js:2641** <error-paths> â Voice preview failure surfaces only as a bare 'â ' glyph for 1.6 seconds â no reason, no toast, and the spent TTS credit is never refund-requested even when fal reports FAILED
  - evidence: previewVoice's catch (2641-2643) sets `btn.textContent = 'â '` then restores 'â¶' after 1600ms â that is the entire failure surface for a paid action (/api/audio charges credits; the 402/insufficient-credits case also lands here with no explanation). The loop does detect terminal fal states (line 2634 breaks on FAILED/ERROR/CANCELED) and job.status_url is in scope, but unlike every generation path it never calls requestRefund, so a fal-confirmed-failed preview quietly keeps the charge. Owner-notes 1181 lists 'voice-preview errors are silent' as queued â confirmed, with the refund omission as the
- **worker.js:3170** <worker-routes> â /api/social/comment/reply is the only social WRITE endpoint with no useQuota gate â unlimited public Instagram comment posts per day â and its catch leaks raw exception text to the client.
  - evidence: Lines 3170-3187: after auth + COMPOSIO_API_KEY check the route goes straight to composioExecute â no `useQuota` call. Sibling write routes are gated: /api/social/dm/send `useQuota(request, "dm", 200)` (line 3063), /api/social/publish `useQuota(request, "publish", 30)` (line 3084); even the read routes carry `analytics` 120/day. Each call is a metered Composio execution against Meta's ~750/hr comment-reply cap (docs/media-agent.md line 68), so an abusive/looping client can burn the Composio meter and the user's Meta rate limit unbounded. Additionally line 3185 returns `String((e && e.message) |
- **worker.js:4116** <worker-routes> â /api/cancel and /api/video/poll validate only the fal URL shape, not ownership â any authenticated user who obtains another user's status_url can poll or cancel that user's queued render.
  - evidence: Lines 4116-4134 (/api/cancel) and 4575-4594 (/api/video/poll): both check `authUser(request)` and a `^https://queue\.fal\.run/...` regex, then execute with the server FAL_KEY; neither cross-checks the request_id against the caller's gen_charges rows (which exist and carry user_id â used by /api/refund at 4183-4192 and the idempotency lookup at 1728-1731). Request ids are unguessable UUIDs delivered only to the owning client, and a maliciously cancelled job is refundable via the client's CANCELED flow (public/chat.js:5260-5272), so exposure is low â but the pattern diverges from /api/refund, wh
- **worker.js:4225** <worker-routes> â /api/import/fetch's free path has no quota or rate limit â any authenticated user gets an unmetered server-side fetch proxy (up to ~29MB returned as base64 per call, unlimited calls/day); only the paid AI-rescue branch is quota'd (scanai 20/day).
  - evidence: Lines 4206-4252: after authUser the route safeFetches the user URL and (for HTML) walks image candidates with retries, returning base64 media to the client â the only useQuota call in the route is inside the AI branch (line 4268, `useQuota(request, "scanai", 20)`). SSRF is well guarded (safeFetch/hostIsBlocked, lines 756-793) and sizes are capped (MAXES line 4222, readCapped), but a scripted client can invoke it in a tight loop as a free CORS-bypass/download proxy, burning Worker CPU and egress. CLAUDE.md documents the scanai quota but no gate exists on the clean-fetch path; every comparably a
- **worker.js:3966** <worker-routes> â Documented director model routing diverges from code: CLAUDE.md says "Sonnet 5 handles High/Ultra/Max plus the ask/error/studio steps", but /api/direct routes ask/error/studio (and Low/Medium compose/revise) to Haiku 4.5.
  - evidence: Lines 3966-3969: `const dirModel = (step === "compose" || step === "revise") && (effort === "high" || effort === "ultra" || effort === "max") ? "claude-sonnet-5" : "claude-haiku-4-5";` â so ask, error, and studio always run Haiku (the in-code comment at 3960-3965 describes this as an A/B-verified split; research alone still runs Sonnet, line 3417). CLAUDE.md's /api/direct description says Sonnet handles "the ask/error/studio steps", and no owner-notes entry records the switch (grep for haiku/A-B in docs/owner-notes.md returns nothing). Billing is consistent with the cheaper model (orchestrator
- **worker.js:3633** <director-prompts> â vidRefLine always describes a single video reference 'labelled @Video1' even when 2-3 Seedance video refs are attached, contradicting the ctx line in the same prompt ('cite them as @Video1...@Video3'); since Seedance only uses references the prompt cites and the auto-append fallback fires only when NO tag is present, @Video2/@Video3 are likely to go uncited and be silently ignored.
...
- **CLAUDE.md:6** <dead-drift> â Doc drift: CLAUDE.md describes navigation as a floating logo menu (#floatNav/#floatMenu, .float-logo/.float-item, toggleFloatMenu) â none of that exists; navigation is now top tabs plus a profile-pop menu, and two views (Integrations, Settings) are undocumented.
...
- **CLAUDE.md:11** <dead-drift> â Doc drift: CLAUDE.md says 'Sonnet 5 handles High/Ultra/Max plus the ask/error/studio steps' â in code Sonnet is used ONLY for High/Ultra/Max compose/revise; ask, error and studio run on Haiku.
  - evidence: worker.js:3966-3969: const dirModel = (step === "compose" || step === "revise") && (effort === "high" || effort === "ultra" || effort === "max") ? "claude-sonnet-5" : "claude-haiku-4-5"; with the comment at 3960-3964: 'Sonnet earns its price ONLY on High/Ultra/Max creative prompt-writing (compose/revise). Everything else runs on Haiku â the routing/classification ask step ..., the low-stakes error/studio steps'. orchestratorCostMicros (worker.js:952-958) also prices ask/error/studio as Haiku.
- **CLAUDE.md:47** <dead-drift> â Doc drift: CLAUDE.md's live-sweep line says '13 video + 11 image models'; the current allowlists have 12 video endpoints and only 2 image models, and the 2026-07-17 removals of Ray 3.2 and OmniHuman are not reflected anywhere in CLAUDE.md.
...
- **worker.js:3323** <dead-drift> â Dead code: /api/direct still accepts and fully implements the 'studio' director step (Studio was removed 2026-07-12) â no client code ever sends step:'studio', and CLAUDE.md line 11 still lists it as a live step.
  - evidence: worker.js:3323 let step = ["compose", "revise", "error", "studio", "research"].includes(body.step) ...; worker.js:3674-3688 carries the full 'You are isibi, the director of a shot-based video studio...' system prompt; orchestratorCostMicros comment (worker.js:946, 957) still prices the studio step. grep "'studio'" in public/chat.js returns zero hits â the only step values the client sends are error/ask/revise/compose/research (chat.js:4966, 4993, 5764, 5826, 5852, 6006). The step remains directly callable by any authed user (charges 0.5 credits, runs a Haiku call) for a feature that no longer 
- **public/chat.js:8746** <dead-drift> â Dead code: enterCrt() (8746), hideCrt() (8757) and crtNoSignal() (8881) are never called, and all three reference DOM ids that no longer exist in index.html (#crtSelect, #crtNote).
  - evidence: grep counts across public/*.js, index.html, worker.js show each name appears exactly once (its definition). enterCrt/hideCrt do getElementById('crtSelect') and crtNoSignal does getElementById('crtNote') â neither id exists in public/index.html (grep returns nothing; the CRT markup only has crtScreen/crtMenu/crtChatbox/crtLandInput). These are leftovers from the earlier 'CRT shown right after sign-in' design; the header comment at 8739-8743 still describes that flow (and a 'VHF knob' that has no markup), while the real flow is initCrt() at 8633 ('the CRT is now the landing itself'). paintCrt's 
- **public/chat.js:4777** <dead-drift> â Dead code: the membership 'output equivalence' helpers IMG_CR, VID_CR, roundTo, estImages, estVideos (chat.js 4777-4781) are defined but never used anywhere.
  - evidence: grep -c: IMG_CR appears 2Ã (definition + use inside estImages), VID_CR 2Ã, roundTo 3Ã (definition + the two est* bodies), and estImages/estVideos each appear exactly once (their own definitions) across chat.js/index.html/worker.js â nothing calls estImages/estVideos, so the whole 4777-4781 block is unreachable. The comment above it (4773-4776) also still says 'AI is the separate Orchestrator add-on now', contradicting the add-on's removal on 2026-07-14 (CLAUDE.md Credits section).
- **public/chat.js:4812** <dead-drift> â Doc drift in code: openCredits() header comment still describes it as 'Focused upsell for the AI Orchestrator add-on ($19.99/mo, at cost)' â the add-on was removed 2026-07-14 and openCredits is now the Plus/Pro/Max pricing page.
...
- **public/styles.css:1643** <dead-drift> â Dead CSS: the removed sidebar workspace nav's rules (.side-nav, .nav-dd-* dropdown, .nav-ico/.nav-gal/.nav-proj/.nav-studio, .side-user/.side-foot/.side-email cluster) match nothing in the DOM or JS.
  - evidence: styles.css 1643-1680 defines .side-nav and the .nav-dd/.nav-dd-btn/.nav-dd-menu/.nav-dd-item/.nav-dd-sep/.nav-dd-account family; grep for 'side-nav', 'nav-dd', 'side-user', 'side-foot' across index.html/chat.js/auth.js returns zero hits (no dynamic construction of these prefixes either). The 6-item Workspace nav was removed 2026-07-15 (owner-notes 'Workspace restructure'), which documented removing the nav but not that its CSS was left behind â unlike the Studio CSS, which owner-notes explicitly records as intentionally kept.
- **public/styles.css:3424** <dead-drift> â Dead CSS: the removed Orchestrator/add-on upsell styles (.orch-up-* at 3424-3443, .addon-* at ~3348-3360, plus .up-modelbox/.up-mchip/.up-mrow etc.) have no matching markup or JS.
  - evidence: styles.css:3424 '.orch-up { text-align: left; ... }' through .orch-up-feat (3435) and styles.css:3348+ .addon-eyebrow/.addon-env/.addon-env-row etc.; grep for 'orch-up' and 'addon-' across index.html/chat.js/auth.js returns zero hits (checked for dynamic prefix construction too â none). These styled the $19.99/mo add-on upsell UI removed 2026-07-14.
- **public/styles.css:4298** <dead-drift> â Dead CSS: CRT 'set' prop rules â .crt-hud/.crt-rec (4298-4302), .crt-panel/.crt-plate (4329-4338), .crt-dial*/.crt-knob* (4340-4352), .crt-laurel, .crt-power â match nothing; the current CRT landing markup has no side panel, knobs, or HUD.
  - evidence: grep 'crt-panel|crt-knob|crt-dial|crt-hud|crt-plate|crt-power|crt-rec|crt-laurel|crtl-note' in public/index.html exits 1 (no matches); index.html's CRT block (lines 33-113) only contains crt-screen/crt-glass/crt-scan/crt-topbar/crt-crest/crt-menu/crt-stage/crt-inbox/crt-chatbox/crt-legal. chat.js builds no crt-knob elements either â its comment at 8742 ('The VHF knob turns with the channel') is stale along with the CSS.
- **public/styles.css:3511** <dead-drift> â Dead CSS: a large block of the replaced Morphic-style marketing landing survives â ~150 of the 314 .mkt-* rules (e.g. .mkt-hero 3511, .mkt-strip 3539, .mkt-pricing/.mkt-plan-*, .mkt-presets, .mkt-final, .mkt-foot-*) plus old Home-landing rules .lp-card/.lp-rec* (2448) match nothing; owner-notes still presents the Morphic design as the shipped landing.
...
- **CLAUDE.md:29** <dead-drift> â Doc drift (minor): CLAUDE.md says "trySave treats 402 as terminal (lastSaveBlock)" â the identifier lastSaveBlock no longer exists; the 402-terminal behavior lives in trySave's returned block field.
  - evidence: grep 'lastSaveBlock' in public/chat.js returns zero hits. The actual mechanism: chat.js:4083 comment 'block is the non-transient 402 reason (free = paid-only, full = cap hit)' and trySave (4086) at 4095-4097 parses the 402 reason and returns it as block â behavior matches the doc, only the named identifier is stale.
- **public/chat.js:2597** <ux-deadends> â While a live TTS voice preview is generating (up to 90s), every other voice's â¶ button â and re-clicks of the same one â silently no-op: the `previewing` guard returns before any UI change on the clicked control.
  - evidence: previewVoice: `if (previewing) return;` (chat.js:2597) runs BEFORE `btn.disabled = true; btn.textContent = 'â¦'` (chat.js:2600-2601), so a second click on any uncached voice does nothing visible. Compounding it, the control is a `<span class="set-voicebtn">` (chat.js:2742), so even the active button's `btn.disabled = true` is acknowledged as a no-op in the code's own comment (chat.js:2584: 'the preview control is a <span>, so btn.disabled is a no-op') â every â¶ stays visually clickable throughout. Reachable path: Settings panel â Voice section â click â¶ on voice A (no static /voices/*.mp3 files
- **public/chat.js:6114** <ux-deadends> â Denying (or approving) one review card wipes EVERY persisted review card in the chat â a second pending card (e.g. the auto-reword offer after a content-filter failure) silently vanishes on the next thread repaint or reload without the user ever acting on it.
  - evidence: clearReviews filters all review messages indiscriminately: `c.msgs = c.msgs.filter((mm) => mm.t !== 'review')` (chat.js:6111-6116), and it's called from both deny (chat.js:6083) and approve (chat.js:6091). Two cards can coexist in one chat: a content-filter kill posts a reworded-prompt review card via offerReword â saveToChat({t:'review',â¦}) (chat.js:5001-5003), and a subsequent Plan-mode message pushes a second one via reviewPrompt (chat.js:6124). Reachable path: Plan mode â render fails on the content filter â 'âï¸ â¦reworded to passâ¦ Approve to try again:' card appears â user instead types a 
- **public/chat.js:5144** <refresh-resume> â Cancel-mid-submit's late response handler calls jobClear(origin) unconditionally by chatId â if the user cancelled and immediately started a NEW generation in the same chat, the old submit's late reply wipes the new run's job record, stripping the new charged run's refresh protection.
  - evidence: Line 5143-5144: `if (!alive()) { jobClear(origin); // cancelled mid-submit â drop the provisionalâ¦`. jobClear (line 4045) filters only on chatId, not on the run's idem. cancelGen deletes the chat from activeGens synchronously, so a new generateMedia can start and write its own record (provisional at 5080, or full record at 5188) while the old fetch is still resolving; when the old reply lands, !alive() is true (myGen differs) and jobClear removes whichever record is current â the NEW run's. The new run keeps working in-memory, but a refresh or dropped reply during it now loses the charged rend
- **public/chat.js:4043** <refresh-resume> â jobsWrite caps the job store at 8 records (slice(-8)) with no notice or refund for the dropped one â the 9th concurrently-outstanding paused/in-flight job silently evicts the oldest paid render's only recovery record.
  - evidence: Line 4043: `function jobsWrite(list) { try { localStorage.setItem(JOBS_KEY, JSON.stringify(list.slice(-8))); } catch {} }`. Records are one-per-chat and persist across sessions until terminal (paused jobs live for days across boots, bounded only by tries<4). A user running generations across many chats â or accumulating paused records during an outage â pushes the oldest record off the end silently: no finishDeadJob resolution, no refund attempt, no message, contradicting the 'every render must end visibly' rule (line 4302-4305). An explicit eviction that routes through finishDeadJob would pre
- **public/chat.js:4326** <refresh-resume> â finishDeadJob never attempts idem-based recovery for a dead provisional record (idem but no statusUrl): a job that WAS charged (reply lost, then 4 transient recovery failures) ends with no refund attempt and only an apologetic message, even though the worker's gen_charges lookup could have produced its statusUrl.
  - evidence: Line 4326: `const refunded = j.statusUrl ? await requestRefund(j.statusUrl) : 0;` â for an idem-only record the refund is skipped entirely and the user gets 'â¦if credits were taken for it, use the same prompt to run it again' (line 4330), i.e. pay again. recoverJob (line 4358) shows the worker can resolve idem â status_url/response_url via the recover re-POST (worker.js:1726 gen_charges lookup), but finishDeadJob doesn't use it before giving up, so a charged-but-reply-lost job whose recovery hit 4 transient errors (each `catch { jobBumpTries }` at line 4375) is neither delivered nor refunded.
- **public/chat.js:6589** <refresh-resume> â doSignOut wipes JOBS_KEY and SAVES_KEY unconditionally â signing out while a charged render is paused/in-flight permanently discards its recovery record with no cancel, refund attempt, or notice, even when the same account signs back in.
...
#### PLAUSIBLE (one skeptic refuted â worth a look, 9)
- worker.js:2508 <provider-leak> [medium] â briefErr passes raw upstream fal error text to the client unscrubbed on submit failure ({error:"submit failed", detail: briefErr(data)}); the worker has no scrubProvider equivalent, so provider hiding
- public/chat.js:3831 <provider-leak> [medium] â Temporary fal.media links delivered to save-blocked users surface the provider domain outside devtools: downloadMedia's fallback opens the raw fal.media URL in a new tab (address bar), and the media m
- worker.js:3675 <director-prompts> [medium] â The studio step's system prompt still sells the removed Studio UI: it tells users about an Export button that stitches shots on-device, export_style transitions, and free on-device trim/speed/reframe/
- worker.js:773 <security> [low] â SSRF guard never resolves DNS, so a public hostname pointing at a private IP is not blocked (DNS rebinding)
- worker.js:4581 <provider-leak> [low] â KNOWN/by-design exposure, noted per instructions: /api/video/poll?url=, /api/cancel and /api/refund carry full queue.fal.run URLs in request params/bodies, and the /api/video response returns fal stat
- public/chat.js:8746 <dom-consistency> [low] â Post-sign-in CRT selector remnants reference ids that exist nowhere: enterCrt looks for #crtSelect (never created, and enterCrt itself is never called), crtNoSignal for #crtNote, and the LIVE paintCrt
- public/chat.js:2508 <dom-consistency> [low] â The entire preset-chip subsystem (PRESET_CATS data, renderPresetsInto, usePreset, renderLpChip, applyPresetRig, ~200 lines) anchors to #lpInput/#lpChipHost/#lpHint, which are created only inside dead 
- public/chat.js:8973 <dom-consistency> [low] â initLeadHero references #leadPh and #leadWord, which exist in no HTML file; the function is itself uncalled (replaced by initCrtStage), so it is dead code whose type() closure would throw on the missi
- worker.js:4596 <worker-routes> [low] â Wrong-method requests to every /api/* route fall through to the static asset handler and return a 404 (asset not found) instead of 405 â e.g. GET /api/save or POST /api/credits gets an HTML-ish 404, a
...
- H1 (worker) Veo/Fast extend + Sound OFF now sends generate_audio:false on
  the extend endpoint (it accepts it despite bareEdit) â the render is
...
- H2 (client) Merged End-frame pairing was DEAD (lived in the clip/audio
  ...
  branch â the ONLY place image kinds resolve. Verified: real file-input
  pairing imageâffirst+flast, counter 2/2, demote on end-remove.
- H3 (worker) Stripe invoice.paid now returns 500 if set_plan fails
  (add_credits is idempotent on ref, so Stripe's retry is safe) â paid
...
- H6 (worker) Ask step no longer gets the raw fal model id â a friendly
  label map ("Kling o3 Pro" etc.) feeds the director, plus an explicit
  provider-nondisclosure rule in the ask prompt.
- H7 (worker) Kling o3 clip-edit @ImageN tags preserved â the reconciler
...
- Money: webm clip-edit quote now matches the worker's max-bill for
  ...
  (the `=== "auto"` guard was dead â ~2Ã overcharge); import AI-rescue now
...
- Resume: finishDeadJob re-persists a bounded (dtriesâ¤3) delivery retry so
...
### LOW audit findings â behavioral fixes + doc drift (2026-07-17)
FIXED (behavioral):
- Audio bills on chars ACTUALLY spoken (min(2000, len)) â a 2-4k plan-mode
  script no longer bills above the 2000-char quote.
- Voice-preview failure now refunds the TTS credit + toasts a reason
  (was a bare "â " glyph, credit silently kept).
- Denying/approving one review card clears ONLY that card â a second
  pending card (e.g. the content-filter reword offer) survives.
- awPlayer torn down on chat switch â the play button no longer plays the
...
- /api/import/fetch free path is now rate-limited (useQuota "import" 120/day)
  â was an unmetered ~29MB server-side fetch relay.
- GPT auto-ratio overcharge (same dead guard as the medium) â fixed.
...
DEFERRED (deliberate â inert or near-unexploitable; sweeping risks the live
app for ~zero runtime benefit):
- Dead JS functions enterCrt/hideCrt/crtNoSignal + renderPublish (Media
  Agent) â never called; live only if some path invokes them (it doesn't).
...
- worker `studio` director step â inert (no client sends step:'studio');
...
- /api/cancel + /api/video/poll ownership: the fal request IDs are random
  ...
  near-unexploitable; a proper per-user jobâuser map is an invasive change
...
## 2026-07-17 â Provider-leak scrub (the two real "plausible" findings)

Standing owner rule (absolute): the user must NEVER see "fal" anywhere â not
...
- worker.js `briefErr()` returned upstream error text verbatim to the client
  ...
  `scrubProvider()` (mirrors the frontend one â strips provider URLs, maps
  standalone `fal`/`fal-ai`/`fal.{ai,run,media}` tokens â "the render
...
- chat.js `downloadMedia()` catch-fallback did `window.open(rawUrl)` on a
  cross-origin fetch failure â for a temp-delivered (save-blocked) render
...
Latent (NOT fixed â unreachable): chat.js:5455 delivers `out.exr_file.url`
(a raw render-service URL) in chat. Only fires for `luma/` models, which
were all removed with Ray 3.2 â no luma entry remains in the picker, so the
...
Defense-in-depth (NOT fixed â near-zero probability, not an error path):
...
## 2026-07-17 â Ray 3.2: full dormant-code removal ("make sure everything from ray3.2 is gone")

Ray was delisted 2026-07-17 (no `luma/` model in either allowlist), but a large
dormant subsystem stayed behind gated on `model.startsWith('luma/')` â which no
...
- **worker.js:** removed `sanitizeRayControls`/RayEditControls, the `isRay` decl +
  ...
  `hdr` local and the Nano/Studio `reframe` (aspect re-crop) are unrelated â kept.
- **index.html:** removed `#rowKf` + `#fileKf`. **styles.css:** removed `#rowKf`
  selectors. **CLAUDE.md:** updated the removal note.

Verified headless: every remaining model (3 Veo Â· 3 Seedance Â· 3 Kling text +
LipSync Â· Gemini Â· 2 image Â· audio) builds its menu/opts/attach-panel/price with
...
attach rows are Image-to-video/Extend-clip/Reference â no Keyframes/HDR/Loop.

## 2026-07-17 â Media Agent: "Schedule post" tab (Instagram, FRONTEND ONLY)
...
- Composer (reuses the .ma-publish publish-composer look): Media (device file â
  ...
  A "Preview Â· not published yet" flag makes the not-live status explicit.
...
- Queue cards: thumb (downscaled 400px for images, ð¬/ð¼ icon otherwise),
  caption (or italic "No caption"), IMAGE/REEL pill, ð dateÂ·time, a gradient
  SCHEDULED status pill, and an Ã remove.
- NOTHING publishes â no backend call anywhere in this tab. Wiring the actual
...
Follow-up (same day): the Schedule composer now also offers "ð¼ From gallery"
â generalized openPubGalleryPicker to take an onPick callback (defaults to the
...
picker lists, selects image+video, queues, persists â no page errors.

Follow-up (same day): the Schedule post tab now shows ONLY the composer â the
...
Follow-up (same day): the Schedule post tab is now TWO COLUMNS â composer on the
...
below the grid (thumb/icon Â· caption Â· timeÂ·type Â· remove Ã). Scheduling jumps
...
Follow-up (same day): calendar day cells now show the POST THUMBNAIL â each chip
is [thumb][compact time] (e.g. red image + "9am"); a video with no poster falls
back to a ð¬/ð¼ icon. Added schTimeShort() for the tight chips (day-panel keeps
the full "9:00 AM"). Verified headless: real thumb data-URLs render as chip
backgrounds, icon fallback for no-thumb.

## 2026-07-18 â Builder buttons: "Outline" language (owner pick)

Explored 10 design directions in artifact mockups (5 recolors, then 5 button
languages on the untouched pinkâamber palette). Owner picked **Outline**:
the gradient moves from fills to borders. Applied to styles.css (late block
so it wins the earlier fills):
- .send â hollow, 1.5px pinkâamber gradient ring (padding-box/border-box
  trick), soft pink glow; hover deepens interior + glow; disabled keeps the
  ring, no glow (earlier box-shadow:none still applies).
- .send-price â gradient TEXT (background-clip:text), no more filled pill.
- .mode-btn.active â outlined with the gradient ring instead of the gradient
...
and inherits the same look â consistent by design.

## 2026-07-18 â Gemini edit: EEA/UK/Switzerland friendly error
...
editing is not supported", and Google's own tip â simple prompts + "Keep
everything else the same." â which our edit-writer already follows). Added
...
blocks it in the EU, UK and Switzerland â pick a Kling o3 model for clip
...
Follow-up: owner â no model suggestion in the regional error. Both messages
...
## 2026-07-18 â Website Builder: standalone view, FRONTEND ONLY (v1)

The landing's "WEBSITE / MOBILE APP" channel becomes a real product surface.
Owner's calls: SEPARATE UI (not a chatbox mode) â the only thing it shares
with the media builder is the â¦ credit ledger; V1 = generate + preview +
iterate (no hosting); engine will be Opus + Gemini (owner supplies the Gemini
key later) through the same credit system â BUILD THE FRONTEND NOW.

Built: a "Websites" top-bar tab â viewSites.
- Start screen: brief textarea + "Build it â¦25" (Outline-language button) +
  a grid of saved projects (live srcdoc thumbnails, delete).
- Workspace: left chat rail (thread + "Update site â¦10" revise composer),
  right preview stage with desktop/tablet/phone viewport toggles + Download
  HTML (Blob a[download]). Back â project list.
- Projects persist in localStorage (zephyr_sites_v1, 20 projects Ã 200KB html
  Ã last 40 msgs). NOTHING calls any API yet â Generate renders a clearly
...
- Engine-phase notes: (1) preview iframes are srcdoc + fully sandboxed and
  inherit the app CSP â inline styles OK, inline <script> BLOCKED, so
  ...
  at 10 credits/call â a â¦25 site fee needs a loop or a raised cap on the
...
Verified headless end to end: createâbuildâreviseâdevice togglesâdownload
enabledâbackâthumbnail cardâreopenâdelete, zero page errors (the sandboxed
preview correctly blocks storage access from inside). Screenshots reviewed.

Follow-up (owner correction): the Websites TAB was wrong â the builder is a
...
boot view at 'sites' and routes through auth into the standalone builder â
...
class (in-sites) hides ALL studio chrome â chats sidebar, Gallery/Avatar/
Media-Agent tabs, the Back arrow â only the profile bubble (shared account/
credits surface) remains; leaving via profileâSettings etc. restores the
...
way back in (signed-in visits skip the landing) â decide later whether the
account menu gets a "Websites" row or the landing stays reachable signed-in.
Verified headless: landingâchannelâbriefâauthâstandalone builder with chrome
...
MIRRORS Lovable's anatomy, skinned in isibi dark + pinkâamber â
- top bar: â back Â· project name + "Previewing last saved version" Â·
  centered â Preview pill + "Homepage" + refresh Â· devices, â¤ download,
...
- left rail: session date stamp + SAMPLE ENGINE flag, Lovable-style messages
  (grey right-aligned user bubbles, plain agent text with a working copy â§
  action), "Ask isibiâ¦" composer with + (inert), Build â¾ selector chip,
  gradient â¦ price, round gradient-ring send;
...
BACKLOG (owner, 2026-07-18 â do NOT build yet): Website Builder "Publish"
...
under isibi.ai subdomains â no third-party coupling, instant deploys, own
domain). Decide when the engine phase starts. No work authorized yet.

ENGINE PLAN (owner, 2026-07-18 â for the Website Builder engine phase):
owner's call on models: "Gemini is better at design, Claude better at backend/
architecture." Agreed pipeline: (1) build â¦25 = Claude-cheap spec â Gemini
visual generation â Claude hardening (semantics/a11y/SEO/form wiring);
(2) revision â¦10 = route by intent (visual â Gemini, functional â Claude)
...
â¦25/â¦10 margins healthy. Needs GEMINI key from owner (pending) â not built.

ENGINE PLAN amendment (owner, 2026-07-18): Claude side is OPUS ONLY â no
...
Opus hardening), so â¦25 would lose money â reprice at engine time (working
range â¦50-75 build / â¦15-25 revision; pin from real token measurements).
Pure-visual revisions route to Gemini alone (no Claude call at all); the
revision-intent router must NOT be Haiku/Sonnet â use a keyword heuristic
or Gemini Flash.

Follow-up: GEMINI_API_KEY (already in the GitHub secrets vault, owner
confirmed the exact name) is now wired into deploy.yml â uploads to the
...
flagship is **Gemini 3.1 Pro** (`gemini-3.1-pro-preview`) â that's the
...
match GEMINI_API_KEY exactly â check the Actions log.

## 2026-07-18 â WEBSITE BUILDER ENGINE WIRED (owner: "engine time")
...
- build (â¦60): Gemini designs the full single-file site from the brief â
...
- revise (â¦20): keyword-routed â functional/correctness instructions â Opus,
  visual â Gemini alone. Router is a regex, deliberately not a model call.
- Money: `useQuota("site", 40)`/day BEFORE the charge; `use_credits` up
  front (402 â not enough, cost in body); EVERY terminal failure refunds the
  full fee via a credit_back LOOP (RPC caps 10/call â 6 calls for â¦60).
  â¦60/â¦20 are the owner-approved range midpoints; token counts are logged
  (console: "site design tokens" / "site build tokens") â tune from real
...
- CSP: added `frame-src 'self' blob:` â the preview now renders from a Blob
...
â¦ pill refreshes after every call; prices now â¦60/â¦20 everywhere; SAMPLE
machinery deleted; hero flag now "Beta"; localStorage html cap 400KB.
Verified headless with a mocked /api/site: buildâblob preview (scripts
...
exist on the Worker) â owner runs the first one.
...
no auto-retry â revisit with idempotency keys if it ever bites.

## 2026-07-18 â Audit round-2 fixes: BILLING batch (money)
...
- **Delete account now cancels Stripe FIRST** (chat.js delete handler): calls
  ...
  ABORTED with a message â fails safe (never orphan a live subscription on a
...
- **TTS undercharge fixed** (worker): input.text is now sliced to 2,000 chars
  â the SAME cap billing uses (was sending up to 4,000 uncut â fal billed us
  up to 2Ã the charge). Fixes the HIGH.
NOTE: the deleteâcancel Stripe path is logic/syntax-verified + fail-safe by
...
## 2026-07-18 â Audit round-2 fixes: DATA-LOSS batch
...
- **Transient media error no longer deletes the message** (buildMedia
  ...
  connection too â the old code spliced the message AND synced the deletion,
...
- **zephyr_assets_at_v1 wiped on sign-out AND account-switch** + the in-memory
  `assetsAt` reset to 0 in both paths. Was surviving â the next account's
...
- **Mid-session re-auth account switch** (finishAuth): a 401 pops the gate via
  showAuthGate() directly (authEntry stays 'stay') â routed to the landing,
  skipping enterApp's account-switch wipe â previous account's chats/avatars
...
## 2026-07-18 â Audit round-2 fixes: REFUND / JOB-RECOVERY batch
...
- **Avatar renders now survive a refresh** (chat.js): the charged render is
  ...
  (resumeJobs â resumeAvatarJob) recovers it: fal COMPLETED â save the avatar;
  stuck/failed â cancel + refund. The sign-out sweep already refunds any
...
- **finishDeadJob cancels before refunding** (chat.js): a job wedged IN_QUEUE
  ...
  cancelThenRefund() cancels first (â CANCELED) so the refund lands. finishDeadJob
...
- **recoverJob no longer destroys the record on a lookup blip** (worker): the
  ...
  (not-ok/throws/unparseable) â only a SUCCESSFUL lookup that finds no charge
...
## 2026-07-18 â Audit round-2 fixes: import overcharge + remaining leak flagged

- **Import AI-rescue no longer charges users who can't save** (worker
  /api/import/fetch): before charging â¦3 for the paid image lookup, it now
  checks storageStatus â a cap-0 user (free/lapsed/top-up-only) gets the
  402 {reason:"free"} upgrade block UNCHARGED, instead of paying â¦3 for an
  image the subsequent /api/save would 402 on anyway. Fails open (ledger
  unreachable â proceed) so a real member is never wrongly blocked. Fixes HIGH.

## 2026-07-18 â demo-hero clones: deleted 1 & 3, kept 2 + closed the serve gap
...
- Owner reviewed all three demo-hero* clones (screenshots) and chose:
  **delete demo-hero (archived landing) + demo-hero-3 (CRT landing), keep
  demo-hero-2** (the full current-app clone â auth.js/chat.js/index.html/
  styles.css) as the design reference.
- **Caught a bug in the prior route guard**: the `448a211` block used
  `/^\/demo-hero(\/|$)/i`, which only matched the bare `/demo-hero` â the
...
## 2026-07-18 â Free-tier video/audio leak CLOSED: same-origin stream proxy (owner picked "a")

- Owner chose option (a): build the streaming proxy, accept the bandwidth.
- **Worker**: two new routes.
  - `POST /api/media-token` (auth'd) â AES-GCM-seals a provider media URL into
    an opaque token. Key is SHA-256(FAL_KEY + "|media-proxy-v1") â no new secret
    to provision. Token = base64url(ivâciphertext) of `{u,e}` (url + 7-day
    expiry). Only provider-host URLs seal (regex-gated); returns 400 otherwise.
  - `GET /api/m/<token>` (NO auth â a <video> src carries no Authorization
    ...
    leaks. Tampered/expired/wrong-host tokens â 404.
- **Client**: `proxyMediaUrl(u)` mints the token and returns `/api/m/<token>`;
  wired into BOTH temp-link paths in buildMedia â the free/full `block` path and
  the transient `saveFailed` path (video/audio only; images already ride a
  data: URL via the client watermark). Retries the mint 2Ã (same-origin+authed,
...
- Net: a free-tier / over-cap video or audio render is delivered on a same-
  origin src â right-click "copy address" and devtools both show isibi.ai, never
  the provider. Cost: Worker egress for every free-tier temp-link play (accepted).

REMAINING MINOR LEAK (not yet fixed â flag):
- **EXR sidecar link** (chat.js ~5341) is delivered as a raw provider URL inside
  ...
  Lower priority (niche pro feature) but violates the same rule â proxy or drop
  it in a later pass. â DONE 2026-07-18: dropped the EXR block entirely (dead
  code since the Ray/HDR pipeline was excised â no model returns exr_file).

## 2026-07-18 â Audit re-verification pass + 2 remaining fixes
...
- Re-verified the 19 confirmed MEDIUM findings (+ leak/security) from the Jul-17
  ...
  majority were ALREADY FIXED by later work â all money mediums (webm clip 30s
  overcharge â clipMeasurable guard; GPT 4K auto â 1K bill; import-rescue -1 â
  ...
  - **galleryDelete hard-delete swallowed a non-throwing failure**: storageDelete
    ...
    set ok=true regardless â phantom delete that reappears on next load with no
    message. Now captures the boolean â toast + card restored on failure.
- LEFT ALONE (per owner's "skip Media Agent"): the orphaned Media-Agent
  ...
  confirmed unreachable dead code â harmless, can delete on the owner's word.
- Bounded residual (not a regression): a new send in the ~45s window after a job
  hits tries==4 can still clobber the dead record before finishDeadJob refunds â
  down from permanent silent loss to a narrow race. Noted, not urgent.

## 2026-07-18 â Low-severity re-verify pass (33 low + 9 plausible)
...
- **deleteChat didn't hydrateStaged** â deleting the active chat left the
...
- **Gallery load-failure looked like an empty gallery** â a failed /api/gallery
  fetch left serverGallery null and showed "Nothing here yet" on a device that
  DOES have saved media. Added a galleryLoadFailed flag â "Couldn't load your
  gallery just now â check your connection and reopen it."
...
- **vidRefLine only cited @Video1** for multi-clip Seedance reference runs (the
  ctx line already pluralized) â extra staged clips went uncited/inert. Now
  pluralizes to @Video1â¦@VideoN when >1.
...
- **Pure dead code / dead CSS** (renderPublish, enterCrt/hideCrt/crtNoSignal,
  ...
  .sb-toast) â cosmetic only, zero user impact; a bulk-deletion sweep in the
...
- **Refresh/resume money-edges** (jobsWrite slice(-8) silent eviction; finishDeadJob
  ...
  scoped by chatId) â narrow charged-render edge cases in the resume machinery;
...
- **Infra/security decisions**: SSRF DNS-rebinding (no trivial Workers fix),
  ...
  wrong-method â 404 not 405 (cosmetic).
- **Media Agent** comment-reply route (missing quota + raw error string) â LEFT
  per owner's "skip Media Agent".

## 2026-07-18 â Audit buckets 2 & 3 (resume-edges + infra), and what's deferred

Bucket 2 â resume/refund edges:
- **Cancel-mid-submit now clears ONLY its own record** (jobClearByIdem) instead
  of the whole chat â a new run started in the same chat after a cancel keeps
  its refresh protection.
- **finishDeadJob now attempts an idem recovery** for provisional (idem-only,
  no statusUrl) records before the refund/apology â a charged render whose submit
  reply was lost can be recovered + delivered instead of only apologized for.
- **jobs cap raised 8â24**: routing an evicted record through finishDeadJob would
...
Bucket 1 â dead code (done in the prior commit): removed ~320 lines of dead
landing/preset/CRT JS. renderPublish left (Media Agent, standing rule).

DELIBERATELY DEFERRED (with reasons â these are NOT clear wins):
...
## 2026-07-18 â FOUC fix: app shell flashed behind the landing on refresh

Owner noticed: refreshing the landing briefly flashed the app UI (top-bar
tabs / chatbox) for a frame. Cause: `.shell` (the app) had no display:none â
it was only made `inert` behind the landing â while `#marketing`/`#authGate`
...
HTML and enterApp() reveals it (`shell.style.display=''`) â the single authed
...
## 2026-07-18 â Website Builder: Opus pass removed, Gemini-only

Owner funded the Gemini key and called it: drop the Opus hardening/architecture
pass â the Website Builder engine is now **Gemini-only** (gemini-3.1-pro-preview).
- build: ONE Gemini pass that both designs AND engineers the site (the old
  Opus-hardening requirements â semantics, 360/768/1200 responsive, a11y, robust
  JS â are folded into the Gemini build prompt).
...
- Pricing UNCHANGED for now (build â¦60 / revise â¦20). Charge timing is already
  correct: credits are only taken when the user clicks Build AFTER typing a brief
  (the /api/site call), never on page load or an empty box â the â¦60 is just the
  price label. NOTE: with Opus gone the real per-build cost dropped a lot, so the
  â¦60/â¦20 price is now high-margin â pending owner decision on whether to lower.

## 2026-07-18 â Website Builder: metered billing + send button (no flat fee)

Owner: make it a send button; credits drawn automatically from the REAL cost now
that we have the Gemini key. Done:
- **UI**: "Build it â¦60" â "Build it â" (send button, no price). Workspace
  composer â¦20/â¦60 chip removed (the â send stays). Hint now: "Credits are based
  on what each build actually uses â and refunded if it fails." Success message
  shows the actual charge, e.g. "(â¦13 used)".
- **Billing (worker /api/site)**: metered on real Gemini tokens. Pricing pinned
  from Google's page â gemini-3.1-pro-preview: $2/M in Â· $12/M out (â¤200k-token
  ...
  24576 output cap) via use_credits so work is never unpaid â run â refund down
...
- Net effect: a typical build (~8k output) now costs ~13 credits instead of the
  old flat 60 â users usually pay LESS, and always exactly what it cost.

## 2026-07-18 â Website Builder engine â gemini-3.5-flash (fixes the 429)

The 3.1-pro-PREVIEW model 429'd at Paid Tier 1 (preview models get near-zero
quota until the account tiers up â a Google-side limit, not billing; owner's key
IS paid/active). Switched to **gemini-3.5-flash** â the current GA flagship, so
...
medium; thinkingConfig.thinkingLevel is the right field â validated because the
3.1 call reached 429, i.e. passed request validation, not 400). Everything else
(reserveârefund metering, send button, refunds) unchanged. A typical build now
lands around ~9 credits.

## TODO (owner-flagged 2026-07-18) â Website Builder refund is off
- Owner noticed the credit REFUND in the Website Builder came back wrong (net
  balance didn't fully restore on a failed/refunded build â e.g. reserve â¦37
  but balance dropped ~â¦17). Suspect: the metered reserveârefund model leans on
  credit_back, which is designed for SMALL (â¤10/call) orchestrator reversals and
  may cap/guard total reversal â so a big reserve refund is partial. Revisit the
  ...
  pre-check) so no large reversal is ever needed. Deferred per owner â fix later.

## 2026-07-18 â Website Builder QUALITY jump (owner: "looks like AI slop")
Three levers, all in /api/site:
- **Real fonts**: SITE_RULES now ALLOWS Google Fonts (<link> to fonts.googleapis.com)
  â the ONLY external resource permitted (still no CDN scripts/frameworks, no
...
- **Thinking**: build now runs at thinkingLevel "high" (design reasoning is where
  quality comes from); revise stays "low" (surgical, cheap). MAX_OUT_TOK 32768â60000
  for room. A high-thinking build now runs ~30-35 credits (metered, still refunded
  to actual); revisions stay cheap.

## 2026-07-18 â Website Builder: REAL image generation (Nano Banana Pro) + billing rewrite
Closes the Lovable gap (they generate photos; now so do we â with our own models):
...
- **Server-side pipeline** (worker helpers genOneSiteImage/storeSiteImage/injectSiteImages):
  ...
  2K), download â upload to the user's Supabase storage (media/<uid>/site/), swap the real
...
- **Billing rewritten to CHARGE-AFTER-SUCCESS** (fixes the refund undercount the owner flagged):
  no more reserveâcredit_back. Flow: readCredits pre-check (â¥ worst-case Gemini) â build â
  charge measured Gemini cost â generate images capped to what the remaining balance affords
  â charge per generated image ($0.15=19cr each). Nothing is charged before success, so a
  failure needs NO refund. Response reports actual total cost + net balance.
- Cost: a full build now â Gemini (~25-35cr, high thinking) + up to 4Ã19 = ~75cr images â
  100-110 credits (~$0.85) when it uses the full image budget; fewer images â less. Revisions
...
- NOTE: latency is now ~1.5-2.5 min/build (high-thinking Gemini + parallel image gen). If
  ...
  token, poll) â watch for it.

## 2026-07-18 â Website Builder: REAL multi-page (owner: the page switcher should work)
...
- **Engine = two-phase** (worker /api/site build): (1) a PLAN pass (high thinking)
  ...
  hexes, Google Font pairing, nav, footer, voice, motifs>"} â decides how many pages
  ...
  carries html+path+design) â {html, path}. Token metering now ACCUMULATES across all
...
- **Client**: site model = pages[] + active + design (legacy single-`html` sites read as
  ...
  picker â so clicking the site's own nav navigates the preview.
...
## 2026-07-18 â Website Builder: make buttons/forms actually WORK (owner: theirs work, ours didn't)
Our sites looked great but were static â dead buttons, inert forms. Fixed:
- **Engine**: SITE_RULES now mandates working interactions â every CTA/nav link uses
  ...
  the list â") since there's no backend yet (never a real-submit, never a dead form); no
  placeholder "#" links. (Replaced the old "forms use action=# inert" rule.)
- **Preview sandbox** widened allow-scripts â "allow-scripts allow-forms allow-popups" so
...
- CAUGHT A SELF-INFLICTED BUG mid-edit: a stray backslash made the SITE_RULES string
  close as \"; (escaped) â it was swallowing following code. Fixed; verified the runtime
  value (2034 chars, clean close).

## 2026-07-18 â HOSTING milestone 1: Publish live to R2 (isibi.ai/s/<slug>)
...
- **wrangler.jsonc**: R2 binding SITES_BUCKET â isibi-sites.
- **DB** (Supabase): published_sites (owner, slug, pages[{path,key}], RLS own-row) +
  site_domains (for custom domains next). Applied via MCP.
- **Worker**: POST /api/site/publish â writes each page to R2 (sites/<slug>/<page>.html),
...
- **Client**: Publish button â sitePublish() posts the pages, drops the live URL in the
  thread (linkified) + a toast; button flips to "Republish"; Share copies the live link.
  site.liveUrl/published persisted.
NEXT: custom domains (Cloudflare for SaaS) â needs the for-SaaS enablement + fallback
origin in the dashboard, then /api/site/domain to create custom_hostnames + serve by Host.

## 2026-07-18 â HOSTING milestone 2: real forms backend
...
- **DB**: site_form_submissions (published_site_id, user_id, slug, form, data jsonb).
  RLS: owner-only SELECT; NO insert policy â only the service-role Worker inserts.
- **Worker**: POST /api/site/form (PUBLIC, anonymous) â caps payload (â¤30 fields,
  values â¤2k), honeypot (_hp) drops bots, validates the slug against published_sites,
...
- **Engine**: SITE_RULES forms now fire-and-forget POST to /api/site/form with
  ...
  success state. In the preview (no /s/ slug) it just no-ops â shows success, stores nothing.
- **Client**: published sites get a ð¥ Inbox button in the workspace top bar â siteInbox()
  modal lists submissions (form name, fields, timestamp), newest first. site.slug stored on publish.

## 2026-07-18 â HOSTING milestone 3: real visitor-auth backend (+ live-map fix)
...
API" â it did NOT transmit the password though). Owner's call: build the REAL auth backend.
- **DB**: site_users (published_site_id, owner_idâauth.users cascade, slug, email citext,
  password_hash, created_at, last_login_at; unique (published_site_id,email)). RLS: owner
  SELECT only (auth.uid()=owner_id); NO write policy â only the service-role Worker writes.
...
- **Worker (brains, storage=Supabase)**: PBKDF2 (100k, SHA-256, 16-byte salt) hashing +
  ...
  Â· POST /api/site/auth/signup {slug,email,password} â hash+insert, returns {ok,token,email}
  Â· POST /api/site/auth/login â verify (constant-time), returns {ok,token,email}
  Â· GET  /api/site/auth/me (Bearer) â validates token for member-page guards
  Â· GET  /api/site/members?slug= (owner JWT, RLS) â the site's sign-ups
  Validation: email regex, password 8â200, dupâfriendly error, honeypot, empty-slugâ
...
- **Client**: published sites get a ð¥ Members button next to ð¥ Inbox â siteMembers() modal
  (email, joined, last login).
- **Note**: accounts work on the PUBLISHED site (real slug), not the in-builder preview â
...
## 2026-07-18 â Preview parity: real identity on BUILD (not just publish)
...
- **Worker /api/site build**: after the pages are built it mints (or reuses by site_id)
  a slug and inserts a DRAFT published_sites row (pages stay off R2 â a draft isn't
...
- **SITE_RULES**: generated sites now define siteSlug() (reads window.__SITE_SLUG__ first,
  else the /s/<slug> path) + a throw-safe `store` helper (try/catch localStorage â in-mem
...
- **Client**: build sends siteId + stores the returned draft slug; sitePreviewSrc injects
  window.__SITE_SLUG__ into the blob preview (runs before the site's JS); ð¥ Inbox + ð¥
...
- **Note/limit**: signup/login/forms/maps now work in the preview (real backend calls,
  real accounts â visible in Members). Full logged-in navigation ACROSS member pages still
...
## 2026-07-18 â Website Builder workspace: Lovable-style chrome + wired Analytics/History
...
- **History rail** (â²): lists every version; **Restore** rolls back (snapshots current
...
- **"Try to fix"** error card over the preview on a failed build/revise â re-runs a fix.
...
- **Analytics WIRED (real)**: site_hits table + site_analytics() RPC (owner-scoped).
  ...
  â distinct-hash = visitors). GET /api/site/analytics. Panel shows real Visitors/Page
...
- Still visual-only (full features, not quick wires): Cloud Database/Emails/Secrets/Edge,
  Security scan, SEOâhead-tag injection, Unpublish.

## 2026-07-18 â Wired: Unpublish, live Cloud cards, Opus security scan
- **Unpublish** (real): POST /api/site/unpublish deletes the site's R2 objects
  ...
  the Publish panel's Unpublish button. Live-tested: 200 â 404 â republish same slug 200.
- **Cloud cards**: Members / Submissions cards clickable â open the real Members /
  Inbox panels (when the site has a slug).
- **Security scan (REAL â Opus 4.8)**: POST /api/site/scan sends the generated code
  ...
  Panel: "Deep security scan Â· Run scan" â severity-coloured issue cards, or
...
- Still genuine backend PRODUCTS (not wires), left "Soon": Cloud Database / Emails /
  Secrets / Edge functions, and SEOâhead-tag injection (owner said skip SEO for now).

## 2026-07-18 â Wired: Cloud â Database (collections)
Public, displayable data store the generated site both writes and reads â for
...
- **Worker**: POST /api/site/data (anon, fail-soft, honeypot drops bots â check is on
  data._hp not just body, fixed in testing; â¤500 records/collection cap) Â·
  GET /api/site/data?slug=&collection= (PUBLIC read, newest-first, cap 100) Â·
  GET /api/site/collections?slug= (owner list, RLS).
- **SITE_RULES**: collections protocol â save via /api/site/data {slug,collection,data}
  + _hp honeypot; render by GET on load; PUBLIC + newest-first.
- **Client**: Cloud â Database card is Live + opens a modal grouping records by
...
## 2026-07-18 â Wired: Cloud â Secrets (encrypted vault)
Owner-managed secrets vault â same contract as Lovable's Secrets page.
...
- **Crypto**: AES-GCM, key derived from SUPABASE_SERVICE_KEY (siteSecretKey/encryptSecret).
  ...
  rotate = re-POST; delete). No decrypt path yet â consumed server-side by Edge
  functions (the next build).
- **Worker**: POST (verify slug ownership â encrypt â upsert on owner_id,slug,name) /
  GET (names only) / DELETE /api/site/secrets (owner JWT + RLS).
- **Client**: Cloud â Secrets card live â vault modal (add name/value, list w/ dates,
  delete). Reuses si-modal.
- Live-tested: add â stored AES-GCM (leaks_plaintext=false) â list returns no value â
  rotate updates the row â delete removes it. (First POST 404'd on edge-propagation lag,
  fine a second later.)
- Remaining Cloud "Soon": Emails, Edge functions (Edge functions = the consumer that
  reads these secrets server-side).

## 2026-07-19 â Bugfix: Website-Builder site photos leaking into the Gallery
- **Owner reported:** the Gallery showed ~6 photos they never generated (real-estate
  houses/interiors). Investigated: NOT a leak â they were in the owner's own
  ...
  ~2241) and saves them to `media/<uid>/site/â¦` with the user's JWT, so they landed
  in the same bucket the Gallery lists â surfaced as gallery cards.
...
- **Left intact on purpose:** the site images stay in storage (the published site
  references them by direct public URL â deleting them would break the live site),
...
## 2026-07-19 â Full Website-Builder test sweep (backend + frontend)
...
- **Backend 48/48** (throwaway isibi user + published site + 2 seeded fns, all
  ...
  â 400**); uploads (PNG stored+serves, SVGâ415, no-slugâ400, owner list); secrets
  ...
  Also incidentally verified unpublish (site â 404) during teardown.
...
- **Round 2 â security/guard sweep, 12/12** (public endpoints, seeded site torn
  down): **SSRF guard verified live** â edge-function `fetch` to cloud-metadata
  `169.254.169.254` and to `127.0.0.1` both BLOCKED (safeFetch â null â step
  ...
  (`gte`+`lte`), `sort`, `limit`+`offset` paging, `total`. Unknown fn â 404,
  malformed numeric filter doesn't crash. Rate limiter (120/min/slug) is
  per-isolate best-effort â not trippable reliably from outside (CF spreads the
  burst); code verified in review.
- **Not run (costs money, pending owner OK):** the 3 paid AI paths â site
  generation (Gemini), security scan (Opus), builder image-gen (fal).

## 2026-07-19 â Builder Attach: choose device vs. gallery
Owner: the builder's **Attach** button should ask *where from* â a device file
or one of the user's own isibi gallery creations â instead of jumping straight
to the file picker.
...
- **Gallery picker:** grid of the user's IMAGE gallery items (reuses
  `galleryItems()` â filtered to `kind==='image'`, so it honors the earlier
  site-image exclusion â only real chat generations show). Multi-select up to the
  ...
  so the canvas is never tainted); only downscales (â¤1600px JPEG) when a file
  would blow the 5 MB attach cap. Same `{data,name}` shape the device path +
  worker (`images[].data` â Gemini inlineData) already expect â no backend change.
- **CSS:** `.stac-opts/.stac-opt` (chooser rows) + `.stg-grid/.stg-cell/.stg-check/
  .stg-foot/.stg-add` (picker) in styles.css.
- **Tested:** headless 6/6 â chooser shows both options; picker renders cells,
...
## 2026-07-20 â Builder: chat-driven page management (add / remove / global edit / regenerate)
Owner (from the "what else" list): do #4 (streaming) plus #1-3 â add a page,
edit across all pages, regenerate a page â **all driven by CHAT, no UI:** "that
...
- **How it routes:** the existing conversational-gate classify (the one call that
  ...
  site is open, a `kind` â {edit, global, addpage, removepage, regenerate} + a
  `page` name. No extra Gemini call â folded into the gate. Biased to `edit`
...
- **global** â "make the footer say X on every page", "change the nav color
...
- **addpage** â "add a contact page", "create a blog": generates the new page's
...
- **removepage** â "delete the about page": drops the page (never home) and strips
  its nav link (incl. an enclosing `<li>`) from every remaining page.
- **regenerate** â "redesign this page", "start the home page over": rebuilds ONE
...
- **New helpers (worker.js):** `extractSiteKit(html)` (strips per-page title/
  desc/og/twitter/favicon + aria-current â the neutral shared shell), `replaceNav`,
...
## 2026-07-20 â Builder #4: live streaming build progress
Owner wanted the build to feel alive instead of one static "Buildingâ¦" for a
...
- **Worker:** wraps the build in a `TransformStream`; `emit()` writes one JSON
  line per event â `{ev:"status",msg}` at each phase (Planning â Designing N
  pages â Adding photos), `{ev:"page",name}` as EACH page finishes generating
  (fires from inside the parallel map), and a terminal `{ev:"done", â¦same
...
- **Tested:** real chunked-NDJSON Node server + Playwright, 12/12 â live line
  transitions through Planning â "Designing 2 pagesâ¦" â "Home â/About â", final
  applies (2 pages, slug, â¦cost, success msg, busy cleared, page picker appears);
...
## 2026-07-20 â Builder: site + URL named after the AI brand, not the raw prompt
Owner: the project cards / URL shouldn't be the raw first message ("hey", "make
me a website") â the builder should name the site itself. It already picks a
...
- **Worker:** the build `{ev:"done"}` payload now includes `brand`; the draft
  slug is derived from the brand (â `hey-studio-ab12cd`) instead of the brief,
...
- **Tested:** E2E 13/13 â card renames to "Brew Coffee Co", slug carried from the
  stream, all prior streaming/routing assertions still green.

## 2026-07-20 â Builder: live "activity log" during a build (Claude-Code style)
Owner: the streamed steps worked but felt slow/sparse â long silent gaps between
the server's checkpoints (plan â each page â photos). Wanted it to look
...
- **Client ticker:** a `setInterval` (1.5s) rotates an ACTIVE line through
  ...
  direction / Picking fontsâ¦"; design: "Designing {page} / Writing the copy for
  {page} / Styling componentsâ¦"; photos: "Art-directing the photos / Generating
  imageryâ¦"), so words keep moving even during a single long Gemini call. Starts
  the instant Send is hit â zero dead air before the first server event.
- **Running log:** finished checkpoints accumulate as dim `â` lines
  ("â Planned the pages", "â Home", "â Listings"); the current step is the
  bright pulsing line. Rendered in BOTH the chat rail and the empty stage during
  the first build. Painted in place (no re-render â preview iframe doesn't flash).
- **Server:** status events now carry `phase` (plan/design/photos) + the real
  ...
  prefers pages not yet â.
- Revise (fast) keeps a simple "Working" pulse. `st-livelog`/`st-ll` CSS added.
- **Tested:** E2E 15/15 (ticker shows plan+design words, "Planned the pages â"
  checkpoint, real page names, â¥3 moving frames, final build applies, brand
  rename, streamed-error path); screenshotted the mid-build log (rail + stage).

## 2026-07-20 â Builder: absorb model rate-limits (429) instead of failing
...
- **Worker (geminiCall):** now RETRIES transient 429/500/503 with backoff
  (1.5s â 4s â 8s, up to 4 attempts) before throwing. A short wait clears a
  per-minute rate cap, so most bursts now recover silently. Non-rate errors and
  the final attempt still throw as before.
- **Client:** a 429/503 that still gets through shows a friendly "â³ The builder's
  busy right now â give it a few seconds, then send again" (no scary error card,
...
- **NOTE for owner:** if 429s persist even after the retries, the Gemini API key's
  ...
  the Google AI side â that's an account/billing change, not a code fix.

## 2026-07-20 â Builder: resilient geminiCall + real error codes (code -1 was hiding the cause)
...
whole token budget â finishReason MAX_TOKENS with no visible text).
...
## 2026-07-20 â Builder: model fallback when gemini-3.5-flash is overloaded (503)
The honest error codes (prev fix) revealed the real failure: **code 503** â
...
- **Fix:** geminiCall now keeps a stable fallback (`GEMINI_FALLBACK =
  ...
  model for the remaining retry attempts â so a capacity blip on one model no
...
## 2026-07-20 â Builder engine switched: Gemini Flash â Claude Sonnet 5 (owner's call)
After the Gemini 503/429 saga, owner chose to move the website-builder engine to
**Claude Sonnet 5** â the same model class Lovable runs on (much stronger design +
...
- **`geminiCall` now calls the Anthropic Messages API** (`claude-sonnet-5`), same
  ...
  helper). `thinking` "high" â max_tokens `MAX_OUT_TOK` (20000), "low" â 8000. No
...
- **Guard** switched `GEMINI_API_KEY` â `ANTHROPIC_API_KEY` (already a Worker
...
- **Billing repriced** to Sonnet 5 rates: `toCredits = ceil((in*3e-6 +
  ...
  user meaningfully more credits, matching the ~$0.5â0.9 real API spend â so the
  owner isn't eating the difference. `MAX_OUT_TOK` 60000â20000 (Anthropic output
  cap; thinking no longer shares the budget).
- Image GENERATION (site photos) is unaffected â that's a separate path, not the
  text engine.
- Client unchanged; E2E 15/15. Live build test pending owner.

## 2026-07-20 â Builder: kill the black flash when switching pages
Owner: switching pages showed a <1s black screen (Lovable doesn't). Cause: the
page picker called renderSites() â full workspace re-render â the iframe element
was DESTROYED and recreated (a fresh iframe paints blank until its src loads), and
only THEN did loadSitePreview POST for the new URL â so there was a guaranteed
...
- **Fix:** new `switchSitePage(path)` swaps the page IN PLACE (preview view only):
  ...
  commits â seamless), and just updates the picker label, the "on" state, the URL
...
- Iframe bg `#0c0b10` â `#fff` so any residual sub-frame gap reads as a normal
  white page load, not a black void.
- **Tested:** headless 9/9 â iframe element identity survives two switches (proves
  no recreate), picker/URL/active update correctly, white bg, zero JS errors.

## 2026-07-20 â React builder: PROJECT KICKOFF (owner: go full Vite/React like Lovable)
...
- **Phase 0 (DONE, validated locally):** a Vite+React+Tailwind project compiles to
  a static `dist/` with RELATIVE asset paths (`./assets/â¦`) â serves from the
  existing `/s/<slug>/` R2 path with NO hosting change. npm i ~16s, `vite build`
  ~1.5s. Key insight: the built React app is just static files â publish path is
  unchanged; only a build STEP is added.
- **Phase 1 (DONE, tested 9/9):** the container build-service â `builder/`
  ...
  contract: `POST /build {files}` â `{ok:true, files:dist}` or `{ok:false, error}`
  (compile errors returned, not crashed â feeds the Phase-4 auto-fix loop). Path
...
- **Phases remaining:** 2 = rewrite the generator so Sonnet emits a Vite/React
  ...
  3 = WorkerâContainer orchestration + R2 publish (needs **Cloudflare Containers
  enabled on the account** â owner step); 4 = build-error auto-fix loop + live code
  view + cutover from static HTML.

## 2026-07-20 â React builder Phase 2 (DONE, validated): generation format + parser
The generator contract for emitting a React/Vite PROJECT (not static HTML):
- **`builder/react-gen.mjs`** â `parseGeneratedFiles(text)` + `REACT_RULES` +
  `REACT_DEPS`. Sonnet emits the whole project as `===FILE: <path>===` blocks
  (delimiter format, NOT JSON â avoids escaping code); the parser turns that into
...
- **Two real findings baked in:** (1) **HashRouter, not BrowserRouter** â the site
...
- **Validated 11/11 end-to-end** (`react-pipeline-test.mjs`): a realistic 12-file
  ...
  â parse â resolve images â build-service â **rendered React SPA screenshotted**
  (Fraunces font, custom Tailwind theme compiled, client-side routing HomeâGallery,
  zero runtime errors). Build ~2.7s. Screenshot: scratchpad/react_home.png.
- **Still Phase 3/4:** wire into the Worker (Sonnet call w/ REACT_RULES â parse â
  image inject â Cloudflare Container build â R2), needs Containers on the account
  (confirmed available) + image-token injection at source level; then auto-fix loop.

## 2026-07-20 â React builder Phase 3a: container infra + health check (DEPLOY TEST)
First change that touches the LIVE deploy config â done minimally + reversibly to
...
- **Deploy risk:** a bad container-config field fails `wrangler deploy` â the live
...
- NEXT (3b): wire Sonnet(REACT_RULES)âparseâimage-injectâcontainer buildâR2 behind
  a flag, then a live React build test.

## 2026-07-20 â React builder Phase 3b: live pipeline (SonnetâcontainerâR2)
...
- **worker.js:** `import {parseGeneratedFiles,REACT_RULES} from "./builder/react-gen.mjs"`
  (wrangler bundles it). New `injectReactImages()` (source-level @@IMG@@ â generated
  hosted URLs, budgeted). `R2_MIME` map. **`/s/<slug>/` serve route extended** to
  serve React dist: root/no-ext â index.html, a path WITH an extension â that exact
  object w/ real content-type (assets/*.js|css). Static sites unchanged (no-ext â
  .html) â unit-tested 7/7.
- **`POST /api/site/react-build`** (auth): Sonnet(REACT_RULES, max 32k) â
  parseGeneratedFiles â injectReactImages â container `/build` â store dist to
  sites/<slug>/â¦ â returns {url:/s/<slug>/, cost, files, buildMs}. Metered at
  ...
  NOT wired into the client yet â tested by direct call.
- Live paid test (one real React build â Sonnet + up to 6 images â ~$1) pending
  owner go.
