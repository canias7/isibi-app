...
export const GAME_3D_RULES =
  ...
  "ASSETS — USE A REAL MODEL FOR CHARACTERS (this is the single biggest quality win, do NOT skip it): a pack of ready CC0 3D models ships with the build. If your game has ANY character — the player's on-screen body OR enemies/creatures — you MUST load a model for them instead of a bare box; box enemies look unfinished. Add the two OPTIONAL imports above and load it: `const res = await SceneLoader.ImportMeshAsync(\"\", \"models/\", \"robot.glb\", scene); const root = res.meshes[0]; root.scaling.setAll(0.5); root.position = new Vector3(x, 0, z);` (RobotExpressive stands ~1.8 units tall — scale to taste). AVAILABLE MODELS: `robot.glb` — a rigged humanoid robot. Use it for the PLAYER body and, cloned + tinted (via a StandardMaterial on the child meshes) + scaled, for the ENEMIES that chase the player. To make an enemy pickable for shooting, set `mesh.isPickable = true` and `mesh.metadata = { enemy: true }` on the loaded ROOT (and rely on descendant picking) or on a small invisible box you parent to it. Reference a model ONLY by a `models/<name>.glb` path from this list (unknown names 404). Models load ASYNC — build the primitive arena + lights FIRST so frame 1 renders, then add the models when the promise resolves; guard the `.then()` so a load failure falls back to a primitive and never crashes. Use primitives for the ground, walls, projectiles, pickups, and HUD. Only fall back to a box for a character if NO available model fits the theme. " +
  "ANIMATIONS (the models are RIGGED — a walking/attacking enemy beats a sliding statue, so animate them): each model ships with named animation clips. robot.glb has: Idle, Walking, Running, Jump, WalkJump, Punch, Death, Wave, Yes, No, ThumbsUp, Sitting, Standing, Dance. " +
