...
export const GAME_3D_RULES =
  ...
  "MODULE PATTERN (follow EXACTLY): `src/main.js` gets the canvas `const canvas = document.getElementById(\"game\")`, then creates the engine using WebGPU when available with a WebGL fallback (COPY THIS EXACT block — the top-level `await` is fine, the build target is esnext): `let engine; try { if (await WebGPUEngine.IsSupportedAsync) { engine = new WebGPUEngine(canvas, { antialias: true }); await engine.initAsync(); } else { engine = new Engine(canvas, true); } } catch (e) { engine = new Engine(canvas, true); }` — the try/catch MUST wrap it so a browser that reports WebGPU but fails to init still falls back to WebGL instead of crashing. Then `const scene = new Scene(engine)`, build the SYNCHRONOUS world (camera, lights, rendering pipeline, arena, props), and **register `engine.runRenderLoop(() => scene.render())` (+ the resize listener) BEFORE you `await` ANY model/asset load**. This ordering is critical: a top-level `await` (e.g. awaiting a model container) blocks every statement after it, so if `runRenderLoop` comes after that await the render loop never starts — and a GLB load can DEADLOCK (its GPU upload needs render frames to complete, but frames are blocked by the await). So the shape is: engine → scene → camera/lights/pipeline/arena → `engine.runRenderLoop(...)` → THEN load models (await them inside an `async` function or `.then()` that runs while the loop is already pumping). Other modules export functions that TAKE `scene` (and helpers) and build part of the world. " +
  ...
  "MANY independently-animated enemies — DON'T clone the mesh (skinned clones don't animate right); instead load an AssetContainer ONCE and instantiate per enemy: `const container = await SceneLoader.LoadAssetContainerAsync(\"models/\", \"robot.glb\", scene);` then for each enemy `const inst = container.instantiateModelsToScene(n => n + enemyId, false); const root = inst.rootNodes[0]; root.position = ...; inst.animationGroups.forEach(a => a.stop()); (inst.animationGroups.find(a => a.name === \"Running\") || inst.animationGroups[0]).play(true);` — each instance has its OWN `rootNodes` + `animationGroups`. IMPORTANT: load the container ONCE at startup BEFORE you spawn the first enemy (show a brief 'Loading…' overlay while it loads) so every enemy is the real animated model from the very first spawn — do NOT spawn box-placeholder enemies that pop in and get replaced (that looks broken). But do this WITHOUT blocking the render loop: `engine.runRenderLoop(...)` must ALREADY be registered, then load in an `async` IIFE — `(async () => { try { container = await SceneLoader.LoadAssetContainerAsync(\"models/\", \"robot.glb\", scene); } catch (e) { useFallback = true; } hideLoadingOverlay(); startSpawning(); })();` — so the arena renders behind the overlay while the model streams in (never a top-level `await` that sits before `runRenderLoop`, which deadlocks). ALWAYS hide the overlay in a `finally`/after the try-catch so a slow or failed load can never leave the game stuck on 'Loading…'. Keep a primitive fallback ONLY for the case the load genuinely throws. " +
  ...
  "PROPS / SET-DRESSING — USE REAL PROP MODELS, NOT BARE BOXES (this, with the character models, is what stops the arena reading as 'just squares'). A themed CC0 prop pack ships with every build; dress the arena with these real models. Pick the ONE theme matching the game's setting and use ONLY its props (do NOT mix themes; the character model soldier.glb/robot.glb is separate and always allowed):\n" +
  "  • sci-fi / space / cyber / futuristic → scifi_*: scifi_machine, scifi_battery, scifi_battery_capsule01, scifi_capsule, scifi_antenna, scifi_drone, scifi_glassscreen, scifi_plug, scifi_minipc, scifi_wall_circle, scifi_bridge.\n" +
  "  • warehouse / industrial / military / shooter / generic → warehouse_*: warehouse_box_a, warehouse_box_b, warehouse_box_c, warehouse_barrel_a, warehouse_barrel_b, warehouse_pallet_large, warehouse_pillar_a, warehouse_pillar_b, warehouse_wall, warehouse_wall_window_open, warehouse_can_a, warehouse_table_medium, warehouse_target, warehouse_target_stand_a, warehouse_coin_a.\n" +
  "  • fantasy / dungeon / medieval / castle → dungeon_*: dungeon_barrel, dungeon_barrels_stack, dungeon_crates, dungeon_chest, dungeon_chest_gold, dungeon_keg, dungeon_pillar, dungeon_table, dungeon_wall, dungeon_torch, dungeon_banner, dungeon_bottle.\n" +
  "  • city / street / modern / urban → city_*: city_dumpster, city_car, city_bench, city_streetlight, city_trafficlight, city_hydrant, city_watertower, city_crate.\n" +
  "Reference a prop ONLY by a `models/<name>.glb` path from the chosen theme's list (unknown names 404). LOAD each distinct prop's AssetContainer ONCE, then instantiate it several times. Props have DIFFERENT native sizes/origins, so AUTO-SCALE + GROUND every instance with this exact helper (declare `let pid = 0;` once): `function placeProp(c, x, z, ry, target){ const inst = c.instantiateModelsToScene(n=>n+\"_\"+(pid++), false); const root = inst.rootNodes[0]; root.position = new Vector3(x,0,z); root.rotation.y = ry; const { min, max } = root.getHierarchyBoundingVectors(); const dim = Math.max(max.x-min.x, max.y-min.y, max.z-min.z) || 1; const s = target/dim; root.scaling.setAll(s); root.position.y = -min.y*s; inst.rootNodes.forEach(r=>r.getChildMeshes().forEach(m=>sg.addShadowCaster(m))); const col = MeshBuilder.CreateBox(\"pc\"+pid, { width:(max.x-min.x)*s, height:(max.y-min.y)*s, depth:(max.z-min.z)*s }, scene); col.position = new Vector3(x,(max.y-min.y)*s/2,z); col.isVisible = false; col.checkCollisions = true; return root; }` — `target` ≈ 1.5-2.5 world units for crates/barrels/props, 3-5 for pillars/walls/machines. Place ~16-28 props: cluster some as cover, vary positions + rotations, leave clear lanes and the player's spawn/front open. Load them RENDER-LOOP-FIRST in an async pass (never a top-level await before runRenderLoop — see MODULE PATTERN). PRIMITIVES stay fine for the ground, the 4 boundary walls, raised platforms, projectiles, and pickups — but the scattered cover/detail must be real prop models. " +
  "EFFECTS / PARTICLES (use the GPU headroom for life + 'juice' — do this): FIRST make ONE reusable procedural particle sprite (no image asset): `const dot = new DynamicTexture(\"dot\", 64, scene, false); const dc = dot.getContext(); const gr = dc.createRadialGradient(32,32,0,32,32,32); gr.addColorStop(0,\"rgba(255,255,255,1)\"); gr.addColorStop(0.4,\"rgba(255,255,255,0.5)\"); gr.addColorStop(1,\"rgba(255,255,255,0)\"); dc.fillStyle = gr; dc.fillRect(0,0,64,64); dot.update();`. (1) ATMOSPHERE — a continuous ambient system of slow-floating neon embers/dust over the whole arena: `const amb = new ParticleSystem(\"amb\", 800, scene); amb.particleTexture = dot; amb.emitter = new Vector3(0,0,0); amb.minEmitBox = new Vector3(-30,0,-30); amb.maxEmitBox = new Vector3(30,10,30); amb.color1 = new Color4(1,0.5,0.8,0.9); amb.color2 = new Color4(0.3,0.85,1,0.9); amb.colorDead = new Color4(0,0,0,0); amb.minSize = 0.06; amb.maxSize = 0.22; amb.minLifeTime = 3; amb.maxLifeTime = 8; amb.emitRate = 180; amb.blendMode = ParticleSystem.BLENDMODE_ADD; amb.direction1 = new Vector3(-0.2,1,-0.2); amb.direction2 = new Vector3(0.2,1.4,0.2); amb.minEmitPower = 0.15; amb.maxEmitPower = 0.5; amb.gravity = new Vector3(0,0.08,0); amb.start();` — drifting embers make the air feel alive. (2) VFX bursts wired to gameplay (reuse the same `dot`): a MUZZLE FLASH at the camera/gun when the player shoots, a SPARK burst at the hit point when a shot connects, and a bigger burst when an enemy DIES. One-shot burst pattern: create a ParticleSystem at the position with `blendMode = ParticleSystem.BLENDMODE_ADD`, set `manualEmitCount = 40`, short `minLifeTime/maxLifeTime` (~0.3-0.5s), `start()`, then dispose it after ~600ms. Small + additive so bloom makes it pop. " +
