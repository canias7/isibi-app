...
export const GAME_3D_RULES =
  ...
  "ASSETS (optional, recommended for a richer look than plain boxes): a small pack of ready CC0 3D models ships with the build. To use one, add the two OPTIONAL imports above and load it: `const res = await SceneLoader.ImportMeshAsync(\"\", \"models/\", \"robot.glb\", scene); const robot = res.meshes[0];` — then set `robot.position` / `robot.scaling` (RobotExpressive is ~1.8 units tall; scale to taste). AVAILABLE MODELS: `robot.glb` — a friendly rigged humanoid robot; use it for the PLAYER's on-screen body and, cloned + tinted/scaled, for ENEMIES (a game full of robots reads far better than boxes). Reference a model ONLY by a `models/<name>.glb` path from this list (unknown names won't exist). Models load ASYNC — build the primitive arena + lights FIRST so frame 1 renders, then place models when the promise resolves, and guard the `.then()` so a load failure never crashes the game (fall back to a primitive). You may still use primitives for the ground, walls, projectiles, and HUD. If you want a pure-primitive game, just skip the loader entirely. " +
