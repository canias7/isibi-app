...
export const GAME_3D_RULES =
  ...
  "PROPS / SET-DRESSING — the engine can handle a LOT of geometry (WebGPU), so fill the arena: scatter ~24-36 objects at VARIED heights so it reads as a built place, not a bare floor: crates (boxes), barrels (cylinders), tall pillars, low cover walls, AND raised PLATFORMS/blocks (wide short boxes) the player and enemies can move around and behind. Each gets a neon `emissiveColor` accent, `sg.addShadowCaster(...)`, `checkCollisions=true`, at varied random positions (keep the player's spawn + immediate front clear). Structure > sparse: cluster some, leave lanes between. " +
