...
import { Bot, Loader2, Upload, Sparkles, Phone, Pencil, Trash2, Plus, Plug, Globe } from "lucide-react";
...
import { getAgent, deleteAgent, type AgentOut } from "@/lib/api";
...
// Customer dashboard agents are always locked to these providers
const CUSTOMER_LLM_PROVIDER = "anthropic";
const CUSTOMER_LLM_MODEL = "claude-haiku-4-5";
const CUSTOMER_VOICE_PROVIDER = "elevenlabs";
const CUSTOMER_VOICE_ID = "21m00Tcm4TlvDq8ikWAM"; // Rachel
const CUSTOMER_TRANSCRIBER_MODEL = "whisper-1";

const FLAT_RATE = "$0.20/min";

interface CustomerDashboardAISettingsProps {
  agents: AgentOut[];
  onAgentsRefresh: () => void;
  onEditingChange?: (isEditing: boolean) => void;
}

export default function CustomerDashboardAISettings({
  agents,
  onAgentsRefresh,
  onEditingChange,
}: CustomerDashboardAISettingsProps) {
  ...
  const [activeSubTab, setActiveSubTab] = useState<"assistant" | "phone" | "integrations">("assistant");
  ...
  const handleSaveAgent = async () => {
    ...
    try {
      ...
      const url = agentId
        ? `https://isibi-backend.onrender.com/api/agents/${agentId}`
        : "https://isibi-backend.onrender.com/api/agents";
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          // Always locked to customer defaults
          model: CUSTOMER_LLM_MODEL,
          llm_provider: CUSTOMER_LLM_PROVIDER,
          voice_provider: CUSTOMER_VOICE_PROVIDER,
          elevenlabs_voice_id: CUSTOMER_VOICE_ID,
          openai_voice: null,
          language: "en",
        }),
      });
          ...
          <div className="grid gap-4">
            {agents.map((agent) => (
              <motion.div
                ...
                className="rounded-2xl border border-border/50 bg-card/60 backdrop-blur-xl p-6 flex items-center justify-between hover:border-primary/20 transition-colors"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {agent.phone_number || "No phone number"} · {agent.business_name || "No business"}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-xs font-medium text-primary bg-primary/10 border border-primary/15 rounded-lg px-2.5 py-1">
                    {FLAT_RATE} flat
                  </span>
                  <Button variant="ghost" size="sm" onClick={() => startEdit(agent.id)}>
            ...
            <div className="grid grid-cols-3 gap-3">
              {([
                ...
                >
                  ...
                  {promptMode === mode.id && (
                    <motion.div
                      layoutId="customer-prompt-mode-indicator"
