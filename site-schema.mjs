...
export function normalizeSchema(spec) {
  ...
  // `schema gen eval`: 1 sample in 20 came back with `tables` as a STRING —
  // `stop_reason: "tool_use"` and 1,680 output tokens, so the model finished
  // normally and wrote plenty, it just serialised the array it was asked for.
  //
  // Before this it matched neither branch below and fell through to zero
  // tables, which the route reports as "that brief didn't describe anything to
  // store" — a 422 on a schema the model actually got right.
  //
  // In keeping with the rest of this function, which is the tolerant boundary
  // by design: the `else if` below already exists to accept a name→definition
  // MAP instead of a list, for exactly the same class of model habit. Parsing
  // is safe — JSON.parse evaluates nothing — and a string that does not parse
  // is left alone, so it still ends up as no tables and is reported as such.
  if (typeof t === "string") {
    try {
...
export async function applySiteSchema(uuid, spec) {
  ...
  // already run on the bare DELTA by then, so a build-revise that re-declared
  // a table without restating its flags rebuilt the read policy without their
  // clauses: a teamScope+expires+scheduled `deals` table re-declared bare
  // came out as `USING (owner_id = app_user_id())` alone — team members lost
  // each other's rows, expired rows reappeared, scheduled rows went public
  // early. Proven by executing policiesFor on both shapes. The same hoist
  // also closes the grant half: a re-declared RETIRED table used to have its
  // public grants re-emitted this apply (grantsFor saw the delta's absent
  // flag) even though the late merge kept `retired` in _meta.
  //
  // The DERIVED shapes are excluded — the loop computes its own columns/refs/
  // rules from THIS run's declaration, and handing it the stored derived
  // copies would override that computation. Scalars and feature declarations
  // only; the late merge still owns the columns union and the mergedTables
  // assembly, reusing this same read.
  let prevStored = null;
  try { prevStored = await loadSiteSchema(uuid); } catch { prevStored = null; }
  const prevByName = new Map(((prevStored && Array.isArray(prevStored.tables)) ? prevStored.tables : [])
    .filter((x) => x && x.name).map((x) => [String(x.name).toLowerCase(), x]));
  for (const t of tables) {
    const prevT = prevByName.get(String(t.name || "").toLowerCase());
    if (prevT) fillFromStored(t, prevT);
  }
  for (const t of tables) {
  ...
  // Runtime errors reported from the published site's own browser tabs, so the
  // owner can learn a page is broken from the platform rather than from
  // customers going quiet (2026-08-14 — the reports were built, packaged and
  // delivered to nobody). Created here, on every build, for the `_sessions`
  // reason: a table that exists only where the feature happened to fire 500s
  // everywhere else. Same grant status as `_secrets` — never passed through
  // `grantsFor`, so the Data API roles hold no privilege on it; only the
  // Worker's own routes read and write it.
  await sqlQuery(uuid, "CREATE TABLE IF NOT EXISTS _errors (id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, at TEXT, message TEXT, stack TEXT, route TEXT, source TEXT)");
  ...
  try {
    ...
    //
    // THE SAME READ THE PRE-DDL FILL USED, not a second one — two reads of one
    // schema are two chances to disagree about what is stored, and the fill and
    // this merge must be describing the same site.
...
/**
 * A boolean flag that can tell "absent" from "explicitly off" — `retiredOf`'s
 * pattern, generalised (2026-08-14 audit).
 *
 * `!!(def.a || def.b)` collapses an OMITTED flag to `false`, and `false` is a
 * value the absent-means-unchanged merge deliberately keeps — so a build-revise
 * that re-declared a table without restating `expires`/`scheduled`/`teamScope`/
 * `trash` recorded them as CLEARED in `_meta` and rebuilt the table's read
 * policy without their clauses: team members lost each other's rows, expired
 * offers reappeared, scheduled posts went public early. Proven by executing
 * `policiesFor` on both shapes. `undefined` when no alias is present lets the
 * merge read silence as silence; an explicit `false` still clears, which is the
...
/**
 * Fill a declared table's silences from its stored self — absent means
 * unchanged, applied BEFORE the DDL and the policies rather than only in the
 * `_meta` merge (2026-08-14 audit; see the call site in `applySiteSchema`).
 *
 * The DERIVED shapes are excluded: the DDL loop computes its own columns/refs/
 * rules from THIS run's declaration, and handing it the stored derived copies
 * would override that computation. BELT-AND-BRACES TODAY, said plainly rather
 * than left looking load-bearing: `norm.push` explicitly overrides all six of
 * these, and this run's `columns` is never undefined, so a mutation removing
 * the exclusion is inert against the current loop — it holds by an emergent
 * property of code one edit away from changing, which is exactly why it stays.
 * Null counts as "said nothing", the same rule as the late merge — the
 * normaliser turns an unmentioned optional field into an explicit null, and
 * nothing can currently express a removal, so no meaning is lost by reading
 * null as silence.
 *
 * Exported so the fix is provable by composition — normalizeSchema's bare
 * re-declaration, filled from the stored table, driven through the real
 * policiesFor — since applySiteSchema itself needs a live database.
 */
export const DERIVED_TABLE_FIELDS = new Set(["columns", "refs", "refModes", "rules", "num", "json"]);
export function fillFromStored(t, prevT) {
  if (!t || !prevT) return t;
  for (const k of Object.keys(prevT)) {
    if (DERIVED_TABLE_FIELDS.has(k)) continue;
    if (t[k] === undefined || t[k] === null) t[k] = prevT[k];
  }
  return t;
}
