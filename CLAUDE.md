...
- **AND THE TWO AXES ARE SET APART (owner, the same day: *"leave more space
  ...
  straight into the one under it. `column-gap: 1rem; row-gap: 6.5rem` now — 56px on the first cut,
  then 80 and **104px** on two further *"more space"* asks, each measured. The
  guard needed no edit across any of them, by design: it requires the row gap to
  EXCEED the column gap rather than pinning a number, so tuning the amount is
  never a test change. **The first reading of that ask was the wrong axis**
