...
- **AND SHOWING THE ROOT PUTS `package.json` IN FRONT OF EVERY CUSTOMER, where
  it reads `"name": "isibi-lovable-clone"`** — the pre-2026-08-30 brand, and a
  name that calls the product a clone of a competitor. Nothing about the change
  caused it; the change is what made it visible. Renaming ripples into
  `package-lock.json` and is a brand call, so it is **OPEN, owner's**. Two more
  the same scan found and did NOT touch: `AGENTS.md` opens "generated by isibi",
  and `src/lib/error-reporting.ts` posts `isibi:runtime-error` to a listener
  that **exists nowhere in this codebase** — a live-looking contract with
  nothing at the other end, inherited from the template's origin.
