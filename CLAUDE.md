...
- **Universal memory** — a **backend/system feature with NO front-end** (user's call): auto-learned creative taste, applied to EVERY generation across all chats (not per-chat). The composer's `write_prompt` tool returns an evolved `memory` list (durable style preferences, ≤12 short phrases) alongside the per-chat `brief`; it commits on approval (`pendingMemory`→`commitMemory`, mirroring the brief) and feeds compose/revise via `directorContext().memory` → `memoryLine` in the worker. Stored in `zephyr_memory_v1` + a per-user `user_memory` Supabase row (items jsonb, enabled bool, RLS own-row; pushMemory/pullMemory last-writer-wins; in `delete_account` + the local wipes). Gated off for audio. The Memory "space" (Builder FAB → `viewMemory`) stays an ambient placeholder — it does NOT surface or edit the memory.
