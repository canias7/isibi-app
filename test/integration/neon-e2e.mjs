...
try {
  ...
  if (enabled) {
    // What the provisioning call ANSWERS. Discarded until 2026-07-30, which meant
    // the one call that creates identity told us nothing about how a client
    // reaches it. Printed as key names with values redacted — a provisioning
    // response is exactly the shape that carries a secret, and this log is a CI
    // artifact.
    const info = enabled && enabled.info;
    const shape = (o, depth = 0) => {
      if (o === null || o === undefined) return String(o);
      if (Array.isArray(o)) return "[" + (o.length ? shape(o[0], depth + 1) + " ×" + o.length : "") + "]";
      if (typeof o !== "object") {
        const v = String(o);
        // A URL is the useful part and is not a credential; anything else long
        // and opaque is treated as one.
        if (/^https?:\/\//.test(v)) return v.slice(0, 120);
        return v.length > 24 ? "<redacted:" + v.length + ">" : v;
      }
      if (depth > 2) return "{…}";
      return "{" + Object.entries(o).map(([k, v]) => k + ": " + shape(v, depth + 1)).join(", ") + "}";
    };
    ok("enabling auth answers with something, not an empty body", !!info, JSON.stringify(info));
    console.log("   provisioning response:", shape(info));
    ...
    // owner_id is a UUID now, and this is the half a source-scanning test cannot
    // prove: that the column Postgres actually built accepts one of these ids and
    // refuses the integer it used to hold. The refusal is the important direction
    // — a half-migrated column would take the integer happily and every member
    // scope would compare the wrong kind of thing.
    if (cols.length) {
      const idCol2 = cols.find((c) => c.column_name === "id");
      if (idCol2) {
        await sqlExec(authConn, 'CREATE TABLE owner_probe (id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, "owner_id" UUID)');
        const someUuid = "3f2504e0-4f89-11d3-9a0c-0305e82c3301";
        await sqlExec(authConn, 'INSERT INTO owner_probe ("owner_id") VALUES (?)', [someUuid]);
        const back = await sqlQuery(authConn, 'SELECT "owner_id" FROM owner_probe');
        ok("a uuid owner_id round-trips", back.length === 1 && String(back[0].owner_id) === someUuid,
          JSON.stringify(back));
        // The scoping comparison the data path actually makes.
        const scoped = await sqlQuery(authConn, 'SELECT id FROM owner_probe WHERE "owner_id"=?', [someUuid]);
        ok("owner scoping matches on a uuid parameter", scoped.length === 1, JSON.stringify(scoped));
        let rejected = false;
        try { await sqlExec(authConn, 'INSERT INTO owner_probe ("owner_id") VALUES (?)', [7]); }
        catch { rejected = true; }
        ok("an INTEGER owner_id is REFUSED — the old shape cannot survive a half-migration", rejected);
        await sqlExec(authConn, "DROP TABLE owner_probe");
      }
    }
