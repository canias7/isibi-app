...
try {
  ...
  if (enabled) {
    ...
    const cols = await sqlQuery(authConn,
      "SELECT column_name, data_type, is_nullable FROM information_schema.columns" +
      " WHERE table_schema='neon_auth' AND table_name='users_sync' ORDER BY ordinal_position");
    ok("users_sync exists", cols.length > 0, JSON.stringify(cols));
    console.log("   users_sync columns:", cols.map((c) => c.column_name + ":" + c.data_type).join(", "));
    ...
    // What the site's own role may do. The whole direction rests on the database
    // being a sandbox for model-written SQL, and that is only true if this role
    // cannot reach the filesystem, shell out, or install extensions.
    for (const [what, sql] of [
      ["read a file", "SELECT pg_read_file('/etc/passwd')"],
      ["shell out via COPY", "COPY (SELECT 1) TO PROGRAM 'id'"],
      ["install an extension", "CREATE EXTENSION IF NOT EXISTS dblink"],
    ]) {
