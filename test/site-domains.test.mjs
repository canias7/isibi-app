...
test("the published bundle learns its own slug from the head", () => {
  ...
  // `composePublish` since stage 7 (2026-09-05): the meta is composed into the
  // sidecar the site's script reads, and the slug rides on it exactly as it
  // rode on the live-prefix writer this used to name.
  const call = worker.indexOf("composePublish(env, slug, dist, {");
